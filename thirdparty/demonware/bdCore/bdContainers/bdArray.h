...
/// The class ensures that the relevant constructors and destructors of \c T
/// are called as objects are pushed, popped and copied into the array. Hence
/// bdArray should typically be used to store objects that allocate memory.
/// Plain data classes will benefit from using bdFastArray which makes no
/// attempt to call constructors or destructors.
template <typename T>
class bdArray
{
	public:
		...
		/// Create a \c bdArray with space to store \a capacity elements.
		...
		inline bdArray(const bdUInt capacity = 0);
		...
		/// \param capacity[in]	The desired capacity.
		/// \param value[in]	The value with which all array elements will be
		///						initialized.
		inline bdArray(const bdUInt capacity,
					   const T &value);

		/// Copy constructor.
		/// Initializes the contents of \a this to be an exact copy of \a a.
		/// \param a[in]	The array to copy.
		inline bdArray(const bdArray<T> &a);

		/// Destructor.
		/// Calls \c ~T() for each array element and deallocates all the
		...
		/// \param a[in]	The array to copy.
		inline void operator= (const bdArray<T> &a);

		/// Get read and write access to the \a i'th element.
		/// No checks are performed to ensure \a i is less than the array size.
		...
		/// yourself.
		/// \param i[in]		The index of the desired element.
		/// \param value[out]	Will be set to be equal to the i'th element, if
		///						it exists.
		/// \return
		...
		/// \note This function calls \c operator= for each each element in the
		/// section, hence the objects in the array pointed to by \a value
		/// should have already been constructed. In this way the semantics are
		/// the same as \c get().
		...
		/// Any new elements created will be initialized with \c T's default
		/// constructor. If the \a i'th element is in range it will be set to
		/// \a value with \c operator=, otherwise it will be set to \a value
		/// with \c T's copy constructor.
		...
		/// (exclusive). This method copies the remaining elements and so does
		...
		/// Remove all occurrences of \a value from the array.
		/// If any occurences of \a value are found they are removed using
		/// \c removeSection(), so the array will not be re-ordered.
		/// \warning This method is O(n).
		/// \param value[in]	The value to match array element against.
		...
		/// Get a pointer tail of the array.
		/// The pointer returned can be used to traverse the array, and points
		/// to the one past the end of the array.
		/// No checks are performed to ensure that the last element exists.
		/// \return	A pointer the tail of the array.
		inline T* end();

	protected:

		inline void moveArrayArray(T *dest,
								   const T *src,
								   const bdUInt n) const;

		/// Copy constructs \a n elements of type \c T from \a src to \a dest.
		/// \warning This function calls placement new which may not be defined
		/// for some basic types. If this is the case \c #include <new.h> or
		/// think about using bdFastArray.
		...
		///					elements should be constructed.
		/// \param n[in]	The number of elements to be constructed.
		inline void copyConstructArrayArray(T *dest,
											const T *src,
											const bdUInt n) const;

		/// Copy constructs \a n copies of \a src to \a dest.
		/// \warning This function calls placement new which may not be defined
		/// for some basic types. If this is the case \c #include <new.h> or
		/// think about using bdFastArray.
		/// \param dest[in]	Pointer to the destination array into which the
		...
		/// \param n[in]	The number of times \a src to be copy constructed
		///					into \a dest.
		inline void copyConstructArrayObject(T *dest,
											 const T &src,
											 const bdUInt n) const;

		/// Default constructs \a n objects of type \c T into \a dest.
		/// \warning This function calls placement new which may not be defined
		/// for some basic types. If this is the case \c #include <new.h> or
		/// think about using bdFastArray.
		/// \param dest[in]	Pointer to the destination array into which the
		///					elements should be constructed.
		/// \param n[in]	The number of times \a src to be default constructed
		///					into \a dest.
		inline void defaultConstructArray(T* dest,
										  const bdUInt n) const;

		/// Copy constructs \a src into \a dest.
		/// \warning This function calls placement new which may not be defined
		/// for some basic types. If this is the case \c #include <new.h> or
		/// think about using bdFastArray.
		...
		inline void copyConstructObjectObject(T *dest,
											  const T &src) const;

		/// Destroys \a n objects of type \c T in the array pointed to by \a
		/// src. This function simply calls \c ~T() for the first \a n elements
		/// of \a src.
		/// \param src[in]	A pointer to the object array to be destroyed.
		/// \param n[in]	The number of objects to be destroyed.
		inline void destruct(T *src,
							 const bdUInt n) const;
		...
		inline T* uninitializedCopy(const bdArray<T> &a) const;
...
#include <bdCore/bdContainers/bdArray.inl>
