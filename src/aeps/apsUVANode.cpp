...
// cNodeRenderer<UVAParticle,apsUVANode>::Render - ea: 0x008167E0
template <>
void cNodeRenderer<UVAParticle, apsUVANode>::Render()
{
    apsUVANode* node = mNode;
    ...
    if (renderer->UseSortedRendering() && node->mRenderSortBuffer != NULL &&
        node->mRenderSortBuffer->capacity >=
            static_cast<unsigned int>(node->mNumParticles)) {
        apsRenderSort::SortPointers<UVAParticle>(
    ...
    for (unsigned char* bytes = iterator->GetNextParticle();
        ...
        UVAParticle* particle = reinterpret_cast<UVAParticle*>(bytes);
