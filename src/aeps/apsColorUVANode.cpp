...
// cNodeRenderer<ColorUVAParticle,apsColorUVANode>::Render - ea: 0x0081B3F0
template <>
void cNodeRenderer<ColorUVAParticle, apsColorUVANode>::Render()
{
    apsColorUVANode* node = mNode;
    ...
    if (renderer->UseSortedRendering() && node->mRenderSortBuffer != NULL &&
        node->mRenderSortBuffer->capacity >=
            static_cast<unsigned int>(node->mNumParticles)) {
        apsRenderSort::SortPointers<ColorUVAParticle>(
    ...
    for (unsigned char* bytes = iterator->GetNextParticle();
        ...
        ColorUVAParticle* particle = reinterpret_cast<ColorUVAParticle*>(bytes);
        ...
        if (velocityFacing) {
            ...
            if (side.m128_f32[0] == 0.0f && side.m128_f32[1] == 0.0f &&
                ...
                side = left.v;
            halfLeft = _mm_mul_ps(
                side, _mm_set1_ps(particle->mRadius * apsCommon::mCamera.mXFlip));
