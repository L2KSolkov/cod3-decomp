...
// cNodeRenderer<ColorRectangleParticle,apsColorRectangleNode>::Render - ea: 0x00815550
template <>
void cNodeRenderer<ColorRectangleParticle, apsColorRectangleNode>::Render()
{
    apsColorRectangleNode* node = mNode;
    ...
    if (renderer->UseSortedRendering() && node->mRenderSortBuffer != NULL &&
        node->mRenderSortBuffer->capacity >=
            static_cast<unsigned int>(node->mNumParticles)) {
        apsRenderSort::SortPointers<ColorRectangleParticle>(
    ...
    for (unsigned char* bytes = iterator->GetNextParticle();
        ...
        ColorRectangleParticle* particle =
            reinterpret_cast<ColorRectangleParticle*>(bytes);
