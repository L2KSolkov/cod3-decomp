...
// cNodeRenderer<RectangleParticle,apsRectangleNode>::Render - ea: 0x0081A170
template <>
void cNodeRenderer<RectangleParticle, apsRectangleNode>::Render()
{
    apsRectangleNode* node = mNode;
    ...
    if (renderer->UseSortedRendering() && node->mRenderSortBuffer != NULL &&
        node->mRenderSortBuffer->capacity >=
            static_cast<unsigned int>(node->mNumParticles)) {
        apsRenderSort::SortPointers<RectangleParticle>(
    ...
    for (unsigned char* bytes = iterator->GetNextParticle();
        ...
        RectangleParticle* particle = reinterpret_cast<RectangleParticle*>(bytes);
        ...
        if (velocityFacing) {
            ...
            halfLeft = _mm_add_ps(_mm_mul_ps(left.v, c),
                                  _mm_mul_ps(up.v, s));
            halfLeft = _mm_mul_ps(halfLeft, _mm_set1_ps(particle->mWidth));
            halfUp = _mm_add_ps(_mm_mul_ps(left.v, _mm_set1_ps(-sine)),
                                _mm_mul_ps(up.v, c));
            halfUp = _mm_mul_ps(halfUp, _mm_set1_ps(particle->mHeight));
        }
...
void apsRectangleNode::Render() {
    cNodeRenderer<RectangleParticle, apsRectangleNode> renderer(this);
    renderer.Render();
}
