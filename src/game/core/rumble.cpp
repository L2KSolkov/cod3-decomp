...
// ============================================================================
// HandleDb / SizedHandle template family (g.o 0x4AE210-0x4B2510)
// ============================================================================
class Entity;
class DObj;
...
template <typename T, int MAXELT, typename H>
class HandleDbFull {
public:
    struct DbElement {
        T*  mObject;  // +0x00
...
template <typename T, int MAXELT, typename H>
HandleDbFull<T, MAXELT, H>::HandleDbFull()
{
    mFreeIndices.Clear();
...
template <typename T, int MAXELT, typename H>
BitSet<1344> HandleDbFull<T, MAXELT, H>::GetAllocatedIndices() const
{
    return ~mFreeIndices;
}
template <typename T, int MAXELT, typename H>
void HandleDbFull<T, MAXELT, H>::ReleaseHandle(Handle h)
{
    if (h.mVal != 0)
    {
        unsigned int v3 = h.mVal & 0xFFF;
        if (v3 >= (unsigned int)MAXELT)
        {
...
template <typename T, int MAXELT, typename H>
T* HandleDbFull<T, MAXELT, H>::DereferenceHandle(Handle h) const
{
    unsigned int v2 = h.mVal & 0xFFF;
    if (v2 < (unsigned int)MAXELT && (h.mVal >> 12) == (unsigned int)mElements[v2].mKey)
        return mElements[v2].mObject;
    return nullptr;
}
template <typename T, int MAXELT, typename H>
T* HandleDbFull<T, MAXELT, H>::GetObject(int idx) const
{
    if ((unsigned int)idx >= (unsigned int)MAXELT)
    {
...
template <typename T, int MAXELT, typename H>
void HandleDbFull<T, MAXELT, H>::BindObjectToHandle(Handle handle, T* obj)
{
...
template class HandleDbFull<Entity, 1344, SizedHandle<12, 20>>;
template class HandleDbFull<DObj, 1344, SizedHandle<12, 20>>;
