...
// ============================================================================
// Raw XAnim entry accessors (anim.o xanim.cpp) - ported from disasm
// ============================================================================
// ea: 0x544A30
...
float XAnimGetLength(AnimTree* anims, unsigned int animIndex)
{
    if (anims == nullptr)
    {
        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        AeAssert::gCurrentFile = "c:\\cod\\code\\game\\xanim.cpp";
        AeAssert::gCurrentLine = 0xA6C;
        AeAssert::gCurrentExpr = "anims";
        if (!AeAssert::IsIgnored() && AeAssert::Assert("old cod assert"))
            __debugbreak();
    }
    XAnimEntry* entry = &anims->entries.mList[animIndex];
