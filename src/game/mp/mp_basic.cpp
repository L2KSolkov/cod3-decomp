...
public:
    ...
    void RunQuery(QueryInterface* newQuery);  // ?RunQuery@MPLiveEngine@@QAEXPAVQueryInterface@@@Z
    void FreeSlot(bool isPrivate);  // ?FreeSlot@MPLiveEngine@@QAEX_N@Z
...
// ea: 0x0072F540
bool MPUIInterface::GameListingStart()
{
    ...
    if (mGameConnectionType == kGameConnectionTypeLan)
    {
        ...
        if (mPeer == nullptr)
        {
            ...
            AeAssert::gCurrentLine = 1072;
            AeAssert::gCurrentExpr = "peer";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert("no peer"))
                __debugbreak();
        }
        ((MPLanDiscovery*)((char*)mPeer + 0x73D0))->Start();
        mLanDiscoveryActive = true;
        return true;
    }
    if (mGameConnectionType == kGameConnectionTypeOnline)
    {
        CDefaultQuery* v15 =
            (CDefaultQuery*)mem_heap_malloc(0x11E0);
        if (v15 != nullptr)
            v15 = new (v15) CDefaultQuery();
        unsigned __int64 queryMinPlayers =
            mQueryParams.mMinPlayers == (unsigned int)-1
                ? 0xFFFFFFFFFFFFFFFFull
                : mQueryParams.mMinPlayers;
        unsigned __int64 queryMaxPlayers =
            mQueryParams.mMaxPlayers == (unsigned int)-1
                ? 0x7FFFFFFFFFFFFFFFull
                : mQueryParams.mMaxPlayers;
        unsigned __int64 querySubType =
            mQueryParams.mGameSubType == (unsigned int)-1
                ? 0x7FFFFFFFFFFFFFFFull
                : mQueryParams.mGameSubType;
        unsigned __int64 queryTeamBalancing =
            mQueryParams.mTeamBalancing == (unsigned int)-1
                ? 0x7FFFFFFFFFFFFFFFull
                : mQueryParams.mTeamBalancing;
        unsigned int mFriendlyFire = mQueryParams.mFriendlyFire;
        unsigned __int64 queryFriendlyFire =
            mFriendlyFire == (unsigned int)-1 ? 0x7FFFFFFFFFFFFFFFull
                                              : mFriendlyFire;
        unsigned int mMapID = mQueryParams.mMapID;
        unsigned __int64 queryGameMap =
            mMapID == (unsigned int)-1 ? 0x7FFFFFFFFFFFFFFFull : mMapID;
        unsigned int mGameType = mQueryParams.mGameType;
        unsigned __int64 queryGameType =
            mGameType == (unsigned int)-1 ? 0x7FFFFFFFFFFFFFFFull : mGameType;
        v15->Query(queryGameType, queryGameMap, 0x7FFFFFFFFFFFFFFFull,
                   queryFriendlyFire, queryTeamBalancing, querySubType,
                   queryMaxPlayers, queryMinPlayers);
        MPLiveEngine* Handle = MPLiveEngine::GetHandle();
        Handle->RunQuery((QueryInterface*)v15);
        mLiveQueryActive = true;
        mQueryFromID = false;
        return true;
    }
    return true;
...
// ea: 0x0074DB30
void MPPlayerManager::HandleVehicleEnter(
    const bdReceivedMessage& receivedMsg)
{
    ...
    if (Player != nullptr)
    {
            ...
            && *(bool*)((char*)this + 0x4112))
        {
            ...
            int seatIdx = 0, entryIdx = 0;
            short int16Temp = 0;
            unsigned int sequence = 0;
            bool ok = false;
            if (MPUtility::ReadVehicleId(buffer, vehicleId)
                && MPUtility::ReadPlayerId(buffer, playerId)
                && MPUtility::ReadSeatIndex(buffer, seatIdx)
                && MPUtility::ReadEntryPoint(buffer, entryIdx)
                && buffer.m_ptr->readInt16(int16Temp))
                ok = true;
            int health = int16Temp;
            if (ok && buffer.m_ptr->readUInt32(sequence))
            {
                VehicleEventAddToQueue(VEHICLE_EVENT_ENTER, vehicleId,
                                       sequence, playerId, health, seatIdx,
                                       entryIdx);
    ...
    const bdReceivedMessage& receivedMsg)
{
    ...
    bdReference<bdBitBuffer> buffer = msg.m_ptr->getPayload();
    MPVote* pVote = (MPVote*)((char*)this + 0x5914);
    unsigned char voteIndex = (unsigned char)-1;
    if (!buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
        || !buffer.m_ptr->readBits(&voteIndex, 8u)
        || voteIndex != pVote->voteIndex)
        goto check_end;
    bool ok = false;
    buffer.m_ptr->readBool(ok);
    bool yes = ok;
    if (ok && pVote->mVoteType == kKickPlayer)
    {
        bdReference<bdConnection> c2 = receivedMsg.getConnection();
        MPPlayer* v8 = GetPlayer(c2);
        if (MPUIInterface::mGameConnectionType
                == kGameConnectionTypeOnline
            && v8 != nullptr && v8->mClientIndex >= 0
            && EntityManager::sInst->GetPlayer(v8->mClientIndex) != nullptr)
        {
            MPPlayer* v9 = GetPlayer(pVote->voteSubject);
            if (v9 != nullptr && v9->mClientIndex >= 0
                && EntityManager::sInst->GetPlayer(v9->mClientIndex)
                       != nullptr)
            {
                Entity* ent = EntityManager::sInst->GetPlayer(
                    v9->mClientIndex);
    ...
    {
        Entity* v14 = player->mClientIndex >= 0
                          ? EntityManager::sInst->GetPlayer(
                                player->mClientIndex)
                          : nullptr;
        if (IsLocalPlayer(v14) && !yes)
        {
            pVote->mNoVotes = (unsigned char)(pVote->mNoVotes + 32);
...
check_end:
    {
        int mNoVotes = pVote->mNoVotes;
        int mYesVotes = pVote->mYesVotes;
        int v17 = pVote->eligableVoters >> 1;
        if (mYesVotes + mNoVotes >= pVote->eligableVoters
            || mYesVotes > v17 || mNoVotes > v17)
            MPUIInterface::ResolveVote();
        if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
            delete buffer.m_ptr;
        if (msg.m_ptr != nullptr && msg.m_ptr->m_refCount-- == 1)
...
// ea: 0x00749DF0
void MPPlayerManager::HandleSpreadFire(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    if (Player != nullptr)
    {
            ...
            && *(bool*)((char*)this + 0x4112))
        {
            bdReference<bdMessage> msg = receivedMsg.getMessage();
            bdReference<bdBitBuffer> buffer = msg.m_ptr->getPayload();
            unsigned char attackingPlayerId = 0;
            float position[3];
            float gunPitch = 0.0f, gunYaw = 0.0f, spread = 0.0f;
            ...
            if (MPUtility::ReadPlayerId(buffer, attackingPlayerId))
            {
                if (buffer.m_ptr != nullptr)
                    ++buffer.m_ptr->m_refCount;
                if (MPUtility::ReadPosition(buffer, position))
                {
                    if (buffer.m_ptr != nullptr)
                        ++buffer.m_ptr->m_refCount;
                    if (MPUtility::ReadAngle(buffer, gunPitch))
                    {
                        if (buffer.m_ptr != nullptr)
                            ++buffer.m_ptr->m_refCount;
                        if (MPUtility::ReadAngle(buffer, gunYaw)
                            && buffer.m_ptr->readChar8((char&)weapon)
                            && buffer.m_ptr->readFloat32(spread)
                            && buffer.m_ptr->readFloat32(
                                coneAngleTangent)
                            && buffer.m_ptr->readInt32(seed))
                            ok = true;
                    }
                }
            }
            MPPlayer* v8 = GetPlayer(attackingPlayerId);
            if (ok && v8 != nullptr && v8->mClientIndex >= 0)
            {
                Entity* ent =
                    EntityManager::sInst->GetPlayer(v8->mClientIndex);
                Spread_Fire_Fake(ent, gunPitch, gunYaw, position,
                                 weapon, spread, coneAngleTangent, seed);
                Entity* v16 =
                    EntityManager::sInst->GetPlayer(v8->mClientIndex);
                Entity* v10 =
                    EntityManager::sInst->GetPlayer(v8->mClientIndex);
                CG_FireWeapon(v10, &v16->s, 0, weapon);
            }
            if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
                delete buffer.m_ptr;
            if (msg.m_ptr != nullptr && msg.m_ptr->m_refCount-- == 1)
...
// ============================================================================
// mp.o batch 18: options/profile menu panel setup + controls save
// ============================================================================

const char* const MPOptionsScreenMenu::kScreenTextGeoms[4] = {
    "text_title_main", "text_title_profile", "text_current_selected",
    "text_title_instructions",
};
const char* const MPOptionsScreenMenu::kScreenOptionGeoms[4] = {
    "slot_01_text_a", "slot_02_text_a", "slot_03_text_a",
    "slot_01_text_b",
};
const char* const MPProfileEditMenu::kProfileTextGeoms[4] = {
    "text_title_main", "text_title_profile", "text_option_selected",
    "text_option_instructions",
};
const char* const MPProfileEditMenu::kProfileTextOptionGeoms[5] = {
    "text_option_01", "text_option_02", "text_option_03",
    "text_option_04", "text_option_05",
};
const char* const MPProfileMainMenu::kProfileTextGeoms[6] = {
    "text_profile_01", "text_profile_02", "text_profile_03",
    "text_profile_04", "text_profile_05", "text_profile_06",
};
const char* const MPOptionsSoundMenu::kSoundTextGeoms[4] = {
    "text_title_main", "text_title_profile", "text_current_selected",
    "text_title_instructions",
};
const char* const MPOptionsSoundMenu::kSoundOptionGeoms[4] = {
    "slot_02_text_a", "FEMENU_COP_VOLUME", "FEMENU_SOUND_INST_VOLUME",
    "",
};
const char* const MPOptionsSoundMenu::kSoundUnusedGeom[23] = {
    "slot_01_arrow_left", "slot_01_arrow_right", "bkg_line_02",
    "bkg_line_01", "bkg_line_03", "bkg_line_04", "bkg_row_01",
    "bkg_row_03", "bkg_row_04", "bkg_row_05", "slot_01_text_a",
    "slot_03_text_a", "slot_03_gauge_back", "slot_03_gauge_edge",
    "slot_03_gauge_fill", "slot_04_text_a", "slot_04_gauge_back",
    "slot_04_gauge_edge", "slot_04_gauge_fill", "slot_05_text_a",
    "slot_05_gauge_back", "slot_05_gauge_edge", "slot_05_gauge_fill",
};
const char* const MPOptionsControlsMenu::kControlsTextGeoms[4] = {
    "text_title_main", "text_title_profile", "text_current_selected",
    "text_title_instructions",
};
const char* const MPOptionsControlsMenu::kControlsOptionGeoms[8] = {
    "slot_01_text_a", "slot_02_text_a", "slot_03_text_a",
    "slot_04_text_a", "slot_05_text_a", "slot_06_text_a",
    "slot_07_text_a", "slot_08_text_a",
};
const char* const MPOptionsControlsMenu::kOptionToggleStrings[2] = {
    "FEMENU_COP_DISABLE", "FEMENU_COP_ENABLE",
};
const char* const MPOptionsGameplayMenu::kGameplayTextGeoms[4] = {
    "text_title_main", "text_title_profile", "text_current_selected",
    "text_title_instructions",
};
const char* const MPOptionsGameplayMenu::kGameplayOptionGeoms[4] = {
    "slot_01_text_a", "slot_02_text_a", "slot_03_text_a",
    "slot_04_text_a",
};
const char* const MPOptionsGameplayMenu::kGameplayOptionToggleGeoms[4] = {
    "slot_01_text_b", "slot_02_text_b", "slot_03_text_b",
    "slot_04_text_b",
};
const char* const MPOptionsGameplayMenu::kGameplayOptionArrowGeoms[8] = {
    "slot_01_arrow_left", "slot_01_arrow_right", "slot_02_arrow_left",
    "slot_02_arrow_right", "slot_03_arrow_left", "slot_03_arrow_right",
    "slot_04_arrow_left", "slot_04_arrow_right",
};
const char* const MPOptionsGameplayMenu::kGameplayOptionToggleStrings[2] =
    {"FEMENU_COP_OFF", "FEMENU_COP_ON"};
const char* const MPOptionsPreferencesMenu::kTextGeoms[4] = {
    "text_title_main", "text_title_profile", "text_current_selected",
    "text_title_instructions",
};
const char* const MPOptionsPreferencesMenu::kOptionGeoms[5] = {
    "slot_01_text_a", "slot_02_text_a", "slot_03_text_a",
    "slot_04_text_a", "slot_05_text_a",
};
const char* const MPOptionsPreferencesMenu::kOnOffToggleStrings[2] = {
    "MPFRONTEND_ANY_CAPS", "MPFRONTEND_OFF",
};

typedef FEComboBox* (__thiscall* FEMenuGap28)(void* self, int a2, int a3,
                                               FEText* a4, FEText* a5,
                                               PanelQuad* a6, void* a7);
typedef FESlider* (__thiscall* FEMenuAddSlider)(void* self, int a2,
                                                 PanelQuad* a3, FEText* a4);
typedef void (__thiscall* FEMenuAddDoubleEntry)(void* self, int a2,
                                                 FEText* a3, FEText* a4);

// ea: 0x00730E40
void MPOptionsSoundMenu::SetPanelFile(PanelFile* pf)
{
    this->panel = pf;
    ...
    if (v5 != nullptr)
        v5->SetNumLines(1);
    this->helpbar1->SetText("FEMENU_HELPBAR_BACK");
    for (int i = 0; i < 4; ++i)
        this->mSoundText[i] =
            this->panel->GetTextPointer(kSoundTextGeoms[i]);
    this->panel->GetTextPointer("slot_02_text_a")
        ->SetText("FEMENU_COP_VOLUME");
    this->mSoundText[0]->SetText("FEMENU_OP_SOUND_TITLE");
    if (this->mSoundText[3] == nullptr)
    {
        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        AeAssert::gCurrentFile =
            "c:\\cod\\code\\game\\mp/ui/MPOptionsSoundMenu.cpp";
        AeAssert::gCurrentLine = 178;
        AeAssert::gCurrentExpr =
            "mSoundText[kTextOptionInstructions]";
        if (!AeAssert::IsIgnored()
            && AeAssert::Assert(
                "Could not locate the text instruction widget."))
            __debugbreak();
    }
    {
        FEText* v13 = this->mSoundText[3];
        color32 v46 = v13->GetColor();
        panel_layer layera = (panel_layer)v13->GetScaleX();
        float x1a = v13->GetY();
        float v38 = v13->GetX();
        FEMultiLineText* v15 =
            (FEMultiLineText*)mem_heap_malloc(0xA8);
    ...
    FESlider* v19 = ((FEMenuAddSlider)(*(void***)this)[11])(
        ...
        this->panel->GetTextPointer("slot_02_text_a"));
    v19->SetRange(0, 50);
    v19->mEnableSound = false;
    const char* hide[12] = {
        "bkg_line_03", "bkg_line_04", "bkg_row_04", "bkg_row_05",
        "slot_04_text_a", "slot_04_gauge_back", "slot_04_gauge_edge",
        "slot_04_gauge_fill", "slot_05_text_a", "slot_05_gauge_back",
        "slot_05_gauge_edge", "slot_05_gauge_fill",
    };
    for (int i = 0; i < 12; ++i)
        this->panel->GetPointer(hide[i])->SetShown(false);
    this->panel->GetPointer("slot_01_gauge_back")->SetShown(false);
    this->panel->GetPointer("slot_01_gauge_edge")->SetShown(false);
    this->panel->GetPointer("slot_01_gauge_fill")->SetShown(false);
    for (int i = 0; i < 23; ++i)
        this->panel->GetPointer(kSoundUnusedGeom[i])->SetShown(false);
}

// ea: 0x00731660
void MPOptionsControlsMenu::SetPanelFile(PanelFile* pf)
{
    this->panel = pf;
    FEText* TextPointer = pf->GetTextPointer("text_helpbar");
    FEMultiLineText* v5 =
    ...
    if (v5 != nullptr)
        v5->SetNumLines(1);
    this->helpbar1->SetText("MPFRONTEND_HELP_SELECT_BACK_MOVEUD");
    for (int i = 0; i < 4; ++i)
        this->mControlsText[i] =
            this->panel->GetTextPointer(kControlsTextGeoms[i]);
    this->mControlsText[0]->SetText("FEMENU_OP_CONTROLS_TITLE");
    if (this->mControlsText[3] == nullptr)
    {
        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        AeAssert::gCurrentFile =
            "c:\\cod\\code\\game\\mp/ui/MPOptionsControlsMenu.cpp";
        AeAssert::gCurrentLine = 178;
        AeAssert::gCurrentExpr =
            "mControlsText[kTextOptionInstructions]";
        if (!AeAssert::IsIgnored()
            && AeAssert::Assert(
                "Could not locate the text instruction widget."))
            __debugbreak();
    ...
    {
        FEText* v15 = this->mControlsText[3];
        color32 v81 = v15->GetColor();
        panel_layer layera = (panel_layer)v15->GetScaleX();
        float x1a = v15->GetY();
        float v65 = v15->GetX();
        FEMultiLineText* v17 =
            (FEMultiLineText*)mem_heap_malloc(0xA8);
    ...
    v38->SetRange(0, 50);
    v38->mEnableSound = false;
    for (int slot = 4; slot < 8; ++slot)
    {
        char right[32], left[32], texta[32], textb[32];
        sprintf(right, "slot_0%d_arrow_right", slot + 1);
        sprintf(left, "slot_0%d_arrow_left", slot + 1);
        sprintf(texta, "slot_0%d_text_a", slot + 1);
        sprintf(textb, "slot_0%d_text_b", slot + 1);
        ((FEMenuGap28)(*(void***)this)[10])(
            this, slot, 2,
            this->panel->GetTextPointer(textb),
            this->panel->GetTextPointer(texta),
            this->panel->GetPointer(left),
            this->panel->GetPointer(right));
    }
    for (int e = 4; e < 8; ++e)
    {
        for (int t = 0; t < 2; ++t)
        {
            Broc::string s(kOptionToggleStrings[t]);
            ((FEComboBox*)this->entries[e])->AddOption(s);
        }
    }
    this->entries[0]->SetValue(7);
    this->entries[7]->SetValue(0);
}

// ea: 0x00732220
void MPOptionsGameplayMenu::SetPanelFile(PanelFile* pf)
{
    this->panel = pf;
    FEText* TextPointer = pf->GetTextPointer("text_helpbar");
    FEMultiLineText* v5 =
    ...
    if (v5 != nullptr)
        v5->SetNumLines(1);
    this->helpbar1->SetText("MPFRONTEND_HELP_SELECT_MOVEUD");
    for (int i = 0; i < 4; ++i)
        this->mGameplayText[i] =
            this->panel->GetTextPointer(kGameplayTextGeoms[i]);
    this->mGameplayText[0]->SetText("FEMENU_OP_GAMEPLAY_TITLE");
    if (this->mGameplayText[3] == nullptr)
    {
        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        AeAssert::gCurrentFile =
            "c:\\cod\\code\\game\\mp/ui/MPOptionsGameplayMenu.cpp";
        AeAssert::gCurrentLine = 150;
        AeAssert::gCurrentExpr =
            "mGameplayText[kTextOptionInstructions]";
        if (!AeAssert::IsIgnored()
            && AeAssert::Assert(
                "Could not locate the text instruction widget."))
            __debugbreak();
    ...
    {
        FEText* v15 = this->mGameplayText[3];
        color32 col2 = v15->GetColor();
        panel_layer layera = (panel_layer)v15->GetScaleX();
        float x1a = v15->GetY();
        float v35 = v15->GetX();
        FEMultiLineText* v17 =
        ...
        if (v17 != nullptr)
            v17 = new (v17)
                FEMultiLineText(v15->GetFont(), x1a, 0.0f, 0, layera,
                                32.0f, 64, (int)col2.i, &col2);
        this->mInstructionsText = v17;
        if (v17 != nullptr)
            v17->SetNumLines(5);
    }
    for (int i = 0; i < 4; ++i)
        this->panel->GetTextPointer(kGameplayOptionGeoms[i])
            ->SetText(kGameplayOptionStrings[i]);
    for (int v21 = 0; v21 < 4; ++v21)
    {
        FEComboBox* combo = ((FEMenuGap28)(*(void***)this)[10])(
            this, v21, 2,
            this->panel->GetTextPointer(
                kGameplayOptionToggleGeoms[v21]),
            this->panel->GetTextPointer(kGameplayOptionGeoms[v21]),
            this->panel->GetPointer(
                kGameplayOptionArrowGeoms[2 * v21]),
            this->panel->GetPointer(
                kGameplayOptionArrowGeoms[2 * v21 + 1]));
        for (int t = 0; t < 2; ++t)
        {
            Broc::string s(kGameplayOptionToggleStrings[t]);
            combo->AddOption(s);
        }
    }
    this->entries[0]->SetValue(3);
    this->entries[3]->SetValue(0);
    this->panel->GetPointer("slot_05_arrow_left")->SetShown(false);
    this->panel->GetPointer("slot_05_arrow_right")->SetShown(false);
    this->panel->GetPointer("bkg_row_05")->SetShown(false);
    this->panel->GetPointer("bkg_line_04")->SetShown(false);
}

// ea: 0x007329A0
void MPOptionsPreferencesMenu::SetPanelFile(PanelFile* pf)
{
    this->panel = pf;
    FEText* TextPointer = pf->GetTextPointer("text_helpbar");
    FEMultiLineText* v5 =
    ...
    for (int i = 0; i < 4; ++i)
        this->mScreenText[i] =
            this->panel->GetTextPointer(kTextGeoms[i]);
    this->mScreenText[0]->SetText("MPFRONTEND_QUICKMATCH_PREFERENCES");
    if (this->mScreenText[3] == nullptr)
    {
        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        AeAssert::gCurrentFile =
        ...
        AeAssert::gCurrentExpr =
            "mScreenText[kTextOptionInstructions]";
        if (!AeAssert::IsIgnored()
            && AeAssert::Assert(
                "Could not locate the text instruction widget."))
            __debugbreak();
    ...
    {
        FEText* v15 = this->mScreenText[3];
        color32 v86 = v15->GetColor();
        panel_layer layera = (panel_layer)v15->GetScaleX();
        float x1a = v15->GetY();
        float v67 = v15->GetX();
        FEMultiLineText* v17 =
        ...
        if (v17 != nullptr)
            v17 = new (v17)
                FEMultiLineText(v15->GetFont(), x1a, 0.0f, 0, layera,
                                32.0f, 64, (int)v86.i, &v86);
        this->mInstructionsText = v17;
        if (v17 != nullptr)
            v17->SetNumLines(5);
    }
    for (int i = 0; i < 5; ++i)
        this->panel->GetTextPointer(kOptionGeoms[i])
            ->SetText(kOptionStrings[i]);
    FEComboBox* v27 = ((FEMenuGap28)(*(void***)this)[10])(
        this, 0, 5,
        this->panel->GetTextPointer("slot_01_text_b"),
        this->panel->GetTextPointer("slot_01_text_a"),
        this->panel->GetPointer("slot_01_arrow_left"),
        this->panel->GetPointer("slot_01_arrow_right"));
    Broc::string any("MPFRONTEND_ANY_CAPS");
    v27->AddOption(any);
    for (int i = 0; i < 4; ++i)
    {
        char buf[16];
        sprintf(buf, "%d", MPUIInterface::mMaxPlayerList[i]);
        Broc::string s(buf);
        v27->AddOptionNoLocalize(s);
    }
    this->mMapCombo = ((FEMenuGap28)(*(void***)this)[10])(
        this, 1, 7,
        this->panel->GetTextPointer("slot_02_text_b"),
        this->panel->GetTextPointer("slot_02_text_a"),
        this->panel->GetPointer("slot_02_arrow_left"),
        this->panel->GetPointer("slot_02_arrow_right"));
    this->mMapCombo->AddOption(any);
    for (int i = 0; i < 6; ++i)
    {
        Broc::string s(MPUIInterface::mGameTypeStrings[i]);
        this->mMapCombo->AddOption(s);
    }
    FEComboBox* v46 = ((FEMenuGap28)(*(void***)this)[10])(
        this, 2, 65,
        this->panel->GetTextPointer("slot_03_text_b"),
        this->panel->GetTextPointer("slot_03_text_a"),
        this->panel->GetPointer("slot_03_arrow_left"),
        this->panel->GetPointer("slot_03_arrow_right"));
    v46->AddOption(any);
    for (int j = 0; j < g_NumBaseMaps; ++j)
    {
        char MapIDbyIndex = MI_GetMapIDbyIndex(j);
        if (!MI_IsAvailableMap(MapIDbyIndex))
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile =
                "c:\\cod\\code\\game\\mp/MPUIInterface.cpp";
        ...
        Broc::string s(MI_GetMapDisplayName(MapIDbyIndex));
        v46->AddOption(s);
    }
    FEComboBox* v55 = ((FEMenuGap28)(*(void***)this)[10])(
        this, 3, 3,
        this->panel->GetTextPointer("slot_04_text_b"),
        this->panel->GetTextPointer("slot_04_text_a"),
        this->panel->GetPointer("slot_04_arrow_left"),
        this->panel->GetPointer("slot_04_arrow_right"));
    for (int t = 0; t < 2; ++t)
    {
        Broc::string s(kOnOffToggleStrings[t]);
        v55->AddOption(s);
    }
    FEComboBox* v63 = ((FEMenuGap28)(*(void***)this)[10])(
        this, 4, 3,
        this->panel->GetTextPointer("slot_05_text_b"),
        this->panel->GetTextPointer("slot_05_text_a"),
        this->panel->GetPointer("slot_05_arrow_left"),
        this->panel->GetPointer("slot_05_arrow_right"));
    for (int t = 0; t < 2; ++t)
    {
        Broc::string s(kOnOffToggleStrings[t]);
        v63->AddOption(s);
...
// ea: 0x00760230
void MPPlayerManager::HandleDropSplitScreenPlayer(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    if (Player != nullptr)
    {
            ...
            && m_ptr->getStatus() == bdConnection::BD_CONNECTED)
        {
            ...
            if (v6 != nullptr)
            {
                int mClientIndex = v6->mClientIndex;
                if (mClientIndex >= 0)
                {
                    svs.clients[mClientIndex].state =
                        (clientState_t)0;
                    DisconnectPlayer(v6);
                    bdReference<bdConnection> c =
                    ...
                    if (isLoopback)
                    {
                        ...
                        for (; i < 1; ++i)
                        {
                            if (*(unsigned char*)((char*)this + 0x4111
                                                  + i)
                                == playerId)
                                break;
                        }
                        *(unsigned char*)((char*)this + 0x4111 + i) = 16;
                        memset(&cg[i], 0, sizeof(cg_t));
                        dword_F6A290[802 * i] = 0;
                        InGameMenuSystem* IGMS =
                            g_femanager.GetIGMS(i);
                        IGMS->gap1C(IGMS, -1.0f);
                        View::UpdateNumViewports();
                        svs.clients[i].state = (clientState_t)0;
                        LocalClient::SetNumLocalClients(
                            LocalClient::NumLocalClients() - 1);
                        View::UpdateViewports(i);
                        LocalClient::UpdatePlayerPorts(-1);
                    }
                }
            }
            if (buffer.m_ptr != nullptr
...
// ea: 0x00733400
void MPProfileEditMenu::SetPanelFile(PanelFile* pf)
{
    this->panel = pf;
    FEText* TextPointer = pf->GetTextPointer("text_helpbar");
    FEMultiLineText* v5 =
    ...
    this->helpbar1->SetText("MPFRONTEND_HELP_SELECT_BACK_MOVEUD");
    for (int i = 0; i < 4; ++i)
        this->mProfileEditText[i] =
            this->panel->GetTextPointer(kProfileTextGeoms[i]);
    this->mProfileEditText[0]->SetText("FEMENU_OP_TITLE_MP");
    if (this->mProfileEditText[3] == nullptr)
    {
        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        AeAssert::gCurrentFile =
            "c:\\cod\\code\\game\\mp/ui/MPProfileEditMenu.cpp";
        AeAssert::gCurrentLine = 160;
        AeAssert::gCurrentExpr =
            "mProfileEditText[kTextOptionInstructions]";
        if (!AeAssert::IsIgnored()
            && AeAssert::Assert(
                "Could not locate the text instruction widget."))
            __debugbreak();
    ...
    {
        FEText* v12 = this->mProfileEditText[3];
        color32 v28 = v12->GetColor();
        panel_layer layera = (panel_layer)v12->GetScaleX();
        float x1a = v12->GetY();
        float v21 = v12->GetX();
        FEMultiLineText* v14 =
            (FEMultiLineText*)mem_heap_malloc(0xA8);
        if (v14 != nullptr)
            v14 = new (v14)
                FEMultiLineText(v12->GetFont(), x1a, 0.0f, 0, layera,
                                32.0f, 64, (int)v28.i, &v28);
        this->mInstructionsText = v14;
        if (v14 != nullptr)
            v14->SetNumLines(5);
    }
    for (int v16 = 0; v16 < 5; ++v16)
    {
        this->mListBox.SetItem(
            v16, 0,
            this->panel->GetTextPointer(kProfileTextOptionGeoms[v16]),
            0);
        this->mListBox.SetText(v16, 0, kProfileTextOptionStrings[v16]);
    }
    this->mListBox.SelectLine(0);
    this->mListBox.SetAllColumnsSelectable(true);
    *(bool*)((char*)&this->mListBox + 0xAC) = true;
    this->mListBox.Refresh();
}

// ea: 0x00733B60
void MPProfileMainMenu::SetPanelFile(PanelFile* pf)
{
    this->mPanel = pf;
    pf->GetPointer("bkg_main_image")->SetShown(false);
    FEText* TextPointer = this->mPanel->GetTextPointer("text_helpbar");
    FEMultiLineText* v6 =
        (FEMultiLineText*)mem_heap_malloc(0xA8);
    if (v6 != nullptr)
    {
    ...
    if (v6 != nullptr)
        v6->SetNumLines(1);
    this->mHelpBar->SetText("FEMENU_OP_HELPBAR");
    this->mPanel->GetTextPointer("text_title_main")
        ->SetText(STBManager::sInst->GetSTBString(
            "FEMENU_PROFILE_OPTIONS"));
    this->mPanel->GetTextPointer("text_title_instructions")
        ->SetText(STBManager::sInst->GetSTBString(
            "FEMENU_PROFILE_INSTRUCTIONS"));
    this->mPanel->GetTextPointer("text_option_01")
        ->SetText(STBManager::sInst->GetSTBString(
            "FEMENU_PROFILE_STICK_LAYOUT_XBOX"));
    this->mPanel->GetTextPointer("text_option_02")
        ->SetText(STBManager::sInst->GetSTBString(
            "FEMENU_PROFILE_BUTTON_LAYOUT"));
    this->mPanel->GetTextPointer("text_option_03")->SetShown(false);
    this->mPanel->GetTextPointer("text_option_set_03")->SetShown(false);
    typedef void (__thiscall* FEMenuGap10)(void* self, int a2, FEText* a3,
                                           int a4);
    for (int i = 0; i < 6; ++i)
    {
        FEText* v25 =
...
// ea: 0x00730690
void MPOptionsScreenMenu::SetPanelFile(PanelFile* pf)
{
    this->panel = pf;
    FEText* TextPointer = pf->GetTextPointer("text_helpbar");
    FEMultiLineText* v5 =
    ...
    if (v5 != nullptr)
        v5->SetNumLines(1);
    this->helpbar1->SetText("FEMENU_OP_HELPBAR");
    for (int i = 0; i < 4; ++i)
        this->mScreenText[i] =
            this->panel->GetTextPointer(kScreenTextGeoms[i]);
    this->mScreenText[0]->SetText("FEMENU_OP_SCREEN_TITLE");
    if (this->mScreenText[3] == nullptr)
    {
        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        AeAssert::gCurrentFile =
            "c:\\cod\\code\\game\\mp/ui/MPOptionsScreenMenu.cpp";
    ...
    {
        FEText* v12 = this->mScreenText[3];
        color32 v50 = v12->GetColor();
        panel_layer layera = (panel_layer)v12->GetScaleX();
        float x1a = v12->GetY();
        float v38 = v12->GetX();
        font_index v16 = v12->GetFont();
        FEMultiLineText* v17 =
            (FEMultiLineText*)mem_heap_malloc(0xA8);
        if (v17 != nullptr)
            v17 = new (v17)
                FEMultiLineText(v16, x1a, 0.0f, 0, layera, 32.0f, 64,
                                (int)v50.i, &v50);
        this->mInstructionsText = v17;
        if (v17 != nullptr)
            v17->SetNumLines(5);
    ...
    this->panel->GetTextPointer("slot_01_text_a")
        ->SetText("FEMENU_COP_SCREENSIZE");
    this->panel->GetTextPointer("slot_02_text_a")
        ->SetText("FEMENU_COP_RESOLUTION");
    PanelQuad* arrowR =
        this->panel->GetPointer("slot_01_arrow_right");
    PanelQuad* arrowL =
        this->panel->GetPointer("slot_01_arrow_left");
    FEText* textA = this->panel->GetTextPointer("slot_01_text_a");
    FEText* textB = this->panel->GetTextPointer("slot_01_text_b");
    ...
    v27->AddOption(s1);
    Broc::string s2("FEMENU_COP_SIZE_WIDESCREEN");
    v27->AddOption(s2);
    PanelQuad* arrowR2 =
        this->panel->GetPointer("slot_02_arrow_right");
    PanelQuad* arrowL2 =
    ...
    v35->AddOption(s3);
    Broc::string s4("FEMENU_COP_RES_HIDEF");
    v35->AddOption(s4);
    (*this->entries)->SetValue(1);
    this->entries[1]->SetValue(0);
}

// ea: 0x00731EF0
bool MPOptionsControlsMenu::SaveOptions()
{
    bool v2 = false;
    int locked_port = controller::inst()->locked_port;
    if (this->entries[2]->GetValue()
        != gSaveGameData[locked_port].mStubData.mHorizontalSensitivity)
    {
        gSaveGameData[controller::inst()->locked_port]
            .mStubData.mHorizontalSensitivity =
            this->entries[2]->GetValue();
        v2 = true;
    }
    if (this->entries[3]->GetValue()
        != gSaveGameData[controller::inst()->locked_port]
               .mStubData.mVerticalSensitivity)
    {
        gSaveGameData[controller::inst()->locked_port]
            .mStubData.mVerticalSensitivity =
            this->entries[3]->GetValue();
        v2 = true;
    }
    if ((this->entries[4]->GetValue() != 0)
        != gSaveGameData[controller::inst()->locked_port]
               .mStubData.mInvertAim)
    {
        gSaveGameData[controller::inst()->locked_port]
            .mStubData.mInvertAim =
            this->entries[4]->GetValue() != 0;
        v2 = true;
    }
    if ((this->entries[5]->GetValue() != 0)
        != gSaveGameData[controller::inst()->locked_port]
               .mStubData.mAdsToggle)
    {
        gSaveGameData[controller::inst()->locked_port]
            .mStubData.mAdsToggle =
            this->entries[5]->GetValue() != 0;
        v2 = true;
    }
    if ((this->entries[6]->GetValue() != 0)
        != (gSaveGameData[controller::inst()->locked_port]
                .mStubData.mTankStyle
            == 1))
    {
        gSaveGameData[controller::inst()->locked_port]
            .mStubData.mTankStyle = this->entries[6]->GetValue();
        v2 = true;
    }
    if ((this->entries[7]->GetValue() != 0)
        == gSaveGameData[controller::inst()->locked_port]
               .mStubData.mVibration)
        return v2;
    gSaveGameData[controller::inst()->locked_port]
        .mStubData.mVibration =
        this->entries[7]->GetValue() != 0;
    return true;
}

// ea: 0x0075FD50
void MPPlayerManager::HandlePickupItem(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    if (Player != nullptr)
    {
            ...
            && *(bool*)((char*)this + 0x4112))
        {
            bdReference<bdMessage> msg = receivedMsg.getMessage();
            bdReference<bdBitBuffer> buffer = msg.m_ptr->getPayload();
            int netIndex = 0, typeIndex = 0;
            unsigned char picker = 0;
            if (buffer.m_ptr->readInt32(netIndex))
            {
                if (buffer.m_ptr != nullptr)
                    ++buffer.m_ptr->m_refCount;
                if (MPUtility::ReadPlayerId(buffer, picker))
                    buffer.m_ptr->readInt32(typeIndex);
            }
            MPPlayer* v7 = GetPlayer(picker);
            if (buffer.m_ptr->testBool())
            {
                bool v8 = buffer.m_ptr->testBool();
                if (gpBrocAPI->mBrocExports.mCallbackPickupScriptItem
                    != nullptr)
                {
                    unsigned int mVal = 0;
                    if (v7 != nullptr && v7->mClientIndex >= 0)
                    {
                        Entity* Entity =
                            EntityManager::sInst->GetPlayer(
                                v7->mClientIndex);
                        if (Entity != nullptr)
                            mVal = Entity->mHandle.mHandle.mVal;
                    }
                    gpBrocAPI->mBrocExports.mCallbackPickupScriptItem(
                        netIndex, mVal, v8);
                }
            }
            else if (v7 != nullptr && v7->mClientIndex >= 0)
            {
                Entity* pickerEnt =
                    EntityManager::sInst->GetPlayer(v7->mClientIndex);
                if (pickerEnt != nullptr)
                {
                    Entity* prevOwner = nullptr;
                    if (!IsLocalPlayer(pickerEnt) || typeIndex == 1)
                    {
                        int ownerId = ((netIndex >> 11) & 0x1F) - 1;
                        MPPlayer* v14 = GetPlayer(ownerId);
                        if (v14 != nullptr)
                        {
                            ::MPEntityHandle handle;
                            handle.mValue = (unsigned short)netIndex;
                            v14->RemoveItem((EDroppedItemTypes)typeIndex,
                                            handle);
                            if (v14->mClientIndex >= 0)
                                prevOwner = EntityManager::sInst->GetPlayer(
                                    v14->mClientIndex);
                        }
                    }
                    if (gpBrocAPI->mBrocExports.mCallbackPickupItem
                        != nullptr)
                    {
                        unsigned int prevVal =
                            prevOwner != nullptr
                                ? prevOwner->mHandle.mHandle.mVal
                                : 0;
                        gpBrocAPI->mBrocExports.mCallbackPickupItem(
                            pickerEnt->mHandle.mHandle.mVal, prevVal);
                    }
                }
            }
            if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
...
// ea: 0x0074E6D0
void MPPlayerManager::HandleGameStateSCF(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    if (Player != nullptr)
    {
            ...
            && m_ptr->getStatus() == bdConnection::BD_CONNECTED)
        {
            ...
            float flagOrigin[3];
            float flagAngles[3];
            unsigned char flagHolder = 0;
            if (buffer.m_ptr->readInt32(currentFlagIndex))
            {
                if (buffer.m_ptr != nullptr)
                    ++buffer.m_ptr->m_refCount;
                if (MPUtility::ReadPosition(buffer, flagOrigin))
                {
                    if (buffer.m_ptr != nullptr)
                        ++buffer.m_ptr->m_refCount;
                    if (MPUtility::ReadAngles(buffer, flagAngles))
                    {
                        if (buffer.m_ptr != nullptr)
                            ++buffer.m_ptr->m_refCount;
                        MPUtility::ReadPlayerId(buffer, flagHolder);
                    }
                }
            }
            if (*(bool*)((char*)this + 0x4112))
            {
                if (gpBrocAPI->mBrocExports.mCallbackGameStateSCF
                    != nullptr)
                {
                    unsigned int holderVal = 0;
                    MPPlayer* v6 = GetPlayer(flagHolder);
                    if (v6 != nullptr && v6->mClientIndex >= 0)
                    {
                        Entity* ent =
                            EntityManager::sInst->GetPlayer(
                                v6->mClientIndex);
                        if (ent != nullptr)
                            holderVal = ent->mHandle.mHandle.mVal;
                    }
                    gpBrocAPI->mBrocExports.mCallbackGameStateSCF(
                        currentFlagIndex,
                        Broc::vector(flagOrigin[0], flagOrigin[1],
                                     flagOrigin[2]),
                        Broc::vector(flagAngles[0], flagAngles[1],
                                     flagAngles[2]),
                        holderVal);
                }
            }
            ...
            {
                *(int*)((char*)this + 0x5904) =
                    currentFlagIndex;  // mDefendingTeam
                *(unsigned char*)((char*)this + 0x58A8) =
                    flagHolder;  // mAlliedFlagStatus
                *(bool*)((char*)this + 0x5868) = true;  // mGameParamsSet
                *(int*)((char*)this + 0x586C) = 2;      // mGameType
                float* loc = (float*)((char*)this + 0x5894);  // mAlliedFlagLocation
                loc[0] = flagOrigin[0];
                loc[1] = flagOrigin[1];
                loc[2] = flagOrigin[2];
            }
...
// ea: 0x0074ACC0
void MPPlayerManager::HandleSpotEntity(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    if (Player != nullptr)
    {
            ...
            && *(bool*)((char*)this + 0x4112))
        {
            ...
            bool spottedPlayer = buffer.m_ptr->testBool();
            if (spottedPlayer)
            {
                unsigned char id = 0;
                if (buffer.m_ptr != nullptr)
                ...
                if (v7 == nullptr)
                    goto done;
                Entity* Entity = v7->mClientIndex >= 0
                                     ? EntityManager::sInst->GetPlayer(
                                           v7->mClientIndex)
                                     : nullptr;
                mEntity = Entity;
                if (Entity == nullptr || Entity->client == nullptr)
                    goto done;
                Entity->client->ps.spotTime = level.time;
                mEntity->client->ps.mLastSpotter.mHandle.mVal =
            ...
            {
                bool spottedVehicle = buffer.m_ptr->testBool();
                if (spottedVehicle)
                {
                    unsigned char vehId = 0;
                    if (buffer.m_ptr != nullptr)
                        ++buffer.m_ptr->m_refCount;
                    MPUtility::ReadVehicleId(buffer, vehId);
                    MPVehicle* Vehicle = GetVehicle(vehId);
                    if (Vehicle == nullptr)
                        goto done;
                    Entity* ent = (Entity*)Vehicle->mEntity;
                    mEntity = ent;
                    if (ent == nullptr || ent->scr_vehicle == nullptr)
                        goto done;
                    ent->scr_vehicle->spotTime = level.time;
                    ent->scr_vehicle->mLastSpotter.mHandle.mVal =
                        Player->mClientIndex >= 0
                            ? EntityManager::sInst->GetPlayer(
                                  Player->mClientIndex)
                                  ->mHandle.mHandle.mVal
                ...
                if (gpBrocAPI->mBrocExports.mCallbackSpotted != nullptr
                    && mEntity != nullptr)
                {
                    unsigned int spotterVal = Player->mClientIndex >= 0
                                                  ? EntityManager::sInst
                                                        ->GetPlayer(
                                                            Player
                                                                ->mClientIndex)
                                                        ...
                                                        .mHandle.mVal
                                                  : 0;
                    gpBrocAPI->mBrocExports.mCallbackSpotted(
                        mEntity->mHandle.mHandle.mVal, spotterVal);
                }
                if (!spottedPlayer && !spottedVehicle)
                {
                    AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
                    AeAssert::gCurrentFile =
                        "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
                    AeAssert::gCurrentLine = 3223;
                    AeAssert::gCurrentExpr =
                        "spottedPlayer || spottedVehicle";
                    if (!AeAssert::IsIgnored()
                        && AeAssert::Assert(
                            "Recieved a spotted message without either a vehicle or player."))
                        __debugbreak();
                }
            }
        done:
...
// ea: 0x0074BB70
void MPPlayerManager::HandleVoteCalled(
    const bdReceivedMessage& receivedMsg)
{
    if (*(bool*)((char*)this + 0x4112))
    {
        ...
        bdReference<bdBitBuffer> buffer = msg.m_ptr->getPayload();
        MPVote* pVote = (MPVote*)((char*)this + 0x5914);
        bool ok = buffer.m_ptr->readDataType(
                      bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
                  && buffer.m_ptr->readBits(&pVote->voteIndex, 8u);
        if (ok)
            ok = buffer.m_ptr->readDataType(
                     bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
                 && buffer.m_ptr->readBits(&pVote->mVoteType, 8u);
        if (ok)
            ok = buffer.m_ptr->readDataType(
                     bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
                 && buffer.m_ptr->readBits(&pVote->callerIndex, 8u);
        if (ok)
        {
            if (buffer.m_ptr->readDataType(
                    bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE))
                buffer.m_ptr->readBits(&pVote->voteSubject, 8u);
        }
        pVote->localVoted =
            *(unsigned char*)((char*)this + 0x4111) == pVote->callerIndex;
        MPPlayer* Player = GetPlayer(pVote->callerIndex);
        const char* v15;
        if (Player != nullptr && Player->mClientIndex >= 0
            && EntityManager::sInst->GetPlayer(Player->mClientIndex)
                   != nullptr)
        {
            Entity* v13 = Player->mClientIndex >= 0
                              ? EntityManager::sInst->GetPlayer(
                                    Player->mClientIndex)
                              : nullptr;
            const char* PlayerName =
                MultiplayerMgr::sInst->GetPlayerName(v13);
            v15 = va("%s \"%s %s\"", "gm", "MPGAME_VOTE_CALLED_BY",
                     PlayerName);
        }
        else
        {
            v15 = va("%s \"%s\"", "gm", "MPGAME_VOTE_CALLED");
        }
        SV_GameSendServerCommand(
            DbLinkedHandle<EntityHandleDb, Entity>(), v15);
        if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
            delete buffer.m_ptr;
        if (msg.m_ptr != nullptr && msg.m_ptr->m_refCount-- == 1)
            delete msg.m_ptr;
...
// ea: 0x0074CB90
void MPPlayerManager::HandlePlayerRespawnRequest(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    if (Player != nullptr)
    {
            ...
            && *(bool*)((char*)this + 0x4112))
        {
            ...
            bdReference<bdBitBuffer> buffer = msg.m_ptr->getPayload();
            unsigned char playerID = 0;
            bool UChar8 = buffer.m_ptr->readUChar8(playerID);
            MPPlayer* v7 = GetPlayer(playerID);
            if (v7 != nullptr && v7->mConnection.m_ptr != nullptr
                && v7->mConnection.m_ptr->getStatus()
                            ...
                            Entity* Entity =
                                v7->mClientIndex >= 0
                                    ? EntityManager::sInst->GetPlayer(
                                          v7->mClientIndex)
                                    : nullptr;
                            Entity->client->pers.nextPlayerClass =
                                (short)playerClass;
                            if (gpBrocAPI->mBrocExports
                                    .mCallbackPlayerRespawnRequest
                                != nullptr)
                            {
...
// ea: 0x007590C0
void MPPlayerManager::HandleVehicleState(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
            ...
            && m_ptr->getStatus() == bdConnection::BD_CONNECTED)
        {
            if (*(bool*)((char*)this + 0x4112))
            {
                ...
                if (MPUtility::ReadVehicleId(buffer, vehicleId))
                {
                    ...
                    if (buffer.m_ptr != nullptr)
                        ++buffer.m_ptr->m_refCount;
                    v9->deserialize(buffer, false, Player->mId);
                    v9->OnModified();
                    if (buffer.m_ptr->testBool())
                    {
                        MPVehicle* VehicleFromOccupant =
                            GetVehicleFromOccupant(Player);
                        if (VehicleFromOccupant == nullptr)
                        {
                            ...
                            AeAssert::gCurrentFile =
                                "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
                            AeAssert::gCurrentLine = 6463;
                            AeAssert::gCurrentExpr = "vehicle";
                            if (!AeAssert::IsIgnored()
                                && AeAssert::Assert(
                                    "Invalid vehicle in player"))
                                __debugbreak();
                        }
                        if (buffer.m_ptr != nullptr)
                            ++buffer.m_ptr->m_refCount;
                        Player->deserializeVehicle(
                            buffer, VehicleFromOccupant);
                        Player->OnModified();
                    }
                }
                if (buffer.m_ptr != nullptr
...
// ea: 0x0074BFB0
void MPPlayerManager::HandleSDBombOperation(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    if (Player != nullptr)
    {
            ...
            && *(bool*)((char*)this + 0x4112))
        {
            ...
            if (buffer.m_ptr != nullptr)
                ++buffer.m_ptr->m_refCount;
            MPUtility::ReadPlayerId(buffer, id);
            if (gpBrocAPI->mBrocExports.mCallbackSDBombOperation != nullptr)
            {
                if (id >= 0x10u)
                {
                    AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
                    AeAssert::gCurrentFile =
                        "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
                    AeAssert::gCurrentLine = 4230;
                    AeAssert::gCurrentExpr = "MPPlayer::IsValid(id)";
                    if (!AeAssert::IsIgnored()
                        && AeAssert::Assert(
                            "Invalid player id on SD operation"))
                        __debugbreak();
                }
                MPPlayer* v11 =
                    (MPPlayer*)((char*)this + 0x1010 + 0x310 * id);
                int mClientIndex = v11->mClientIndex;
                if (mClientIndex < 0
                           ...
                           == nullptr)
                {
                    ...
                    AeAssert::gCurrentFile =
                        "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
                    AeAssert::gCurrentLine = 4231;
                    AeAssert::gCurrentExpr =
                        "mPlayers[id].GetEntity()";
                    if (!AeAssert::IsIgnored()
                        && AeAssert::Assert(
                            "Invalid player entity on SD operation"))
                        __debugbreak();
                }
                Entity* ent = v11->mClientIndex >= 0
                                  ? EntityManager::sInst->GetPlayer(
                                        v11->mClientIndex)
                                  : nullptr;
                if (id < 0x10u && ent != nullptr)
                {
                    bool v13 = buffer.m_ptr->testBool();
                    gpBrocAPI->mBrocExports.mCallbackSDBombOperation(
                        ent->mHandle.mHandle.mVal, v13);
                }
            }
            if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
                delete buffer.m_ptr;
...
// ea: 0x0075F870
void MPPlayerManager::HandleRoundOver(
    const bdReceivedMessage& receivedMsg)
{
    ...
    if (buffer.m_ptr->readDataType(
        ...
        && buffer.m_ptr->readBits(&condition, 0x20u))
        ;
    if (buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_SIGNED_CHAR8_TYPE))
        buffer.m_ptr->readBits(&winner, 8u);
    for (int i = 0; i < 16; ++i)
    {
        MPPlayer* p = (MPPlayer*)((char*)this + 0x1010 + 0x310 * i);
        if (p->mId < 0x10u && p->mClientIndex != 0)
        {
            bool wasInVeh = p->mInVehicle;
            *(bool*)((char*)p + 0x45) = false;  // mPlaying
            if (wasInVeh)
            {
                MPVehicle* VehicleFromOccupant =
                    GetVehicleFromOccupant(p);
                if (VehicleFromOccupant != nullptr)
                {
                    Entity* ent = (Entity*)VehicleFromOccupant->mEntity;
    ...
    if (*(bool*)((char*)this + 0x4112)
        && gpBrocAPI->mBrocExports.mCallbackRoundOver != nullptr)
    {
        Broc::string teamName;
        if (winner == 2)
            teamName = "allies";
        else if (winner == 1)
            teamName = "axis";
        else
            teamName = Broc::string();
        gpBrocAPI->mBrocExports.mCallbackRoundOver(condition, teamName);
    }
    if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
        delete buffer.m_ptr;
    if (msg.m_ptr != nullptr && msg.m_ptr->m_refCount-- == 1)
...
// ea: 0x0074B770
void MPPlayerManager::HandleVehicleMantled(
    const bdReceivedMessage& receivedMsg)
{
    if (!*(bool*)((char*)this + 0x4112))
        return;
    ...
    if (MPUtility::ReadVehicleId(buffer, hitVehicleID))
    {
        ...
        if (buffer.m_ptr->readDataType(bdBitBuffer::BD_BB_BOOL_TYPE))
        {
            ...
            if (buffer.m_ptr->readBits(&b, 1u))
            {
                if (b != 0)
                {
                    ...
                    if (buffer.m_ptr != nullptr)
                        ++buffer.m_ptr->m_refCount;
                    if (!MPUtility::ReadPlayerId(buffer, killerPlayerID))
                        hasKiller = false;
                }
                readOk = true;
            }
        }
        Entity* mEntity = (Entity*)((MPVehicle*)((char*)this + 0x4120
                                                 + 0x210 * hitVehicleID))
                              ->mEntity;
        ...
        if (mEntity != nullptr)
        {
            if (hasKiller)
            {
                ...
                if (Player != nullptr)
                    killer = Player->mClientIndex >= 0
                                 ? EntityManager::sInst->GetPlayer(
                                       Player->mClientIndex)
                                 : nullptr;
            }
            if (gpBrocAPI->mBrocExports.mCallbackVehicleMantled != nullptr)
            {
                unsigned int killerVal =
                    killer != nullptr ? killer->mHandle.mHandle.mVal : 0;
                gpBrocAPI->mBrocExports.mCallbackVehicleMantled(
                    mEntity->mHandle.mHandle.mVal, killerVal);
            }
            mEntity->scr_vehicle->Mantled(killer);
        }
        else
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
...
// ea: 0x0074A040
void MPPlayerManager::HandleBulletHit(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    if (Player != nullptr)
    {
            ...
            && *(bool*)((char*)this + 0x4112))
        {
            ...
            float position[3];
            float normal[3];
            unsigned char v20 = 0, v21 = 0;
            if (buffer.m_ptr != nullptr)
                ++buffer.m_ptr->m_refCount;
            MPUtility::ReadPosition(buffer, position);
            if (buffer.m_ptr != nullptr)
                ++buffer.m_ptr->m_refCount;
            if (MPUtility::ReadNormal(buffer, normal)
                && buffer.m_ptr->readChar8((char&)v20)
                && buffer.m_ptr->readChar8((char&)v21))
            {
                ...
                origin.v.m128_f32[0] = position[0];
                origin.v.m128_f32[1] = position[1];
                origin.v.m128_f32[2] = position[2];
                CG_BulletHitEvent(v8, &origin, normal, v21, v20, nullptr);
...
// ea: 0x00757E70
void MPPlayerManager::HandlePlayerTeam(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    ...
    if (buffer.m_ptr != nullptr)
        ++buffer.m_ptr->m_refCount;
    if (MPUtility::ReadPlayerId(buffer, id)
        && buffer.m_ptr->readInt16(team)
        && buffer.m_ptr->readInt16(rank))
    {
        MPPlayer* v7 = (MPPlayer*)((char*)this + 0x1010 + 0x310 * id);
        v7->mPlayerInfoSet = true;
        v7->mTeam = team;
        v7->mRank = rank;
        if (buffer.m_ptr->testBool())
        {
            buffer.m_ptr->readString(name);
            v7->SetName(name.getBuffer());
        }
        bool v11 = buffer.m_ptr->testBool();
        v7->UpdateInGamePlayerInfo(v11, false);
    }
    if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
        delete buffer.m_ptr;
    if (msg.m_ptr != nullptr && msg.m_ptr->m_refCount-- == 1)
...
// ea: 0x0074C4E0
void MPPlayerManager::HandleAnimEvent(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    if (Player != nullptr)
    {
            ...
            && *(bool*)((char*)this + 0x4112))
        {
            ...
            bdReference<bdBitBuffer> buffer = msg.m_ptr->getPayload();
            unsigned char id = 16;
            unsigned int animEvent = 0;
            if (buffer.m_ptr != nullptr)
                ++buffer.m_ptr->m_refCount;
            bool PlayerId = MPUtility::ReadPlayerId(buffer, id);
            if (buffer.m_ptr->readRangedUInt32(animEvent, 0, 0x19u, true)
                && PlayerId)
            {
                MPPlayer* v8 = GetPlayer(id);
                if (v8 != nullptr && v8->mClientIndex >= 0
                    && EntityManager::sInst->GetPlayer(v8->mClientIndex)
                           != nullptr)
                {
                    if (animEvent >= 0x19)
                    {
                        ...
                        AeAssert::gCurrentFile =
                            "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
                        AeAssert::gCurrentLine = 4463;
                        AeAssert::gCurrentExpr =
                            "animEvent >= 0 && animEvent < MP_ANIM_MAX";
                        if (!AeAssert::IsIgnored()
                            && AeAssert::Assert(
                                "Unknown animEvent message."))
                            __debugbreak();
                    }
                    Entity* ent = EntityManager::sInst->GetPlayer(
                        v8->mClientIndex);
...
// ea: 0x0072FC70
const int MPUIInterface::GetOptionFromValue(eSetting setting, int value,
                                            eGameType gameType)
{
    int i;
    switch (setting)
    ...
    case SETTING_TIME_LIMIT:
        ...
        __debugbreak();
        return 0;
    case SETTING_SCORE_LIMIT:
        i = 0;
        if (GetScoreLimitCount(gameType) <= 0)
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPUIInterface.cpp";
            AeAssert::gCurrentLine = 1609;
            AeAssert::gCurrentExpr = "0";
            if (AeAssert::IsIgnored()
                || !AeAssert::Assert("old cod assert"))
                return 0;
        ...
        while (GetScoreLimit(i, gameType) != value)
        {
            if (++i >= GetScoreLimitCount(gameType))
            {
                AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
                AeAssert::gCurrentFile =
                    "c:\\cod\\code\\game\\mp/MPUIInterface.cpp";
        ...
        return i;
    case SETTING_ROUND_LIMIT:
        i = 0;
        while (GetRoundLimit(i) != value)
        {
            if (++i >= 1)
            {
                AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        ...
        return i;
    case SETTING_RESPAWN_TIME:
        i = 0;
        while (GetRespawnTime(i) != value)
        {
            if (++i >= 3)
            {
                AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        ...
        return i;
    case SETTING_LAST_MAN_STANDING:
        return value;
    case SETTING_MAX_PLAYERS:
        for (i = 0; i < 4; ++i)
        {
            if (GetMaxPlayers(i) == value)
                return i;
        }
        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPUIInterface.cpp";
...
// ea: 0x0073D4B0
void MPUIInterface::LoadMap(int map, bool restart, bool mapRot)
{
    PauseMenu* pause = (PauseMenu*)((void**)g_femanager.mIGMS[0]->menus)[0];
    pause->UnPause();
    SetupCvars(true);
    char command[32];
    if (mapRot)
    {
        strcpy(command, "nextmap");
    }
    else if (restart)
    {
        strcpy(command, "map_restart");
    }
    else
    {
    ...
    {
    case 0: Cvar_Set("mp_gametype", "war"); break;
    case 1: Cvar_Set("mp_gametype", "ctf"); break;
    case 2: Cvar_Set("mp_gametype", "scf"); break;
    case 3: Cvar_Set("mp_gametype", "hq"); break;
    case 4: Cvar_Set("mp_gametype", "tdm"); break;
    case 5: Cvar_Set("mp_gametype", "dm"); break;
    case 7: Cvar_Set("mp_gametype", "dom"); break;
    case 8: break;
    default:
        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPUIInterface.cpp";
        AeAssert::gCurrentLine = 2320;
        AeAssert::gCurrentExpr = "0";
        if (!AeAssert::IsIgnored()
            && AeAssert::Assert("GameType index not recognized"))
            __debugbreak();
        break;
    }
    if (!MI_IsAvailableMap((char)map))
    {
        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPUIInterface.cpp";
        AeAssert::gCurrentLine = 2325;
        AeAssert::gCurrentExpr = "MI_IsAvailableMap(map)";
        if (!AeAssert::IsIgnored()
            && AeAssert::Assert("Map index not recognized"))
            __debugbreak();
    }
    char tmpstr[128];
    sprintf(tmpstr, "%s %s\n", command, MI_GetMapShortname((char)map));
    Cbuf_AddText(tmpstr);
    BrocSys::InitMPCallbacks();
}

// ea: 0x0073C8F0
bool MPUtility::ReadUserCmd(bdReference<bdBitBuffer> buffer,
                            usercmd_s& cmd)
{
    bool ok = false;
    int intTemp = 0;
    ...
    if (buffer.m_ptr->readDataType(
        ...
        && buffer.m_ptr->readBits(&intTemp, 0x20u))
        ok = true;
    cmd.serverTime = intTemp;
    if (!ok
        || !(ok = true,
             buffer.m_ptr->readRangedInt32(intTemp, 0, 16)))
        ok = false;
    cmd.buttons = intTemp;
    if (!ok
        || !buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_SIGNED_CHAR8_TYPE)
        || !(ok = true, buffer.m_ptr->readBits(&char8Temp, 8u)))
        ok = false;
    cmd.weapon = char8Temp;
    if (ok
        && buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER32_TYPE)
        && buffer.m_ptr->readBits(&intTemp, 0x20u))
    ...
    if (ok
        ...
        ok = false;
    cmd.angles[1] = intTemp;
    if (!ok
        || !buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_SIGNED_CHAR8_TYPE)
        || !(ok = true, buffer.m_ptr->readBits(&char8Temp, 8u)))
        ok = false;
    cmd.forwardmove = char8Temp;
    if (!ok
        || !buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_SIGNED_CHAR8_TYPE)
        || !(ok = true, buffer.m_ptr->readBits(&char8Temp, 8u)))
        ok = false;
    cmd.rightmove = char8Temp;
    if (!ok
        || !buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_SIGNED_CHAR8_TYPE)
        || !(ok = true, buffer.m_ptr->readBits(&char8Temp, 8u)))
        ok = false;
    cmd.upmove = char8Temp;
    if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
        delete buffer.m_ptr;
    return ok;
}
...
// ea: 0x0073C320
void MPUtility::WritePlayerData(bdReference<bdBitBuffer> buffer,
                                const PlayerData& playerData)
{
    unsigned char id = (unsigned char)playerData.id;
    if (buffer.m_ptr != nullptr)
        ++buffer.m_ptr->m_refCount;
    MPUtility::WritePlayerId(buffer, id);
    if (buffer.m_ptr != nullptr)
        ++buffer.m_ptr->m_refCount;
    MPUtility::WritePosition(buffer, playerData.origin);
    if (buffer.m_ptr != nullptr)
        ++buffer.m_ptr->m_refCount;
    MPUtility::WriteAnglesYawPitch(buffer, playerData.angles);
    buffer.m_ptr->writeDataType(bdBitBuffer::BD_BB_BOOL_TYPE);
    unsigned char b = playerData.playing ? 0xFF : 0;
    buffer.m_ptr->writeBits(&b, 1u);
    if (buffer.m_ptr != nullptr)
        ++buffer.m_ptr->m_refCount;
    MPUtility::WriteVehicleId(buffer, playerData.vehicleId);
    buffer.m_ptr->writeRangedInt32(playerData.vehicleHealth, 0, 2000);
    if (buffer.m_ptr != nullptr)
        ++buffer.m_ptr->m_refCount;
    MPUtility::WritePlayerClass(buffer, playerData.playerClass);
    buffer.m_ptr->writeDataType(
        (bdBitBuffer::bdBitBufferDataType)0x10u);
    unsigned int len = (unsigned int)strlen(playerData.name);
    buffer.m_ptr->writeBits(playerData.name, 8 * len + 8);
    buffer.m_ptr->writeDataType(
        bdBitBuffer::BD_BB_SIGNED_INTEGER32_TYPE);
    buffer.m_ptr->writeBits(&playerData.vehicleEventSequence, 0x20u);
    buffer.m_ptr->writeDataType(
        bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE);
    unsigned char li = playerData.localIdx;
    buffer.m_ptr->writeBits(&li, 8u);
    buffer.m_ptr->writeDataType(bdBitBuffer::BD_BB_BOOL_TYPE);
    b = playerData.teamAxis ? 0xFF : 0;
    buffer.m_ptr->writeBits(&b, 1u);
    if (LiveWrapper::theWrapper->sessionState == kInSession)
    {
        buffer.m_ptr->writeBits(&playerData.livePlayer, 0x200u);
        buffer.m_ptr->writeDataType(bdBitBuffer::BD_BB_BOOL_TYPE);
        b = playerData.wasInvited ? 0xFF : 0;
        buffer.m_ptr->writeBits(&b, 1u);
    }
    if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
        delete buffer.m_ptr;
...
// ea: 0x0073CB40
void sServerCreateParams::Serialize(bdReference<bdBitBuffer> buffer)
{
    for (int v3 = 0; v3 < 24; ++v3)
    {
        unsigned char v = (unsigned char)mName[v3];
        buffer.m_ptr->writeDataType(bdBitBuffer::BD_BB_SIGNED_CHAR8_TYPE);
        buffer.m_ptr->writeBits(&v, 8u);
    }
    unsigned char v = mMapID;
...
// ea: 0x0073DDE0
bool MPGameInfo::deserialize(bdReference<bdCommonAddr> localAddr,
                             bdBitBuffer& bitBuffer)
{
    bool ok;
    if (localAddr.m_ptr != nullptr)
        ++localAddr.m_ptr->m_refCount;
    ok = bdGameInfo::deserialize(localAddr.m_ptr, bitBuffer);
    if (ok)
    {
        if (!bitBuffer.readDataType(
                bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
            || !bitBuffer.readBits(&m_publicOpen, 8u)
            || !bitBuffer.readDataType(
                bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
            || !bitBuffer.readBits(&m_privateOpen, 8u)
            || !bitBuffer.readDataType(
                bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
            || !bitBuffer.readBits(&m_publicFilled, 8u)
            || !bitBuffer.readDataType(
                bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
            || !bitBuffer.readBits(&m_privateFilled, 8u)
            || (bitBuffer.readDataType(bdBitBuffer::BD_BB_FULL_TYPE),
                !bitBuffer.readBits(mName, 0xC0u))
            || !bitBuffer.readDataType(
                bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
            || !bitBuffer.readBits(&mMapID, 8u)
            || !bitBuffer.readDataType(
                bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
            || !bitBuffer.readBits(&mGameType, 8u)
            || !bitBuffer.readDataType(
                bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
            || !bitBuffer.readBits(&mGameSubType, 8u)
            || !bitBuffer.readBool(mTeamBalancing)
            || !bitBuffer.readBool(mFriendlyFire)
            || !bitBuffer.readBool(mEnableAARVote)
            || !bitBuffer.readBool(mEnablePenaltyVote))
        {
            ok = false;
        }
    }
...
// ea: 0x0073EF30
void kuju::cBezierTrajectoryInterpolator::reset(
    const math::Position3& initialPoint, const math::Dir3& initialSpeed,
    float initialDate, const math::Position3& finalPoint,
    const math::Dir3& finalSpeed, float finalDate, bool forceLinear)
{
    mInitialDate = initialDate;
    mTimeInterval = finalDate - initialDate;
    math::Dir3 finalSpeedV = finalSpeed;
    math::Dir3 initialSpeedV = initialSpeed;
    float distV[4];
    distV[0] = finalPoint.v.m128_f32[0] - initialPoint.v.m128_f32[0];
    distV[1] = finalPoint.v.m128_f32[1] - initialPoint.v.m128_f32[1];
    distV[2] = finalPoint.v.m128_f32[2] - initialPoint.v.m128_f32[2];
    distV[3] = 0.0f;
    float dist2 = distV[0] * distV[0] + distV[1] * distV[1]
                  + distV[2] * distV[2];
    float dist = sqrtf(dist2);
    float speedDiff[3];
    speedDiff[0] = finalSpeedV.v.m128_f32[0] - initialSpeedV.v.m128_f32[0];
    speedDiff[1] = finalSpeedV.v.m128_f32[1] - initialSpeedV.v.m128_f32[1];
    speedDiff[2] = finalSpeedV.v.m128_f32[2] - initialSpeedV.v.m128_f32[2];
    float dot = dist > 0.0f
                    ? (speedDiff[0] * distV[0] + speedDiff[1] * distV[1]
                       + speedDiff[2] * distV[2])
                          / dist
                    : 0.0f;
    if (forceLinear || mTimeInterval * dot > dist2)
    {
        // tLinearInterpolator<Position3> at +0x40
        memcpy((char*)this + 0x40, &initialPoint, sizeof(math::Position3));
        *(float*)((char*)this + 0x50) = initialDate;
        memcpy((char*)this + 0x60, &finalPoint, sizeof(math::Position3));
        *(float*)((char*)this + 0x70) = finalDate;
        float inv = mTimeInterval != 0.0f ? 1.0f / mTimeInterval : 0.0f;
        float speed[4];
        speed[0] = distV[0] * inv;
        speed[1] = distV[1] * inv;
        speed[2] = distV[2] * inv;
        speed[3] = 0.0f;
        memcpy((char*)this + 0x80, speed, sizeof(math::Dir3));
        mInterpolationType = kInterpolationLinear;
    }
    else
    {
        float third = mTimeInterval * 0.33333334f;
        math::Dir3 inflexFinal;
        inflexFinal.v.m128_f32[0] = -finalSpeedV.v.m128_f32[0] * third;
        inflexFinal.v.m128_f32[1] = -finalSpeedV.v.m128_f32[1] * third;
        inflexFinal.v.m128_f32[2] = -finalSpeedV.v.m128_f32[2] * third;
        inflexFinal.v.m128_f32[3] = 0.0f;
        math::Dir3 inflexInitial;
        inflexInitial.v.m128_f32[0] = initialSpeedV.v.m128_f32[0] * third;
        inflexInitial.v.m128_f32[1] = initialSpeedV.v.m128_f32[1] * third;
        inflexInitial.v.m128_f32[2] = initialSpeedV.v.m128_f32[2] * third;
        inflexInitial.v.m128_f32[3] = 0.0f;
        ((kuju::cBezier*)(char*)this)->reset(initialPoint, inflexInitial,
                                             finalPoint, inflexFinal);
        mInterpolationType = kInterpolationBezier;
    }
}

// ea: 0x007355E0
void kuju::knetuser::cVoiceNetworkManager::getDiagnostics(
    sDiagnostics* diagnostics, const kuju::knet::sTime& time)
{
    ...
    for (unsigned int v4 = 0; v4 < 0x10; ++v4)
    {
        if (((1u << v4) & mActivePlayerVoices.mBitPlayers) != 0)
        {
            diagnostics->mPendingVoicePacketCount[v4] = 1;
            diagnostics->mTimeSinceLastPacket[v4] =
                time.mTime - mTimeVoiceActive[v4].mTime;
        }
        else
        {
            diagnostics->mPendingVoicePacketCount[v4] = 0;
            diagnostics->mTimeSinceLastPacket[v4] = 0;
        }
        int i = 0;
        for (sVoicePacket* v7 = mPendingVoicePacketList[v4]; v7 != nullptr;
             ++i)
            v7 = v7->mNext;
        diagnostics->mPacketQueueLength[v4] = i;
    }
    unsigned int v10 = 0;
    for (sVoicePendingDispatchPacket* p = mVoicePendingDispatchPacketList;
         p != nullptr; p = p->mNext)
    {
        diagnostics->mPendingDispatchPacketSource[v10++] = p->mSourcePlayer;
    }
    diagnostics->mPendingDispatchPacketCount = v10;
    diagnostics->mTimeSinceLastDispatch.mTime =
        time.mTime - mLastDispatchTime.mTime;
    memcpy(diagnostics->mRecentlyDispatchedPacketSources,
           mRecentlyDispatchedPacketSources, 10);
    memcpy(diagnostics->mRecentlyDispatchedPacketRoutes,
           mRecentlyDispatchedPacketRoutes, 10);
    memcpy(diagnostics->mRecentlyReceivedPacketSources,
           mRecentlyReceivedPacketSources, 10);
    memcpy(diagnostics->mRecentlyReceivedPacketRoutes,
           mRecentlyReceivedPacketRoutes, 10);
    diagnostics->mRecentlyDispatchedPacketIndex =
        mRecentlyDispatchedPacketIndex;
    diagnostics->mRecentlyReceivedPacketIndex =
        mRecentlyReceivedPacketIndex;
    diagnostics->mMissedPackets = mMissedPackets;
}

// ea: 0x0073F3F0
void kuju::knetuser::cVoiceNetworkManager::resetPlayer(
    unsigned long playerIndex)
{
    if (playerIndex >= 0x10)
    {
        ...
        AeAssert::gCurrentFile =
            "c:\\cod\\code\\game\\mp/knetuser/cvoicenetworkmanager.cpp";
        AeAssert::gCurrentLine = 178;
        AeAssert::gCurrentExpr = "playerIndex < 16";
        if (!AeAssert::IsIgnored()
            && AeAssert::Assert(defaultFileName))
            __debugbreak();
    }
    while (mPendingVoicePacketList[playerIndex] != nullptr)
        flushFirstPacketInList(playerIndex, 1u);
    mLastReceivedSeqID[playerIndex] = 0xFFFFFFFF;
    mLastDispatchedSeqID[playerIndex] = 0;
    unsigned int mask = 1u << playerIndex;
    for (sVoicePendingDispatchPacket* p = mVoicePendingDispatchPacketList;
         p != nullptr;)
    {
        sVoicePendingDispatchPacket* next = p->mNext;
        if ((p->mPlayersToSendTo.mBitPlayers & mask) != 0)
        {
            p->mPlayersToSendTo.mBitPlayers =
                (unsigned short)(p->mPlayersToSendTo.mBitPlayers & ~mask);
            if (p->mPlayersToSendTo.mBitPlayers == 0)
                discardVoicePendingDispatchPacket(p);
        }
        p = next;
    }
    if (mActivePlayerVoices.containsPlayer(playerIndex) != 0)
    {
        if (mNumActivePlayerVoices == 0)
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile =
                "c:\\cod\\code\\game\\mp/knetuser/cvoicenetworkmanager.cpp";
            AeAssert::gCurrentLine = 221;
            AeAssert::gCurrentExpr = "mNumActivePlayerVoices";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert(defaultFileName))
                __debugbreak();
        }
        mActivePlayerVoices.removePlayer(playerIndex);
        mTimeVoiceActive[playerIndex].mTime = 0;
        --mNumActivePlayerVoices;
    }
    for (int i = 0; i < 16; ++i)
        mPlayerDistances[playerIndex][i] = -1.0f;
}

// ea: 0x0073FA30
void kuju::knetuser::cVoiceNetworkManager::
    updateVoiceBandwidthSimulation(const kuju::knet::sTime& time)
{
    static bool s_initialized = false;
    static kuju::knet::sTime s_interval;
    if (!s_initialized)
    {
        s_initialized = true;
        s_interval.mTime = 200;
    }
    if (time.mTime - mLastTime.mTime > s_interval.mTime)
    {
        ...
        if (mLastTime.mTime != 0)
            mLastTime.mTime = s_interval.mTime + mLastTime.mTime;
        else
            mLastTime = time;
        unsigned long firstPlayer = 0, secondPlayer = 0;
        getPlayerIndicesToSendTo(firstPlayer, secondPlayer);
        for (int i = 27; i != 0; --i)
        {
            short v = -1024;
            buffer.m_ptr->writeDataType(
        ...
        if (firstPlayer != (unsigned long)-1)
        {
            MPPlayerSet players;
            players.mBitPlayers = (unsigned short)(1u << firstPlayer);
            if (msg != nullptr)
                ++msg->m_refCount;
            ...
            if (msg != nullptr)
                ++msg->m_refCount;
            pm->Send(message, players, false);
            if (secondPlayer != (unsigned long)-1)
            {
                MPPlayerSet players2;
                players2.mBitPlayers = (unsigned short)(1u << secondPlayer);
                if (msg != nullptr)
                    ++msg->m_refCount;
                pm->Send(message, players2, false);
                if (msg != nullptr)
                    ++msg->m_refCount;
                pm->Send(message, players2, false);
            }
...
// ============================================================================
// mp.o batch 18: player manager handlers
// ============================================================================

// ea: 0x007373F0
void MPPlayerManager::AddLocalPlayer(int localPlayer)
{
    bdSession* mSession = *(bdSession**)((char*)this + 0x4114);
    if (mSession != nullptr
        && mSession->getStatus() == bdSession::BD_SESSION_READY)
    {
        bdMessage* msg = new bdMessage(0x1Eu, false);
        bdReference<bdMessage> addPlayerReq;
        addPlayerReq.m_ptr = msg;
        if (msg != nullptr)
            ++msg->m_refCount;
        extern int g_NumBdMessages;
        ++g_NumBdMessages;
        ...
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE);
        unsigned char lp = (unsigned char)localPlayer;
        buffer.m_ptr->writeBits(&lp, 8u);
        if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
            delete buffer.m_ptr;
        MPLiveEngine* Handle = MPLiveEngine::GetHandle();
        bdReference<bdBitBuffer> buffer2 = msg->getPayload();
        buffer2.m_ptr->writeBool(Handle->invited);
        if (buffer2.m_ptr != nullptr && buffer2.m_ptr->m_refCount-- == 1)
            delete buffer2.m_ptr;
        if (*(unsigned char*)((char*)this + 0x4111 + localPlayer) == 17)
            return;
        bdReference<bdConnection> host = mSession->getHost();
        ...
        bool sent = mSession->sendHost(addPlayerReq, true);
        if (host.m_ptr != nullptr && host.m_ptr->m_refCount-- == 1)
            delete host.m_ptr;
        if (sent)
            *(unsigned char*)((char*)this + 0x4111 + localPlayer) = 17;
        if (msg != nullptr && msg->m_refCount-- == 1)
            delete msg;
    }
}

// ea: 0x007381B0
void MPPlayerManager::FlushBufferedMessages(
    bdReference<bdConnection> connection)
{
    int num = *(int*)((char*)this + 0x0C);  // mNumBufferedMessages
    for (int v3 = 0; v3 < num; ++v3)
    {
        bdConnection* c =
            *(bdConnection**)((char*)this + 0x810 + 4 * v3);
        if (c == connection.m_ptr)
        {
            if (c != nullptr && c->m_refCount-- == 1)
                delete c;
            *(bdConnection**)((char*)this + 0x810 + 4 * v3) = nullptr;
            bdMessage* m =
                *(bdMessage**)((char*)this + 0x10 + 4 * v3);
            if (m != nullptr && m->m_refCount-- == 1)
                delete m;
            *(bdMessage**)((char*)this + 0x10 + 4 * v3) = nullptr;
        }
    }
    int new_count = 0;
    for (int v9 = 0; v9 < num; ++v9)
    {
        bdConnection** conns =
            (bdConnection**)((char*)this + 0x810);
        bdMessage** msgs = (bdMessage**)((char*)this + 0x10);
        if (conns[v9] == nullptr)
        {
            int v11 = v9;
            while (v11 < num && conns[v11] == nullptr)
                ++v11;
            if (v11 >= num)
                break;
            if (v11 != v9)
        ...
        && player->mConnection.m_ptr != nullptr)
    {
        ...
        if (player->mInVehicle)
        {
            ...
            if (VehicleFromOccupant != nullptr)
            {
                ...
                int health = mEntity != nullptr ? mEntity->health : 0;
                VehicleFromOccupant->GetOutOfVehicle(player, health, true);
            }
        }
        if (player->mMasterClient)
        {
            ...
            {
                MPPlayer* v8 = GetPlayer(v7);
                if (v8 != nullptr && v8->mId < 0x10u
                    && v8->mConnection.m_ptr != nullptr
                    && player != v8)
                {
                    bdReference<bdConnection> other =
                        v8->GetConnection();
                    bdReference<bdConnection> mine =
                        player->GetConnection();
                    bool same = other.m_ptr == mine.m_ptr;
                    if (mine.m_ptr != nullptr
                        && mine.m_ptr->m_refCount-- == 1)
                        delete mine.m_ptr;
                    if (other.m_ptr != nullptr
                        && other.m_ptr->m_refCount-- == 1)
                        delete other.m_ptr;
                    if (same)
                    {
                        v8->mMasterClient = true;
                        break;
                    }
                }
            }
        }
        ClientDisconnect(player, false);
        if (MPUIInterface::mGameConnectionType
            == kGameConnectionTypeOnline)
        {
            XUID remotePlayer;
            memcpy(&remotePlayer, &player->xuid[0], sizeof(XUID));
            MPLiveEngine* Handle = MPLiveEngine::GetHandle();
            LiveWrapper::theWrapper->RemoveRemotePlayer(remotePlayer);
        }
        player->Reset(true);
        bdConnection* m_ptr = player->mConnection.m_ptr;
        if (m_ptr != nullptr && m_ptr->m_refCount-- == 1)
        ...
        *(bool*)((char*)player + 0x45) = false;  // mPlaying
        --*(unsigned char*)((char*)this + 0x4110);  // mNumPlayers
        if (MultiplayerMgr::sInst->mPeer != nullptr
            && ((bdSession*)((char*)MultiplayerMgr::sInst->mPeer + 0x7448))
                   ->getRole()
                   == bdSession::BD_SESSION_HOST)
        {
            int PublicOpen, PrivateOpen, PublicFilled, PrivateFilled;
            if (LiveWrapper::theWrapper->sessionState == kInSession)
            {
                MPLiveEngine* eng = MPLiveEngine::GetHandle();
                eng->FreeSlot(*(bool*)((char*)player + 0x98));  // usedPrivateSlot
            }
            if (MPUIInterface::mGameConnectionType
                == kGameConnectionTypeLan)
            {
                PrivateFilled = 0;
                PublicFilled = *(unsigned char*)((char*)this + 0x4110);
                PrivateOpen = 0;
                PublicOpen =
                    MPUIInterface::mServerParams.mMaxPlayers - PublicFilled;
            }
            else
            {
                MPLiveEngine* v21 = MPLiveEngine::GetHandle();
                MPLiveEngine* v22 = MPLiveEngine::GetHandle();
                MPLiveEngine* v23 = MPLiveEngine::GetHandle();
                MPLiveEngine* v24 = MPLiveEngine::GetHandle();
                PrivateFilled = v21->liveSession->PrivateFilled;
                PublicFilled = v22->liveSession->PublicFilled;
                PrivateOpen = v23->liveSession->PrivateOpen;
                PublicOpen = v24->liveSession->PublicOpen;
            }
            MultiplayerMgr::sInst->mPeer->UpdateNumPlayers(
                PublicOpen, PrivateOpen, PublicFilled, PrivateFilled);
        }
    }
}

// ea: 0x00759800
void MPPlayerManager::SendPlayerTeam(const MPPlayer* player,
                                     bool autoTeamBalance, bool forceSend)
{
    bdSession* mSession = *(bdSession**)((char*)this + 0x4114);
    if (mSession->getStatus() != bdSession::BD_SESSION_NOT_CONNECTED
        && (forceSend
            || mSession->getRole() == bdSession::BD_SESSION_HOST))
    {
        bdMessage* msg = new bdMessage(0x60u, false);
        bdReference<bdMessage> message;
        message.m_ptr = msg;
        if (msg != nullptr)
            ++msg->m_refCount;
        ...
        if (mClientIndex < 0
            || (v7 = EntityManager::sInst->GetPlayer(mClientIndex))
                   == nullptr)
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile =
                "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
            AeAssert::gCurrentLine = 7089;
            AeAssert::gCurrentExpr = "entity";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert("Invalid player entity"))
                __debugbreak();
        }
        unsigned char id = player->mId;
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WritePlayerId(buffer, id);
        short team = (short)v7->sentient->eTeam;
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER16_TYPE);
        buffer.m_ptr->writeBits(&team, 0x10u);
        short rank = v7->client->pers.rank;
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER16_TYPE);
        ...
        unsigned char b = 0;
        buffer.m_ptr->writeBits(&b, 1u);
        buffer.m_ptr->writeBool(autoTeamBalance);
        if (msg != nullptr)
            ++msg->m_refCount;
                                ...
                                bool hostOnly, bool autoBalance)
{
    ...
    if (mPeer == (MPPeer*)-29920)
    {
        ...
        AeAssert::gCurrentLine = 1384;
        AeAssert::gCurrentExpr = "mPeer->GetPlayerManager()";
        if (!AeAssert::IsIgnored()
            && AeAssert::Assert(
                "Peer does not yet have a PlayerManager"))
            __debugbreak();
    }
    MPPlayerManager* pm =
        (MPPlayerManager*)((char*)mPeer + 0x74E0);
    MPPlayer* v8 = pm->GetPlayer((const Entity*)player);
    if (hostOnly || !pm->IsLocalPlayer(player))
    {
        if (((bdSession*)((char*)mPeer + 0x7448))->getRole()
            != bdSession::BD_SESSION_HOST)
            return false;
        player->sentient->eTeam = (team_t)team;
        if (v8 != nullptr)
            v8->mTeam = (short)team;
        pm->SendPlayerTeam(v8, autoBalance, false);
    }
    else
    {
        if (gpBrocAPI->mBrocExports.mCallbackCanTeamChange != nullptr
            && !gpBrocAPI->mBrocExports.mCallbackCanTeamChange(
                player->mHandle.mHandle.mVal, team == TEAM_ALLIES))
        {
            if (gpBrocAPI->mBrocExports.mCallbackPlayerTeamChange != nullptr)
                gpBrocAPI->mBrocExports.mCallbackPlayerTeamChange(
                    player->mHandle.mHandle.mVal, 0, 1);
        ...
        if (v8 != nullptr)
            v8->mTeam = (short)team;
        pm->SendPlayerTeam(v8, false, true);
        if (gpBrocAPI->mBrocExports.mCallbackPlayerTeamChange != nullptr)
        {
            gpBrocAPI->mBrocExports.mCallbackPlayerTeamChange(
                player->mHandle.mHandle.mVal, autoBalance, 0);
            return true;
        }
    }
    return true;
...
// ea: 0x007438C0
void MPPeer::SendGameStateSCF(Entity* player, int currentFlagIndex,
                              ...
                              Entity* flagHolder)
{
               ...
               == bdSession::BD_SESSION_HOST)
    {
        ...
        MPPlayer* v7 = p_mPlayerManager->GetPlayer(flagHolder);
        bdMessage* msg = new bdMessage(0x3Cu, false);
        bdReference<bdMessage> message;
        message.m_ptr = msg;
        if (msg != nullptr)
            ++msg->m_refCount;
        ...
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER32_TYPE);
        buffer.m_ptr->writeBits(&currentFlagIndex, 0x20u);
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WritePosition(buffer, flagPosition);
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WriteAngles(buffer, flagAngles);
        unsigned char id = v7 != nullptr ? v7->mId : 16;
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WritePlayerId(buffer, id);
        const MPPlayer* v12 = p_mPlayerManager->GetPlayer(player);
        if (v12 != nullptr)
        {
            if (msg != nullptr)
                ++msg->m_refCount;
            p_mPlayerManager->SendPlayer(v12, message, true);
        }
        else
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
...
// ea: 0x00743CF0
void MPPeer::SendGameStateSD(Entity* player, Entity* planter,
                             Entity* defuser, bool planted,
                             const math::Position3& bombPosition,
                             const math::Dir3& bombAngles, int bombTimeLeft)
{
    if (!IsLocalPlayer(player)
        && ((bdSession*)((char*)this + 0x7448))->getStatus()
               != bdSession::BD_SESSION_NOT_CONNECTED
               ...
               == bdSession::BD_SESSION_HOST)
    {
        MPPlayerManager* p_mPlayerManager =
            (MPPlayerManager*)((char*)this + 0x74E0);
        MPPlayer* v10 = p_mPlayerManager->GetPlayer(planter);
        const MPPlayer* defusing = p_mPlayerManager->GetPlayer(defuser);
        bdMessage* msg = new bdMessage(0x3Eu, false);
        bdReference<bdMessage> message;
        message.m_ptr = msg;
        if (msg != nullptr)
            ++msg->m_refCount;
        ...
        ++g_NumBdMessages;
        bdReference<bdBitBuffer> buffer = msg->getPayload();
        unsigned char pid = v10 != nullptr ? v10->mId : 16;
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WritePlayerId(buffer, pid);
        unsigned char def = defusing != nullptr ? defusing->mId : 16;
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WritePlayerId(buffer, def);
        buffer.m_ptr->writeBool(planted);
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WritePosition(buffer, bombPosition);
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WriteAngles(buffer, bombAngles);
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_UNSIGNED_INTEGER16_TYPE);
        buffer.m_ptr->writeBits(&bombTimeLeft, 0x10u);
        const MPPlayer* v18 = p_mPlayerManager->GetPlayer(player);
        if (v18 != nullptr)
        {
            if (msg != nullptr)
                ++msg->m_refCount;
            p_mPlayerManager->SendPlayer(v18, message, true);
        }
        else
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
            AeAssert::gCurrentLine = 1901;
            AeAssert::gCurrentExpr = "mpPlayer";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert(
                    "Invalid multiplayer player for player!"))
...
// ea: 0x00743AE0
void MPPeer::SendGameStateDOM(Entity* player, int flag0, int flag1,
                              int flag2, int flag3, int flag4)
{
    if (!IsLocalPlayer(player)
        ...
        && ((bdSession*)((char*)this + 0x7448))->getRole()
               == bdSession::BD_SESSION_HOST)
    {
        bdMessage* msg = new bdMessage(0x3Du, false);
        bdReference<bdMessage> message;
        message.m_ptr = msg;
        if (msg != nullptr)
            ++msg->m_refCount;
        ...
        MPPlayerManager* p_mPlayerManager =
            (MPPlayerManager*)((char*)this + 0x74E0);
        const MPPlayer* v11 = p_mPlayerManager->GetPlayer(player);
        if (v11 == nullptr)
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
            AeAssert::gCurrentLine = 1861;
            AeAssert::gCurrentExpr = "mpPlayer";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert(
                    "Invalid multiplayer player for player!"))
        ...
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER32_TYPE);
        buffer.m_ptr->writeBits(&flag0, 0x20u);
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER32_TYPE);
        buffer.m_ptr->writeBits(&flag1, 0x20u);
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER32_TYPE);
        buffer.m_ptr->writeBits(&flag2, 0x20u);
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER32_TYPE);
        buffer.m_ptr->writeBits(&flag3, 0x20u);
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER32_TYPE);
        buffer.m_ptr->writeBits(&flag4, 0x20u);
        if (v11 != nullptr)
        {
            if (msg != nullptr)
                ++msg->m_refCount;
            p_mPlayerManager->SendPlayer(v11, message, true);
        }
        if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
            delete buffer.m_ptr;
        if (message.m_ptr != nullptr && message.m_ptr->m_refCount-- == 1)
...
// ea: 0x00763E40
void sGameListing::FromXboxLive(const CDefaultResult& info)
{
    bdReference<bdCommonAddr> local =
        bdSingleton<bdNetImpl>::getInstance()->getLocalCommonAddr();
    bdReference<bdCommonAddr> address;
    if (local.m_ptr != nullptr)
        ++local.m_ptr->m_refCount;
    address.m_ptr =
        new bdCommonAddr(local.m_ptr, *(const XNADDR*)&info.HostAddress,
                         1000);
    MPGameInfo* gi = nullptr;
    if (address.m_ptr != nullptr)
        ++address.m_ptr->m_refCount;
    gi = new MPGameInfo(0x4156005Du, address,
                        (unsigned char)info.PublicFilled,
                        (unsigned char)info.PrivateFilled,
                        (unsigned char)(unsigned int)(uintptr_t)info.pQosInfo,
                        (unsigned char)info.host_name[0]);
    bdReference<MPGameInfo> gameInfo;
    gameInfo.m_ptr = gi;
    if (gi != nullptr)
        ++gi->m_refCount;
    gi->setSecurityID(*(const XNKID*)info.KeyExchangeKey);
    gi->setSecurityKey(*(const XNKEY*)&info.KeyExchangeKey[8]);
    if (mGameInfo.m_ptr != nullptr && mGameInfo.m_ptr->m_refCount-- == 1)
        delete mGameInfo.m_ptr;
    mGameInfo.m_ptr = gi;
    if (gi != nullptr)
        ++gi->m_refCount;
    ...
    mGameInfo.m_ptr->mTeamBalancing = info.team_balancing[0] != 0;
    mGameInfo.m_ptr->mFriendlyFire = info.friendly_fire[0] != 0;
    mValidVersion = 1;
    if (address.m_ptr != nullptr)
        ++address.m_ptr->m_refCount;
    mGameInfo.m_ptr->mQosProbeHandle =
        MultiplayerMgr::sInst->mPeer->AddQosProbe(
            address, gi->getSecurityID(), gi->getSecurityKey(), true);
    if (gi != nullptr && gi->m_refCount-- == 1)
        delete gi;
    if (address.m_ptr != nullptr && address.m_ptr->m_refCount-- == 1)
        delete address.m_ptr;
    if (local.m_ptr != nullptr && local.m_ptr->m_refCount-- == 1)
        delete local.m_ptr;
}

// ============================================================================
// mp.o batch 18: peer sends
// ============================================================================

// ea: 0x0075BA50
void MPPeer::VehicleFireMissile(Entity* vehEnt, int weapon,
                                const math::Position3& position,
                                const math::Dir3& dir)
{
    if (vehEnt->scr_vehicle == nullptr)
    {
        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
        AeAssert::gCurrentLine = 930;
        AeAssert::gCurrentExpr = "vehEnt->scr_vehicle";
        if (!AeAssert::IsIgnored()
            && AeAssert::Assert("old cod assert"))
            __debugbreak();
    }
        ...
        != bdSession::BD_SESSION_NOT_CONNECTED)
    {
        MPPlayerManager* p_mPlayerManager =
            (MPPlayerManager*)((char*)this + 0x74E0);
        MPVehicle* Vehicle = p_mPlayerManager->GetVehicle(vehEnt);
        if (Vehicle != nullptr)
        {
            bdMessage* msg = new bdMessage(0x4Du, false);
            bdReference<bdMessage> message;
            message.m_ptr = msg;
            if (msg != nullptr)
                ++msg->m_refCount;
            ...
            if (buffer.m_ptr != nullptr)
                ++buffer.m_ptr->m_refCount;
            MPUtility::WriteVector(buffer, &dir.v.m128_f32[0]);
            buffer.m_ptr->writeDataType(bdBitBuffer::BD_BB_SIGNED_CHAR8_TYPE);
...
// ea: 0x00744D20
void MPPeer::VehicleRequestOwnership(Entity* vehicle, Entity* newOwner)
{
    if (((bdSession*)((char*)this + 0x7448))->getStatus()
        != bdSession::BD_SESSION_NOT_CONNECTED)
    {
        ...
        MPPlayer* mpPlayer = p_mPlayerManager->GetPlayer(newOwner);
        if (mpPlayer == nullptr)
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
            ...
            AeAssert::gCurrentExpr = "mpPlayer";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert("Could not find player"))
                __debugbreak();
        }
        bdMessage* msg = new bdMessage(0x62u, false);
        bdReference<bdMessage> message;
        message.m_ptr = msg;
        if (msg != nullptr)
            ++msg->m_refCount;
        ...
        ++g_NumBdMessages;
        bdReference<bdBitBuffer> buffer = msg->getPayload();
        unsigned char vid = v5->mId;
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WriteVehicleId(buffer, vid);
        unsigned char pid = mpPlayer->mId;
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WritePlayerId(buffer, pid);
        if (msg != nullptr)
...
// ea: 0x007422F0
void MPPeer::PlayerRespawn(Entity* player, const math::Position3& position,
                           const math::Dir3& angles, int team)
{
    ...
    {
        bdMessage* msg = new bdMessage(0x30u, false);
        bdReference<bdMessage> message;
        message.m_ptr = msg;
        if (msg != nullptr)
            ++msg->m_refCount;
        ...
        MPPlayerManager* p_mPlayerManager =
            (MPPlayerManager*)((char*)this + 0x74E0);
        unsigned char playera = p_mPlayerManager->GetPlayer(player)->mId;
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        ...
        int nextPlayerClass = player->client->pers.nextPlayerClass;
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WritePlayerClass(buffer, nextPlayerClass);
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WritePlayerTeam(buffer, (team_t)team);
        if (msg != nullptr)
            ++msg->m_refCount;
        p_mPlayerManager->SendAll(message, true, false);
        MPPlayer* v11 = p_mPlayerManager->GetPlayer(player);
        tlPrintf("Host: Sent respawn info for player %d\n", v11->mId);
        if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
            delete buffer.m_ptr;
        if (message.m_ptr != nullptr && message.m_ptr->m_refCount-- == 1)
            delete message.m_ptr;
...
// ea: 0x0075B330
void MPPeer::BulletHit(const math::Position3& position,
                       const math::Dir3& normal, unsigned char surfaceType,
                       unsigned char weapon, Entity* hitEntity)
{
    if (((bdSession*)((char*)this + 0x7448))->getStatus()
            != bdSession::BD_SESSION_NOT_CONNECTED
        && !PM_CanSimulateFiringWeapon(weapon))
    {
        bdMessage* msg = new bdMessage(0x27u, false);
        bdReference<bdMessage> message;
        message.m_ptr = msg;
        if (msg != nullptr)
            ++msg->m_refCount;
        ...
        buffer.m_ptr->writeDataType(bdBitBuffer::BD_BB_SIGNED_CHAR8_TYPE);
        unsigned char w = weapon;
        buffer.m_ptr->writeBits(&w, 8u);
        MPPlayerManager* p_mPlayerManager =
            (MPPlayerManager*)((char*)this + 0x74E0);
        if (hitEntity != nullptr && hitEntity->client != nullptr)
        {
...
// ea: 0x00743440
void MPPeer::SendGameStateHQ(Entity* player, unsigned int stage,
                             ...
                             bool alliesDefending, bool pointAIsHQ)
{
    if (!IsLocalPlayer(player)
        ...
        && ((bdSession*)((char*)this + 0x7448))->getRole()
               == bdSession::BD_SESSION_HOST)
    {
        bdMessage* msg = new bdMessage(0x3Fu, false);
        bdReference<bdMessage> message;
        message.m_ptr = msg;
        if (msg != nullptr)
            ++msg->m_refCount;
        ...
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_UNSIGNED_INTEGER32_TYPE);
        buffer.m_ptr->writeBits(&stage, 0x20u);
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WritePosition(buffer, pA);
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WritePosition(buffer, pB);
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_UNSIGNED_INTEGER32_TYPE);
        buffer.m_ptr->writeBits(&triggerIndex, 0x20u);
        buffer.m_ptr->writeBool(alliesDefending);
        buffer.m_ptr->writeBool(pointAIsHQ);
        const MPPlayer* v13 = p_mPlayerManager->GetPlayer(player);
        if (v13 != nullptr)
        {
            if (msg != nullptr)
                ++msg->m_refCount;
            p_mPlayerManager->SendPlayer(v13, message, true);
        }
        else
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
            AeAssert::gCurrentLine = 1757;
            AeAssert::gCurrentExpr = "mpPlayer";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert(
                    "Invalid multiplayer player for player!"))
...
// ============================================================================
// mp.o batch 18: player anim/state, UI interface, utility serialization,
// voice diagnostics/bandwidth, game-listing conversion
// ============================================================================

// ea: 0x0072CC20
void MPPlayer::Reset(bool reset_persistent)
{
    if (reset_persistent)
    {
        mPlayerInfoSet = false;
        mRank = 0;
        mTotalScore = 0;
        mTotalKills = 0;
        mTotalDeaths = 0;
    ...
    mNetPosture = 0;
    mNetWeapon = 0;
    memset(&mInterpolatedPosition, 0, 0x30);
    mNetHeading = 0.0f;
    mNetPitch = 0.0f;
    mInterpolationState = 0;
    mInterpolatedPitch = 0.0f;
    mInterpolatedLean = 0.0f;
    int mTime = MultiplayerMgr::sInst->getLocalTime().mTime;
    mLastLegsYawTime.mTime = mTime;
    mLastReceivedTime.mTime = mTime;
    mLastInterpolatedTime.mTime = mTime;
    mStarvationTime.mTime = mTime;
    mLastAnimState = 0;              // kAnimStateUnknown
    mEventAnimPlaying = false;
    mLastVoiceReceivedTime = 0;
    mStanceChangeTime = 0;
    bWasVehicleAnimating = true;
    for (int i = 0; i < 3; ++i)
    {
        int* yaw = (int*)((char*)this + 0x2C8 + 0x18 * i);
        yaw[-1] = 0;   // mLastLegsYawAngle
        yaw[0] = 0;    // mLegsYawing
        yaw[1] = 0;    // mLastLegsDirectionChange
        yaw[2] = 0;    // mLastLegsYawTime
        yaw[3] = MultiplayerMgr::sInst->getLocalTime().mTime;
    }
    mAverageUpdateInterval = gStartupAverageUpdateInterval.mTime * 0.001f;
    mNbReceivedMessages = 0;
    mLastAnimTime = 0;
    mVehicleEventSequence = 0;
    mInVehicle = false;
    int mClientIndex = this->mClientIndex;
    if (mClientIndex >= 0
        && EntityManager::sInst->GetPlayer(mClientIndex) != nullptr)
    {
        Entity* Player = mClientIndex >= 0
                             ? EntityManager::sInst->GetPlayer(mClientIndex)
                             : nullptr;
        G_SetAnimTree(Player, nullptr);
        DObj* mDObj = Player->mDObj;
        if (mDObj != nullptr)
        {
            AnimationPlayer* v12 = (AnimationPlayer*)mDObj->animPlayers[0];
            if (v12 != nullptr)
                v12->Reset();
        }
    }
...
{
    struct MPYawState {
        float mLastLegsYawAngle;        // +0x00
        int   mLastLegsYawAnglePrev;    // +0x04
        int   mLegsYawing;              // +0x08
        int   mLastLegsDirectionChange; // +0x0C
        int   mLastLegsYawTime;         // +0x10
        float mLegsLiftFraction;        // +0x14
    };
    int viewYaw = time.mTime - mLastLegsYawTime.mTime;
    Entity* Player = mClientIndex >= 0
                         ? EntityManager::sInst->GetPlayer(mClientIndex)
                         : nullptr;
    float mAngle = mInterpolatedHeading;
    if (yawType == 0)
        Player->client->mStepViewYaw = mAngle;
    if (Player->client == nullptr)
        return;
    float clampTolerance = bJumpPlayed ? 0.0f : 130.0f;
    float swingSpeed = 220.0f;
    MPYawState* yaw = (MPYawState*)((char*)this + 0x2C8 + 0x18 * yawType);
    if (!IsPlayerFullySeatedInVehicle(Player))
    {
        if (yaw->mLegsYawing == 0)
            yaw->mLastLegsDirectionChange = time.mTime;
        float mAngle2 = (float)(time.mTime - yaw->mLastLegsYawTime);
        if (mAngle2 * 0.001f < 0.40000001f)
            swingSpeed = powf(mAngle2 * 0.0025000002f, 2.0f) * 220.0f;
        SwingAngles(mAngle, 0.0f, clampTolerance, swingSpeed,
                    yaw->mLastLegsYawAngle, yaw->mLegsYawing, viewYaw,
                    (float)yaw->mLastLegsYawAnglePrev);
        goto update_fraction;
    }
    if (byte_100000
        & *(unsigned char*)(*(unsigned char**)((char*)Player + 596) + 244))
    {
        if (yaw->mLegsYawing == 0)
            yaw->mLastLegsDirectionChange = time.mTime;
        float mAngle2 = (float)(time.mTime - yaw->mLastLegsYawTime);
        if (mAngle2 * 0.001f < 0.40000001f)
            swingSpeed = powf(mAngle2 * 0.0025000002f, 2.0f) * 220.0f;
        float targetYawa;
        if (*(int*)((char*)Player + 980) != 0)
        {
            float parentAxis[4][3];
            G_CalcTagParentAxis(Player, parentAxis);
            targetYawa = vectoyaw(parentAxis[0]);
        }
        else
        {
            targetYawa = mInterpolatedHeading;
        }
        SwingAngles(targetYawa, 0.0f, clampTolerance, swingSpeed,
                    yaw->mLastLegsYawAngle, yaw->mLegsYawing, viewYaw,
                    (float)yaw->mLastLegsYawAnglePrev);
        goto update_fraction;
    }
    if (bIdle)
    {
        ...
        if (mAngle2 * 0.001f < 0.40000001f)
            swingSpeed = powf(mAngle2 * 0.0025000002f, 2.0f) * 220.0f;
        SwingAngles((float)targetYaw, yawThreshold, clampTolerance,
                    swingSpeed, yaw->mLastLegsYawAngle, yaw->mLegsYawing,
                    viewYaw, (float)yaw->mLastLegsYawAnglePrev);
        if (yaw->mLegsYawing != 0)
        {
            float v17 = (viewYaw / leftLegLiftDuration) * 0.001f
                        + yaw->mLegsLiftFraction;
            if (v17 >= 1.0f)
                v17 = 1.0f;
            yaw->mLegsLiftFraction = v17;
        }
        else
        {
        update_fraction:
            float v17 =
                yaw->mLegsLiftFraction
                - (viewYaw / leftLegLiftDuration) * 0.001f;
            if (v17 <= 0.0f)
                v17 = 0.0f;
            yaw->mLegsLiftFraction = v17;
        }
        yaw->mLastLegsYawAnglePrev = (int)yaw->mLastLegsYawAngle;
        return;
    }
    float v23 = yaw->mLegsLiftFraction
                - (viewYaw / leftLegLiftDuration) * 0.001f;
    if (v23 <= 0.0f)
        v23 = 0.0f;
    yaw->mLegsLiftFraction = v23;
    yaw->mLegsYawing = 1;
    if (moveYaw == -1000.0f)
    {
        float dx = mNetSpeed.v.m128_f32[0], dy = mNetSpeed.v.m128_f32[1],
              dz = mNetSpeed.v.m128_f32[2];
        float mag = sqrtf(dx * dx + dy * dy + dz * dz);
        const float* v = mag <= 0.5f ? &mInterpolatedSpeed.v.m128_f32[0]
                                     : &mNetSpeed.v.m128_f32[0];
        moveYaw = vectoyaw(v);
    }
    int dirType = 4;
    float targetYawb = moveYaw;
    if (!bJumpPlayed && bStrafing)
    {
        int v26 = (mAnimLegs - 1) & 3;
        if (v26 == 2)
        {
            dirType = 1;
            float cand = targetYawb - 90.0f;
            if (fabsf(AngleNormalize180(cand)) < 90.0f)
                targetYawb = cand;
            else
                goto no_dir;
        }
    ...
    {
        dirType = 3;
        if (fabsf(AngleNormalize180(targetYawb - (mAngle + 180.0f))) < 90.0f)
        {
            targetYawb += 180.0f;
            goto do_swing;
        }
    ...
    {
        dirType = 4;
        if (fabsf(AngleNormalize180(targetYawb - mAngle)) < 90.0f)
            goto do_swing;
    }
no_dir:
    dirType = 0;
do_swing:
    if (dirType == yaw->mLastLegsDirectionChange)
    {
        // keep current start time
    }
    else
    {
        yaw->mLastLegsDirectionChange = dirType;
        yaw->mLastLegsYawTime = time.mTime;
    }
    float mAngle2 = (float)(time.mTime - yaw->mLastLegsYawTime);
    if (mAngle2 * 0.001f < 0.40000001f)
        swingSpeed = powf(mAngle2 * 0.0025000002f, 2.0f) * 220.0f;
    float targetYawc = AngleMod(targetYawb);
    if (dirType != 0)
        SwingAngles(targetYawc, 0.0f, clampTolerance, swingSpeed,
                    yaw->mLastLegsYawAngle, yaw->mLegsYawing, viewYaw,
                    (float)yaw->mLastLegsYawAnglePrev);
    yaw->mLastLegsYawAnglePrev = (int)yaw->mLastLegsYawAngle;
}
                           ...
                           bool useLastWalking)
{
    ...
    float v23 = velocity[1];
    float angleToVelYaw = sqrtf(v23 * v23 + v22 * v22);
    float angle = vectoyaw(&v22);
    float v21 = AngleNormalize180(angle - AngleNormalize180(yaw));
    float dx = mNetSpeed.v.m128_f32[0], dy = mNetSpeed.v.m128_f32[1],
          dz = mNetSpeed.v.m128_f32[2];
    float mag = sqrtf(dx * dx + dy * dy + dz * dz);
    float v8 = mag >= 2.0f ? 2.0f : 9.0f;
    bool wasIdle = v8 > angleToVelYaw;
    bIdle = wasIdle;
    if (wasIdle)
    {
        bSprinting = false;
    ...
    {
        if (!useLastWalking && bWalking != (angleToVelYaw < 95.0f))
            bWalking = angleToVelYaw < 95.0f;
        float v11 = fabsf(v21);
        bStrafing = v11 >= 60.0f && v11 <= 120.0f;
        bBackwards = v11 > 120.0f;
        bSprinting = v11 <= 120.0f && (mAnimFlags & 0x400) != 0;
    }
    bClimbing = (mAnimFlags & 0x800) != 0;
    bInAir = (mAnimFlags & 0x1000) != 0;
    bCrouching = (mAnimFlags & 0x200) != 0;
    bProne = (mAnimFlags & 0x100) != 0;
    if (wasIdle)
    {
        if (bProne)
            return 2;
        return (mAnimFlags & 0x200) != 0 ? 1 : 0;
    }
    if (bSprinting && (mAnimFlags & 0x200) == 0 && !bStrafing
        && !bWalking)
        return 31;
    int result;
    if (bProne)
        result = 5;
    else if ((mAnimFlags & 0x200) != 0)
        result = 13;
    else
        result = 8 * !bWalking + 9;
    if (bStrafing)
...
// ============================================================================
// mp.o batch 19 - MPPeer sends / vehicle messages (MPPeer.cpp)
// ============================================================================
// ea: 0x00744550
void MPPeer::AttemptToGetInVehicle(Entity* vehicle, Entity* player,
                                   int seatIdx, int entryIdx)
{
    ...
    if (p_mSession->getStatus() != bdSession::BD_SESSION_NOT_CONNECTED)
    {
        MPPlayerManager* p_mPlayerManager =
            (MPPlayerManager*)((char*)this + 0x74E0);
        MPVehicle* v6 = p_mPlayerManager->GetVehicle(vehicle);
        if (v6 == nullptr)
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
            AeAssert::gCurrentLine = 2076;
            AeAssert::gCurrentExpr = "mpVehicle";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert("Could not find vehicle"))
                __debugbreak();
        }
        MPPlayer* mpPlayer = p_mPlayerManager->GetPlayer(player);
        if (mpPlayer != nullptr)
        {
            if (seatIdx == 7
                || v6->mNumOccupants
                       != G_GetVehicleSeatCount((Entity*)v6->mEntity))
            {
                bdMessage* msg = new bdMessage(0x49u, false);
                bdReference<bdMessage> message;
                message.m_ptr = msg;
                if (msg != nullptr)
                    ++msg->m_refCount;
                ...
                ++g_NumBdMessages;
                bdReference<bdBitBuffer> buffer = msg->getPayload();
                unsigned char vehicleId = v6->mId;
                if (buffer.m_ptr != nullptr)
                    ++buffer.m_ptr->m_refCount;
                MPUtility::WriteVehicleId(buffer, vehicleId);
                if (buffer.m_ptr != nullptr)
                    ++buffer.m_ptr->m_refCount;
                MPUtility::WriteSeatIndex(buffer, seatIdx);
                if (buffer.m_ptr != nullptr)
                    ++buffer.m_ptr->m_refCount;
                MPUtility::WriteEntryPoint(buffer, entryIdx);
                unsigned char playerId = mpPlayer->mId;
                if (buffer.m_ptr != nullptr)
                    ++buffer.m_ptr->m_refCount;
                MPUtility::WritePlayerId(buffer, playerId);
                p_mPlayerManager->SendAll(message, true, true);
...
// ea: 0x0075B0C0
void MPPeer::SpreadFire(Entity* player, float gunPitch, float gunYaw,
                        ...
                        float spread, float coneAngleTangent, int seed)
{
    ...
    if (p_mSession->getStatus() != bdSession::BD_SESSION_NOT_CONNECTED)
    {
        ...
        if (v10 != nullptr)
        {
            bdMessage* msg = new bdMessage(0x67u, false);
            bdReference<bdMessage> message;
            message.m_ptr = msg;
            if (msg != nullptr)
                ++msg->m_refCount;
            ...
            if (buffer.m_ptr != nullptr)
                ++buffer.m_ptr->m_refCount;
            MPUtility::WritePosition(buffer, weaponPosition);
            if (buffer.m_ptr != nullptr)
                ++buffer.m_ptr->m_refCount;
            MPUtility::WriteAngle(buffer, gunPitch);
            if (buffer.m_ptr != nullptr)
                ++buffer.m_ptr->m_refCount;
            MPUtility::WriteAngle(buffer, gunYaw);
            buffer.m_ptr->writeDataType(
                bdBitBuffer::BD_BB_SIGNED_CHAR8_TYPE);
            unsigned char w = (unsigned char)weapon;
            buffer.m_ptr->writeBits(&w, 8u);
            buffer.m_ptr->writeDataType(bdBitBuffer::BD_BB_FLOAT32_TYPE);
            buffer.m_ptr->writeBits(&spread, 0x20u);
            buffer.m_ptr->writeDataType(bdBitBuffer::BD_BB_FLOAT32_TYPE);
            buffer.m_ptr->writeBits(&coneAngleTangent, 0x20u);
            buffer.m_ptr->writeDataType(
                bdBitBuffer::BD_BB_SIGNED_INTEGER32_TYPE);
            buffer.m_ptr->writeBits(&seed, 0x20u);
            p_mPlayerManager->SendOthers(message, v10, false);
            if (buffer.m_ptr != nullptr
                && buffer.m_ptr->m_refCount-- == 1)
                delete buffer.m_ptr;
            if (message.m_ptr != nullptr
        ...
        else
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
            AeAssert::gCurrentLine = 604;
            AeAssert::gCurrentExpr = "attackingPlayer";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert("old cod assert"))
                __debugbreak();
        }
...
// ea: 0x007447C0
void MPPeer::GetOutOfVehicle(Entity* vehicle, int vehSeatIdx)
{
    bdSession* p_mSession = (bdSession*)((char*)this + 0x7448);
    if (p_mSession->getStatus() != bdSession::BD_SESSION_NOT_CONNECTED)
    {
        MPPlayerManager* p_mPlayerManager =
            (MPPlayerManager*)((char*)this + 0x74E0);
        MPVehicle* v4 = p_mPlayerManager->GetVehicle(vehicle);
        if (v4 == nullptr)
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
            AeAssert::gCurrentLine = 2107;
            AeAssert::gCurrentExpr = "mpVehicle";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert("Could not find vehicle"))
                __debugbreak();
        }
        if (v4->mNumOccupants == 0)
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
            AeAssert::gCurrentLine = 2108;
            AeAssert::gCurrentExpr = "mpVehicle->IsOccupied()";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert("Vehicle is not occupied"))
                __debugbreak();
        }
        bdMessage* msg = new bdMessage(0x48u, false);
        bdReference<bdMessage> message;
        message.m_ptr = msg;
        if (msg != nullptr)
            ++msg->m_refCount;
        ...
        ++g_NumBdMessages;
        bdReference<bdBitBuffer> buffer = msg->getPayload();
        unsigned char vehicleId = v4->mId;
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WriteVehicleId(buffer, vehicleId);
        unsigned char occupantId =
            *(unsigned char*)((char*)v4 + 0x08 + vehSeatIdx);
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WritePlayerId(buffer, occupantId);
        short health = (short)((Entity*)v4->mEntity)->health;
        buffer.m_ptr->writeDataType(bdBitBuffer::BD_BB_SIGNED_INTEGER16_TYPE);
        buffer.m_ptr->writeBits(&health, 0x10u);
        MPPlayer* Player = p_mPlayerManager->GetPlayer(occupantId);
        if (Player == nullptr)
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
            ...
            AeAssert::gCurrentExpr = "player";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert("Invalid player in vehicle"))
                __debugbreak();
        }
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_UNSIGNED_INTEGER32_TYPE);
        buffer.m_ptr->writeBits(&Player->mVehicleEventSequence, 0x20u);
        p_mPlayerManager->SendAll(message, true, false);
        if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
            delete buffer.m_ptr;
        if (message.m_ptr != nullptr
...
// ea: 0x00743640
void MPPeer::SendGameStateCTF(Entity* player,
                              ...
                              Entity* axis_flag_holder)
{
               ...
               == bdSession::BD_SESSION_HOST)
    {
        MPPlayerManager* p_mPlayerManager =
            (MPPlayerManager*)((char*)this + 0x74E0);
        MPPlayer* v9 = p_mPlayerManager->GetPlayer(allied_flag_holder);
        MPPlayer* AxisFlag = p_mPlayerManager->GetPlayer(axis_flag_holder);
        bdMessage* msg = new bdMessage(0x3Bu, false);
        bdReference<bdMessage> message;
        message.m_ptr = msg;
        if (msg != nullptr)
            ++msg->m_refCount;
        ...
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WritePosition(buffer, allied_flag);
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WriteAngles(buffer, alliedAngles);
        unsigned char alliedId = v9 != nullptr ? v9->mId : 16;
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WritePlayerId(buffer, alliedId);
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WritePosition(buffer, axis_flag);
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WriteAngles(buffer, axisAngles);
        unsigned char axisId = AxisFlag != nullptr ? AxisFlag->mId : 16;
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WritePlayerId(buffer, axisId);
        const MPPlayer* v13 = p_mPlayerManager->GetPlayer(player);
        if (v13 != nullptr)
        {
            if (msg != nullptr)
...
// ea: 0x00742910
void MPPeer::NextRound(bool allowChange)
{
    bdSession* p_mSession = (bdSession*)((char*)this + 0x7448);
    if (p_mSession->getStatus() != bdSession::BD_SESSION_NOT_CONNECTED
        && p_mSession->getRole() == bdSession::BD_SESSION_HOST)
    {
        bdMessage* msg = new bdMessage(0x35u, false);
        bdReference<bdMessage> message;
        message.m_ptr = msg;
        if (msg != nullptr)
            ++msg->m_refCount;
        ...
        bdReference<bdBitBuffer> buffer = msg->getPayload();
        buffer.m_ptr->writeDataType(bdBitBuffer::BD_BB_BOOL_TYPE);
        unsigned char b = allowChange ? 0xFF : 0;
        buffer.m_ptr->writeBits(&b, 1u);
        if (allowChange)
        {
            if (buffer.m_ptr != nullptr)
        ...
        if (allowChange)
        {
            CSession* p_liveSession = MPLiveEngine::GetHandle()->liveSession;
            p_liveSession->Setgame_map(
                MPUIInterface::mNextServerParams.mMapID);
            p_liveSession->Setgame_type(
                MPUIInterface::mNextServerParams.mGameType);
            p_liveSession->Setsub_type(
                MPUIInterface::mNextServerParams.mGameSubType);
            p_liveSession->Setteam_balancing(
                MPUIInterface::mNextServerParams.mTeamBalancing);
            p_liveSession->Setfriendly_fire(
                MPUIInterface::mNextServerParams.mFriendlyFire);
            p_liveSession->Update();
            bdDiscoveryServer* ds =
                (bdDiscoveryServer*)((char*)this + 0x73B4);
            bdReference<bdGameInfo> info = ds->getGameInfo();
            MPGameInfo* v8 = (MPGameInfo*)info.m_ptr;
            if (info.m_ptr != nullptr)
            {
                if (--info.m_ptr->m_refCount == 0)
                    delete info.m_ptr;
            ...
            if (v8 != nullptr)
            {
                v8->mMapID = MPUIInterface::mNextServerParams.mMapID;
                v8->mGameType = MPUIInterface::mNextServerParams.mGameType;
                v8->mGameSubType =
                    MPUIInterface::mNextServerParams.mGameSubType;
                v8->mFriendlyFire =
                    MPUIInterface::mNextServerParams.mFriendlyFire != 0;
                v8->mTeamBalancing =
                    MPUIInterface::mNextServerParams.mTeamBalancing != 0;
                v8->mEnableAARVote =
                    MPUIInterface::mNextServerParams.mEnableAARVote != 0;
                v8->mEnablePenaltyVote =
                    MPUIInterface::mNextServerParams.mEnablePenaltyVote != 0;
            }
...
// ea: 0x00743040
void MPPeer::SendGameState(Entity* player, int currentTime, int timeLimit,
                           int scoreLimit, int roundLimit,
                           bool friendlyFire, bool lastManStanding,
                           bool teamBalance, int respawnTime,
                           int alliesScore, int axisScore,
                           ...
                           int roundCount)
{
    if (!player->IsLocalPlayer()
        ...
        && ((bdSession*)((char*)this + 0x7448))->getRole()
               == bdSession::BD_SESSION_HOST)
    {
        bdMessage* msg = new bdMessage(0x3Au, false);
        bdReference<bdMessage> message;
        message.m_ptr = msg;
        if (msg != nullptr)
            ++msg->m_refCount;
        ...
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER32_TYPE);
        buffer.m_ptr->writeBits(&currentTime, 0x20u);
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER32_TYPE);
        buffer.m_ptr->writeBits(&timeLimit, 0x20u);
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER32_TYPE);
        buffer.m_ptr->writeBits(&scoreLimit, 0x20u);
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER32_TYPE);
        buffer.m_ptr->writeBits(&roundLimit, 0x20u);
        buffer.m_ptr->writeBool(friendlyFire);
        buffer.m_ptr->writeBool(lastManStanding);
        buffer.m_ptr->writeBool(teamBalance);
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER32_TYPE);
        buffer.m_ptr->writeBits(&respawnTime, 0x20u);
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER32_TYPE);
        buffer.m_ptr->writeBits(&alliesScore, 0x20u);
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER32_TYPE);
        buffer.m_ptr->writeBits(&axisScore, 0x20u);
        buffer.m_ptr->writeBool(roundStarted);
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER32_TYPE);
        buffer.m_ptr->writeBits(&roundOver, 0x20u);
        buffer.m_ptr->writeDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER32_TYPE);
        buffer.m_ptr->writeBits(&roundCount, 0x20u);
        MPPlayerManager* p_mPlayerManager =
            (MPPlayerManager*)((char*)this + 0x74E0);
        const MPPlayer* v27 = p_mPlayerManager->GetPlayer(player);
        if (v27 != nullptr)
        {
            if (msg != nullptr)
                ++msg->m_refCount;
            p_mPlayerManager->SendPlayer(v27, message, true);
        }
        else
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
...
// ea: 0x00741580
void MPPeer::VehicleMantled(Entity* vehicle, Entity* killer)
{
    const Entity* v3 = vehicle;
    if (vehicle->scr_vehicle == nullptr)
    {
        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
        ...
        != bdSession::BD_SESSION_NOT_CONNECTED)
    {
        MPPlayerManager* p_mPlayerManager =
            (MPPlayerManager*)((char*)this + 0x74E0);
        MPVehicle* v6 = p_mPlayerManager->GetVehicle(v3);
        if (v6 == nullptr)
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
            AeAssert::gCurrentLine = 1102;
            AeAssert::gCurrentExpr = "mpVehicle";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert("Could not find vehicle"))
                __debugbreak();
        ...
        bdMessage* msg = new bdMessage(0x5Fu, false);
        bdReference<bdMessage> message;
        message.m_ptr = msg;
        if (msg != nullptr)
            ++msg->m_refCount;
        ...
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        MPUtility::WriteVehicleId(buffer, vehicleId);
        const Entity* v9 = killer;
        if (killer != nullptr)
        {
            if (killer->client == nullptr)
            {
                AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
                AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
                AeAssert::gCurrentLine = 1115;
                AeAssert::gCurrentExpr = "killer->client";
                if (!AeAssert::IsIgnored()
                    && AeAssert::Assert("old cod assert"))
                    __debugbreak();
            }
            MPPlayer* Player = p_mPlayerManager->GetPlayer(v9);
            if (Player != nullptr)
            {
                buffer.m_ptr->writeDataType(
                    bdBitBuffer::BD_BB_BOOL_TYPE);
...
// ea: 0x00744A50
void MPPeer::AttemptVehicleSeatChange(Entity* vehicle, Entity* player,
                                      int newSeatIdx)
{
    if (((bdSession*)((char*)this + 0x7448))->getStatus()
        != bdSession::BD_SESSION_NOT_CONNECTED)
    {
        ...
        {
            if (G_GetVehicleInfo(vehicle) == nullptr)
            {
                AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
                AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
                AeAssert::gCurrentLine = 2147;
                AeAssert::gCurrentExpr = "info";
                if (!AeAssert::IsIgnored()
                    && AeAssert::Assert("Unable to get vehicle info."))
                    __debugbreak();
            }
            unsigned char newSeat =
                *(unsigned char*)((char*)v6 + 0x08 + newSeatIdx);
            if (newSeat == 16)
            {
                if (newSeatIdx != 1
                    || *(unsigned char*)((char*)v6 + 0x0E) == 16)
                    goto send;
            }
            ...
            if (*(unsigned char*)((char*)v6 + 0x0E)
                != p_mPlayerManager->GetLocalPlayer(currCl)->mId)
                return;
        send:
            int* timer = (int*)((char*)this + 0xD2A4);
            if (*timer <= level.time || *timer > level.time + 1000)
            {
                *timer = level.time + 1000;
                bdMessage* msg = new bdMessage(0x4Au, false);
                bdReference<bdMessage> message;
                message.m_ptr = msg;
                if (msg != nullptr)
                    ++msg->m_refCount;
                ...
                if (buffer.m_ptr != nullptr)
                    ++buffer.m_ptr->m_refCount;
                MPUtility::WriteVehicleId(buffer, vehicleId);
                buffer.m_ptr->writeInt16(
                    (short)((Entity*)v6->mEntity)->health);
                if (buffer.m_ptr != nullptr)
                    ++buffer.m_ptr->m_refCount;
                MPUtility::WriteSeatIndex(buffer, newSeatIdx);
                unsigned char playerId = v7->mId;
                if (buffer.m_ptr != nullptr)
                    ++buffer.m_ptr->m_refCount;
                MPUtility::WritePlayerId(buffer, playerId);
...
// ea: 0x00751200
void MPPeer::VehicleDamage(Entity* hitEntity, Entity* attacker,
                           const math::Position3& position,
                           const math::Dir3& normal, short damage,
                           int weapon, unsigned char mod, short dflags)
{
    if (hitEntity->scr_vehicle == nullptr)
    {
        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
        AeAssert::gCurrentLine = 959;
        AeAssert::gCurrentExpr = "hitEntity->scr_vehicle";
        if (!AeAssert::IsIgnored()
            && AeAssert::Assert("old cod assert"))
            __debugbreak();
    }
        ...
        != bdSession::BD_SESSION_NOT_CONNECTED)
    {
        MPPlayerManager* p_mPlayerManager =
            (MPPlayerManager*)((char*)this + 0x74E0);
        MPVehicle* Vehicle = p_mPlayerManager->GetVehicle(hitEntity);
        if (Vehicle != nullptr)
        {
            bdMessage* msg = new bdMessage(0x4Eu, false);
            bdReference<bdMessage> message;
            message.m_ptr = msg;
            if (msg != nullptr)
                ++msg->m_refCount;
            ...
            ++g_NumBdMessages;
            bdReference<bdBitBuffer> buffer = msg->getPayload();
            unsigned char vehicleId = Vehicle->mId;
            if (buffer.m_ptr != nullptr)
                ++buffer.m_ptr->m_refCount;
            MPUtility::WriteVehicleId(buffer, vehicleId);
            if (buffer.m_ptr != nullptr)
            ...
            buffer.m_ptr->writeDataType(
                bdBitBuffer::BD_BB_SIGNED_INTEGER16_TYPE);
            buffer.m_ptr->writeBits(&damage, 0x10u);
            buffer.m_ptr->writeDataType(
                bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE);
            buffer.m_ptr->writeBits(&mod, 8u);
            buffer.m_ptr->writeDataType(
                bdBitBuffer::BD_BB_SIGNED_INTEGER16_TYPE);
            buffer.m_ptr->writeBits(&dflags, 0x10u);
            if (attacker != nullptr && attacker->client != nullptr)
            {
                MPPlayer* Player = p_mPlayerManager->GetPlayer(attacker);
                if (Player != nullptr)
                {
                    buffer.m_ptr->writeDataType(
                        bdBitBuffer::BD_BB_BOOL_TYPE);
            ...
            if (hitEntity->r.mOwner.mHandle.mVal != 0)
            {
                const Entity* v20 = *hitEntity->r.mOwner;
                const MPPlayer* v21 = p_mPlayerManager->GetPlayer(v20);
                if (msg != nullptr)
                    ++msg->m_refCount;
                p_mPlayerManager->SendPlayer(v21, message, true);
            }
            else
            {
                if (msg != nullptr)
...
// ea: 0x00741250
void MPPeer::VehicleDeath(Entity* hitEntity, Entity* killer, int weapon,
                          int mod)
{
    const Entity* v5 = hitEntity;
    if (hitEntity->scr_vehicle == nullptr)
    {
        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
        AeAssert::gCurrentLine = 1032;
        AeAssert::gCurrentExpr = "hitEntity->scr_vehicle";
        if (!AeAssert::IsIgnored()
            && AeAssert::Assert("old cod assert"))
            __debugbreak();
        ...
        != bdSession::BD_SESSION_NOT_CONNECTED)
    {
        MPPlayerManager* p_mPlayerManager =
            (MPPlayerManager*)((char*)this + 0x74E0);
        MPVehicle* Vehicle = p_mPlayerManager->GetVehicle(v5);
        if (Vehicle == nullptr)
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
            AeAssert::gCurrentLine = 1038;
            AeAssert::gCurrentExpr = "mpVehicle";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert("Could not find vehicle"))
                __debugbreak();
        ...
        bdMessage* msg = new bdMessage(0x4Fu, false);
        bdReference<bdMessage> message;
        message.m_ptr = msg;
        if (msg != nullptr)
            ++msg->m_refCount;
        ...
        if (killer != nullptr)
        {
            if (killer->client == nullptr)
            {
                AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
                AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
                AeAssert::gCurrentLine = 1051;
                AeAssert::gCurrentExpr = "killer->client";
                if (!AeAssert::IsIgnored()
                    && AeAssert::Assert("old cod assert"))
                    __debugbreak();
            }
            MPPlayer* Player = p_mPlayerManager->GetPlayer(v10);
            if (Player != nullptr)
            {
                buffer.m_ptr->writeDataType(
                    bdBitBuffer::BD_BB_BOOL_TYPE);
        ...
        unsigned char v18;
        const MPPlayer* v15 = p_mPlayerManager->GetPlayer(v10);
        if (v15 == nullptr || v10->client->ps.vehPos != 0
            || p_mPlayerManager->GetVehicleFromOccupant(v15) == nullptr
            || p_mPlayerManager->GetVehicleFromOccupant(v15)->mEntity
                   == nullptr
            || ((Entity*)p_mPlayerManager->GetVehicleFromOccupant(v15)
                    ->mEntity)
                       ->s.weapon == 0)
        {
            v18 = (unsigned char)weapon;
        }
        else
        {
            v18 = ((Entity*)p_mPlayerManager->GetVehicleFromOccupant(v15)
                       ->mEntity)
                      ->s.weapon;
        }
        buffer.m_ptr->writeDataType(bdBitBuffer::BD_BB_SIGNED_CHAR8_TYPE);
        buffer.m_ptr->writeBits(&v18, 8u);
        unsigned char m = (unsigned char)mod;
        buffer.m_ptr->writeDataType(bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE);
        buffer.m_ptr->writeBits(&m, 8u);
        p_mPlayerManager->SendAll(message, true, false);
        if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
            delete buffer.m_ptr;
        if (message.m_ptr != nullptr
...
// ea: 0x00741840
void MPPeer::PlayerDamage(Entity* hitEntity, Entity* attacker,
                          const math::Position3& position,
                          const math::Dir3& normal, int weapon,
                          short damage, unsigned char mod, short dflags,
                          int hitLocation)
{
    const Entity* v10 = hitEntity;
    if (hitEntity->client == nullptr)
    {
        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
        AeAssert::gCurrentLine = 1146;
        ...
        != bdSession::BD_SESSION_NOT_CONNECTED)
    {
        MPPlayerManager* p_mPlayerManager =
            (MPPlayerManager*)((char*)this + 0x74E0);
        const MPPlayer* Player = p_mPlayerManager->GetPlayer(v10);
        if (Player != nullptr)
        {
            const MPPlayer* attackerPlayer =
                p_mPlayerManager->GetPlayer(attacker);
            bdMessage* msg = new bdMessage(0x2Du, false);
            bdReference<bdMessage> message;
            message.m_ptr = msg;
            if (msg != nullptr)
                ++msg->m_refCount;
            ...
            else
            {
                ...
                unsigned char b = 0xFF;
                buffer.m_ptr->writeBits(&b, 1u);
                unsigned char attackerId = attackerPlayer->mId;
                if (buffer.m_ptr != nullptr)
                    ++buffer.m_ptr->m_refCount;
            ...
            buffer.m_ptr->writeDataType(
                bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE);
            buffer.m_ptr->writeBits(&mod, 8u);
            short d = (short)(dflags | 0x80);
            buffer.m_ptr->writeDataType(
                bdBitBuffer::BD_BB_SIGNED_INTEGER16_TYPE);
            buffer.m_ptr->writeBits(&d, 0x10u);
            unsigned char hitLoc = (unsigned char)hitLocation;
            buffer.m_ptr->writeDataType(
                bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE);
            buffer.m_ptr->writeBits(&hitLoc, 8u);
            unsigned char w = (unsigned char)weapon;
            buffer.m_ptr->writeDataType(
                bdBitBuffer::BD_BB_SIGNED_CHAR8_TYPE);
            buffer.m_ptr->writeBits(&w, 8u);
...
// ea: 0x00740CF0
void MPPeer::SpotEntity(Entity* ent)
{
    Entity* v2 = ent;
    if (ent == nullptr)
    {
        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
        AeAssert::gCurrentLine = 836;
        AeAssert::gCurrentExpr = "ent";
        if (!AeAssert::IsIgnored()
            && AeAssert::Assert("invalid entity"))
            __debugbreak();
    }
    if (v2->scr_vehicle == nullptr && v2->client == nullptr)
    {
        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPeer.cpp";
        AeAssert::gCurrentLine = 837;
        AeAssert::gCurrentExpr = "ent->scr_vehicle || ent->client";
        if (!AeAssert::IsIgnored()
            && AeAssert::Assert("invalid entity"))
            __debugbreak();
    }
    if (((bdSession*)((char*)this + 0x7448))->getStatus()
        != bdSession::BD_SESSION_NOT_CONNECTED)
    {
        bdMessage* msg = new bdMessage(0x5Eu, false);
        bdReference<bdMessage> message;
        message.m_ptr = msg;
        if (msg != nullptr)
            ++msg->m_refCount;
        ...
        {
            MPPlayer* Player = p_mPlayerManager->GetPlayer(v2);
            if (Player == nullptr)
            {
                AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
                AeAssert::gCurrentFile =
                ...
                if (!AeAssert::IsIgnored()
                    && AeAssert::Assert(
                        "No valid MPPlayer entity for the entity."))
                    __debugbreak();
                if (buffer.m_ptr != nullptr
                    && buffer.m_ptr->m_refCount-- == 1)
                    delete buffer.m_ptr;
        ...
        if (v2->scr_vehicle != nullptr)
        {
            MPVehicle* Vehicle = p_mPlayerManager->GetVehicle(v2);
            if (Vehicle == nullptr)
            {
                AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
                AeAssert::gCurrentFile =
                ...
                if (!AeAssert::IsIgnored()
                    && AeAssert::Assert(
                        "No valid MPVehicle entity for the entity."))
                    __debugbreak();
                if (buffer.m_ptr != nullptr
                    && buffer.m_ptr->m_refCount-- == 1)
                    delete buffer.m_ptr;
...
// ea: 0x007619D0
bool MPPeer::CreateLocalGameInfo(bdReference<MPGameInfo>& gameInfo)
{
    bdNetImpl* Instance = bdSingleton<bdNetImpl>::getInstance();
    bdNetStatus Status = Instance->getStatus();
    bool ok = Status == BD_NET_DONE;
    if (Status != BD_NET_DONE)
        return ok;
    bdSecurityKeyMap* keyMap = Instance->getKeyMap();
    XNKID secID;
    XNKEY secKey;
    memset(&secID, 0, sizeof(secID));
    memset(&secKey, 0, sizeof(secKey));
    if (memcmp(gameInfo.m_ptr->getSecurityID().ab, secID.ab, 8u) == 0
        || memcmp(gameInfo.m_ptr->getSecurityKey().ab, secKey.ab, 0x10u)
               == 0)
    {
        ok = keyMap->create(secID, secKey);
    }
    else
    {
        secID = gameInfo.m_ptr->getSecurityID();
        secKey = gameInfo.m_ptr->getSecurityKey();
    }
    if (!ok)
    {
        bdMessageProxy proxy(
            "c:\\cod\\code\\game\\mp/MPPeer.cpp",
            "bool __thiscall MPPeer::CreateLocalGameInfo(class bdReference<class MPGameInfo> &)",
            0xBDBu, "dw/warn/");
        proxy.log("AJP - MPPeer::CreateLocalGameInfo",
                  "Failed to create local MPGameInfo.");
        return ok;
    }
    bdReference<bdCommonAddr> hostAddr = Instance->getLocalCommonAddr();
    MPGameInfo* v10 = new MPGameInfo(
        0x2C0DC0Du, hostAddr, secID, secKey,
        (unsigned char)(MPUIInterface::mServerParams.mMaxPlayers
                        - MPUIInterface::mServerParams.mPrivateSlots - 1),
        MPUIInterface::mServerParams.mPrivateSlots, 1, 0);
    bdReference<MPGameInfo> info;
    info.m_ptr = v10;
    if (v10 != nullptr)
        ++v10->m_refCount;
    if (&info != &gameInfo)
    {
        ...
        if (gameInfo.m_ptr != nullptr)
        {
            if (m_ptr->m_refCount-- == 1 && gameInfo.m_ptr != nullptr)
                delete gameInfo.m_ptr;
        }
        gameInfo.m_ptr = v10;
        if (v10 != nullptr)
    ...
    strcpy(gameInfo.m_ptr->mName, MPUIInterface::mServerParams.mName);
    gameInfo.m_ptr->mMapID = MPUIInterface::mServerParams.mMapID;
    gameInfo.m_ptr->mGameType = MPUIInterface::mServerParams.mGameType;
    gameInfo.m_ptr->mGameSubType =
        MPUIInterface::mServerParams.mGameSubType;
    gameInfo.m_ptr->mFriendlyFire =
        MPUIInterface::mServerParams.mFriendlyFire != 0;
    gameInfo.m_ptr->mTeamBalancing =
        MPUIInterface::mServerParams.mTeamBalancing != 0;
    gameInfo.m_ptr->mEnableAARVote =
        MPUIInterface::mServerParams.mEnableAARVote != 0;
    gameInfo.m_ptr->mEnablePenaltyVote =
    ...
    {
        MPGameInfo* m_ptr = p_mCurrentGameInfo->m_ptr;
        if (p_mCurrentGameInfo->m_ptr != nullptr)
        {
    ...
    if (v10 != nullptr && v10->m_refCount-- == 1)
        delete v10;
    return ok;
}

// ============================================================================
// mp.o batch 19 - MPPlayerManager handlers (MPPlayerMgr.cpp)
// ============================================================================
// ea: 0x007499F0
                                      ...
                                      MPPlayer* player)
{
    ...
    data.id = 16;
    LivePlayer* livePlayer = (LivePlayer*)&data.livePlayer;
    livePlayer->Reset();
    int mClientIndex = player->mClientIndex;
    data.playing = false;
    data.vehicleId = 10;
    ...
    data.teamAxis = false;
    data.name[0] = 0;
    memset(data.origin, 0, 24);
    data.wasInvited = false;
    Entity* v4 = mClientIndex >= 0
                     ? EntityManager::sInst->GetPlayer(mClientIndex)
                     : nullptr;
    data.origin[0] = player->mInterpolatedPosition.v.m128_f32[0];
    data.origin[1] = player->mInterpolatedPosition.v.m128_f32[1];
    data.angles[2] = 0.0f;
    data.angles[0] = 0.0f;
    data.angles[1] = *(float*)((char*)player + 0x218);  // mInterpolatedHeading
    data.id = player->mId;
    data.origin[2] = player->mInterpolatedPosition.v.m128_f32[2];
    if (LiveWrapper::theWrapper->sessionState == kInSession)
    {
        MPLiveEngine* Handle = MPLiveEngine::GetHandle();
    ...
    if (v4 == nullptr || v4->sentient == nullptr
        || v4->client->pers.playerState != 3)
        data.playing = false;
    else
        data.playing = true;
    data.vehicleId = 10;
    if (v4 != nullptr && v4->client != nullptr)
        data.playerClass = v4->client->pers.playerClass;
    else
        data.playerClass = -1;
    unsigned char v12 = player->mId;
    data.vehicleHealth = 0;
    int v13 = 0;
    while (*(unsigned char*)((char*)this + 0x4111 + v13) != v12)
    {
        if (++v13 >= 1)
        {
            v13 = -1;
    ...
    data.localIdx = (unsigned char)v13;
    if (player->mInVehicle)
    {
        MPVehicle* VehicleFromOccupant = GetVehicleFromOccupant(player);
        MPVehicle* v15 = VehicleFromOccupant;
        if (VehicleFromOccupant == nullptr
            || VehicleFromOccupant->mNumOccupants == 0)
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile =
                "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
            AeAssert::gCurrentLine = 2624;
            AeAssert::gCurrentExpr = "vehicle && vehicle->IsOccupied()";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert("OY! Vehicle is fucked"))
                __debugbreak();
        }
        data.vehicleId = v15->mId;
        data.vehicleHealth = ((Entity*)v15->mEntity)->health;
        data.vehicleEventSequence = player->mVehicleEventSequence;
    }
    if (buffer.m_ptr != nullptr)
        ++buffer.m_ptr->m_refCount;
    MPUtility::WritePlayerData(buffer, data);
    if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
        delete buffer.m_ptr;
}

// ea: 0x00748B40
void MPPlayerManager::SendOtherEnemiesInRange(
    const MPPlayer* localPlayer, bdReference<bdMessage> message,
    int maxRange, bool inRange)
{
    if (localPlayer == nullptr)
    {
        if (message.m_ptr != nullptr && message.m_ptr->m_refCount-- == 1)
    ...
    if (mClientIndex < 0
        || (Player = EntityManager::sInst->GetPlayer(mClientIndex))
               == nullptr)
    {
        if (message.m_ptr != nullptr && message.m_ptr->m_refCount-- == 1)
            delete message.m_ptr;
    ...
    {
        MPPlayer* mPlayers = (MPPlayer*)((char*)this + 0x1010 + 0x310 * i);
        if (mPlayers != nullptr && mPlayers->mId < 0x10u
            && mPlayers->mConnection.m_ptr != nullptr)
        {
            bdReference<bdConnection> v32 = mPlayers->GetConnection();
            bdConnection* m_ptr = v32.m_ptr;
            if (v32.m_ptr != nullptr && !IsLocalId(mPlayers->mId)
                && mPlayers->mClientIndex >= 0)
            {
                Entity* v11 = EntityManager::sInst->GetPlayer(
                    mPlayers->mClientIndex);
                if (v11 != nullptr)
                {
                    bool v14 = true;
                    if (cgGlobal.teamGame)
                    {
                        sentient_s* sentient = Player->sentient;
                        if (sentient == nullptr || v11->sentient == nullptr
                            || v11->sentient->eTeam == sentient->eTeam)
                            v14 = false;
                    }
                    float dx = Player->r.currentOrigin.v.m128_f32[0]
                               - v11->r.currentOrigin.v.m128_f32[0];
                    float dy = Player->r.currentOrigin.v.m128_f32[1]
                               - v11->r.currentOrigin.v.m128_f32[1];
                    float dz = Player->r.currentOrigin.v.m128_f32[2]
                               - v11->r.currentOrigin.v.m128_f32[2];
                    float distSq = dx * dx + dy * dy + dz * dz;
                    if ((maxRange > sqrt(distSq) && v14) == inRange)
                    {
                        bool skipSend = false;
                        if (!mPlayers->mMasterClient)
                        {
                            MPPlayer* master = nullptr;
                            for (unsigned int j = 0; j < 16; ++j)
                            {
                                MPPlayer* v17 = GetPlayer((int)j);
                                if (v17 != nullptr && v17->IsValid()
                                    && v17->mMasterClient)
                                {
                                    if (v17->GetConnection().m_ptr
                                        == mPlayers->GetConnection().m_ptr)
                                    {
                                        master = v17;
                                        break;
                                    }
                            ...
                            if (master != nullptr)
                            {
                                if (sent_to.containsPlayer(
                                        (unsigned long)master->mId)
                                    == 0)
                                {
                                    sent_to.addPlayer(master->mId);
                                }
                                else
                                {
                                    sent_to.addPlayer(mPlayers->mId);
                                    skipSend = true;
                                }
                            }
                        }
                        ...
                        {
                            sent_to.addPlayer(mPlayers->mId);
                            SendPlayer(mPlayers, message, false);
                        }
                    }
                }
            }
...
// ea: 0x00759A20
void MPPlayerManager::SendLocalPlayerTeam(const MPPlayer* localPlayer,
                                          const MPPlayer* player)
{
    bdMessage* msg = new bdMessage(0x60u, false);
    bdReference<bdMessage> message;
    message.m_ptr = msg;
    ...
    if (dword_F6A290[0] == 2)
    {
        unsigned char v7 = *(unsigned char*)((char*)this + 0x4111);
        MPPlayer* v5 = nullptr;
        if (v7 < 0x10u)
            v5 = (MPPlayer*)((char*)this + 0x1010 + 0x310 * v7);
        if (v5->mClientIndex >= 0)
        {
            Entity* v8 = EntityManager::sInst->GetPlayer(v5->mClientIndex);
            if (v8 != nullptr)
            {
                unsigned char v9 = *(unsigned char*)((char*)this + 0x4111);
                MPPlayer* v10 = nullptr;
                if (v9 < 0x10u)
                    v10 = (MPPlayer*)((char*)this + 0x1010 + 0x310 * v9);
                unsigned char id = v10->mId;
                if (buffer.m_ptr != nullptr)
                    ++buffer.m_ptr->m_refCount;
                MPUtility::WritePlayerId(buffer, id);
                Client* client = v8->client;
                short eTeam = v8->sentient->eTeam;
                v5->mPlayerInfoSet = true;
                v5->mTeam = eTeam;
                v5->mRank = client->pers.rank;
                buffer.m_ptr->writeDataType(
                    bdBitBuffer::BD_BB_SIGNED_INTEGER16_TYPE);
                buffer.m_ptr->writeBits(&eTeam, 0x10u);
                short rank = v8->client->pers.rank;
                buffer.m_ptr->writeDataType(
                    bdBitBuffer::BD_BB_SIGNED_INTEGER16_TYPE);
                buffer.m_ptr->writeBits(&rank, 0x10u);
                buffer.m_ptr->writeDataType(bdBitBuffer::BD_BB_BOOL_TYPE);
                unsigned char b = 0xFF;
                buffer.m_ptr->writeBits(&b, 1u);
                unsigned char v16 = *(unsigned char*)((char*)this + 0x4111);
                MPPlayer* v17 = nullptr;
                if (v16 < 0x10u)
                    v17 = (MPPlayer*)((char*)this + 0x1010 + 0x310 * v16);
                {
                    bdString str(v17->mName);
                    unsigned int len = 8 * str.getLength() + 8;
                    buffer.m_ptr->writeDataType(
                        (bdBitBuffer::bdBitBufferDataType)0x10u);
                    buffer.m_ptr->writeBits(str.getBuffer(), len);
                }
                buffer.m_ptr->writeDataType(bdBitBuffer::BD_BB_BOOL_TYPE);
                b = 0;
                buffer.m_ptr->writeBits(&b, 1u);
                Entity* v21 = v5->mClientIndex >= 0
                                  ? EntityManager::sInst->GetPlayer(
                                        v5->mClientIndex)
                                  : nullptr;
                if (!IsLocalPlayer(v21))
                {
                    bdReference<bdConnection> conn = v5->GetConnection();
                    bool hasConn = conn.m_ptr != nullptr;
                    if (conn.m_ptr != nullptr
                        && conn.m_ptr->m_refCount-- == 1)
                        delete conn.m_ptr;
                    if (!hasConn)
                    {
                        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
...
// ea: 0x0074E450
void MPPlayerManager::HandleGameStateHQ(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    if (Player != nullptr)
    {
            ...
            && m_ptr->getStatus() == bdConnection::BD_CONNECTED)
        {
            ...
            bool alliesDefending = false;
            bool pointAIsHQ = false;
            bool v6 = false;
            if (buffer.m_ptr->readUInt32(stage))
            {
                if (buffer.m_ptr != nullptr)
                    ++buffer.m_ptr->m_refCount;
                if (MPUtility::ReadPosition(buffer, vA))
                {
                    if (buffer.m_ptr != nullptr)
                        ++buffer.m_ptr->m_refCount;
                    if (MPUtility::ReadPosition(buffer, vB)
                        && buffer.m_ptr->readUInt32(triggerIndex)
                        && buffer.m_ptr->readBool(alliesDefending))
                    {
                        buffer.m_ptr->readBool(pointAIsHQ);
                        v6 = pointAIsHQ;
                    }
                }
            }
            if (*(bool*)((char*)this + 0x4112))  // mLocalPlayerInGame
            {
                if (gpBrocAPI->mBrocExports.mCallbackGameStateHQ != nullptr)
                {
                    gpBrocAPI->mBrocExports.mCallbackGameStateHQ(
                        stage, Broc::vector(vA[0], vA[1], vA[2]),
                        Broc::vector(vB[0], vB[1], vB[2]), triggerIndex,
                        alliesDefending, v6);
                }
            }
            else
            {
                *(unsigned int*)((char*)this + 0x58EC) = stage;  // mHQStage
                float* vA_dst = (float*)((char*)this + 0x58F0);  // mHQvA
                vA_dst[0] = vA[0];
                vA_dst[1] = vA[1];
                vA_dst[2] = vA[2];
                float* vB_dst = (float*)((char*)this + 0x58FC);  // mHQvB
                vB_dst[0] = vB[0];
                vB_dst[1] = vB[1];
                vB_dst[2] = vB[2];
                *(bool*)((char*)this + 0x5868) = true;   // mGameParamsSet
                *(int*)((char*)this + 0x586C) = 3;       // mGameType
                *(unsigned int*)((char*)this + 0x5908) = triggerIndex;
                *(bool*)((char*)this + 0x590C) = alliesDefending;
                *(bool*)((char*)this + 0x5910) = v6;
            }
            if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
                delete buffer.m_ptr;
            if (msg.m_ptr != nullptr && msg.m_ptr->m_refCount-- == 1)
...
// ea: 0x0074E930
void MPPlayerManager::HandleGameStateSD(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    if (Player != nullptr)
    {
            ...
            && m_ptr->getStatus() == bdConnection::BD_CONNECTED)
        {
            bdReference<bdMessage> msg = receivedMsg.getMessage();
            bdReference<bdBitBuffer> buffer = msg.m_ptr->getPayload();
            unsigned char planter = 16;
            unsigned char defuser = 16;
            bool bombPlanted = false;
            float bombPosition[3] = { 0.0f, 0.0f, 0.0f };
            float bombAngles[3] = { 0.0f, 0.0f, 0.0f };
            unsigned short timeLeft = 0;
            if (buffer.m_ptr != nullptr)
                ++buffer.m_ptr->m_refCount;
            if (MPUtility::ReadPlayerId(buffer, planter))
            {
                if (buffer.m_ptr != nullptr)
                    ++buffer.m_ptr->m_refCount;
                if (MPUtility::ReadPlayerId(buffer, defuser)
                    && buffer.m_ptr->readBool(bombPlanted))
                {
                    if (buffer.m_ptr != nullptr)
                        ++buffer.m_ptr->m_refCount;
                    if (MPUtility::ReadPosition(buffer, bombPosition))
                    {
                        if (buffer.m_ptr != nullptr)
                            ++buffer.m_ptr->m_refCount;
                        if (MPUtility::ReadAngles(buffer, bombAngles))
                            buffer.m_ptr->readUInt16(timeLeft);
                    }
                }
            }
            if (*(bool*)((char*)this + 0x4112))
            {
                if (gpBrocAPI->mBrocExports.mCallbackGameStateSD != nullptr)
                {
                    Broc::vector pos(bombPosition[0], bombPosition[1],
                                     bombPosition[2]);
            ...
            {
                *(unsigned char*)((char*)this + 0x58C1) = defuser;
                *(unsigned char*)((char*)this + 0x58C0) = planter;
                *(bool*)((char*)this + 0x58C2) = bombPlanted;
                float* pos_dst = (float*)((char*)this + 0x58C4);
                pos_dst[0] = bombPosition[0];
                pos_dst[1] = bombPosition[1];
                pos_dst[2] = bombPosition[2];
                *(unsigned short*)((char*)this + 0x58DC) = timeLeft;
                *(bool*)((char*)this + 0x5868) = true;
                *(int*)((char*)this + 0x586C) = 8;
                float* ang_dst = (float*)((char*)this + 0x58D0);
                ang_dst[0] = bombAngles[0];
                ang_dst[1] = bombAngles[1];
                ang_dst[2] = bombAngles[2];
            }
            if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
                delete buffer.m_ptr;
            if (msg.m_ptr != nullptr && msg.m_ptr->m_refCount-- == 1)
...
// ea: 0x007397E0
void MPPlayerManager::HandleGameState(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    if (Player != nullptr)
    {
            ...
            && m_ptr->getStatus() == bdConnection::BD_CONNECTED)
        {
            ...
            int roundCount = 0;
            int v6 = 0;
            if (buffer.m_ptr->readInt32(currentTime)
                && buffer.m_ptr->readInt32(timeLimit)
                && buffer.m_ptr->readInt32(scoreLimit)
                && buffer.m_ptr->readInt32(roundLimit)
                && buffer.m_ptr->readBool(friendlyFire)
                && buffer.m_ptr->readBool(lastManStanding)
                && buffer.m_ptr->readBool(teamBalance)
                && buffer.m_ptr->readInt32(respawnTime)
                && buffer.m_ptr->readInt32(alliesScore)
                && buffer.m_ptr->readInt32(axisScore)
                && buffer.m_ptr->readBool(roundStarted)
                && buffer.m_ptr->readInt32(roundOver))
            {
                buffer.m_ptr->readInt32(roundCount);
                v6 = roundCount;
            }
            if (*(bool*)((char*)this + 0x4112))
            {
                if (gpBrocAPI->mBrocExports.mCallbackGameState != nullptr)
                {
                    gpBrocAPI->mBrocExports.mCallbackGameState(
                        currentTime, timeLimit, scoreLimit, roundLimit,
                        friendlyFire, lastManStanding, teamBalance,
            ...
            {
                *(float*)((char*)this + 0x5870) = (float)currentTime;
                *(int*)((char*)this + 0x5874) = timeLimit;
                *(int*)((char*)this + 0x5878) = scoreLimit;
                *(int*)((char*)this + 0x587C) = roundLimit;
                *(int*)((char*)this + 0x5880) = alliesScore;
                *(bool*)((char*)this + 0x5888) = friendlyFire;
                *(bool*)((char*)this + 0x5889) = lastManStanding;
                *(bool*)((char*)this + 0x588A) = teamBalance;
                *(int*)((char*)this + 0x588C) = respawnTime;
                *(bool*)((char*)this + 0x5868) = true;
                *(bool*)((char*)this + 0x58E0) = roundStarted;
                *(bool*)((char*)this + 0x58E1) = roundOver != 0;
                *(int*)((char*)this + 0x58E4) = v6;
            }
            if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
                delete buffer.m_ptr;
            if (msg.m_ptr != nullptr && msg.m_ptr->m_refCount-- == 1)
...
// ea: 0x0074E0E0
void MPPlayerManager::HandleGameStateCTF(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    if (Player != nullptr)
    {
            ...
            && m_ptr->getStatus() == bdConnection::BD_CONNECTED)
        {
            bdReference<bdMessage> msg = receivedMsg.getMessage();
            bdReference<bdBitBuffer> buffer = msg.m_ptr->getPayload();
            float allied_flag_location[3] = { 0.0f, 0.0f, 0.0f };
            float allied_flag_angles[3] = { 0.0f, 0.0f, 0.0f };
            unsigned char allied_flag_status = 0;
            float axis_flag_location[3] = { 0.0f, 0.0f, 0.0f };
            float axis_flag_angles[3] = { 0.0f, 0.0f, 0.0f };
            unsigned char axisPlayer = 0;
            if (buffer.m_ptr != nullptr)
                ++buffer.m_ptr->m_refCount;
            if (MPUtility::ReadPosition(buffer, allied_flag_location))
            {
                if (buffer.m_ptr != nullptr)
                    ++buffer.m_ptr->m_refCount;
                if (MPUtility::ReadAngles(buffer, allied_flag_angles))
                {
                    if (buffer.m_ptr != nullptr)
                        ++buffer.m_ptr->m_refCount;
                                                ...
                                                allied_flag_status))
                    {
                                                    ...
                                                    axis_flag_location))
                        {
                            if (buffer.m_ptr != nullptr)
                                ++buffer.m_ptr->m_refCount;
                            if (MPUtility::ReadAngles(buffer,
                                                      axis_flag_angles))
                            {
                                if (buffer.m_ptr != nullptr)
                                    ++buffer.m_ptr->m_refCount;
                                MPUtility::ReadPlayerId(buffer, axisPlayer);
                            }
                        }
                    }
                }
            ...
            if (*(bool*)((char*)this + 0x4112))
            {
                if (gpBrocAPI->mBrocExports.mCallbackGameStateCTF
                    != nullptr)
                {
                    unsigned int alliedHolder = 0;
                    unsigned int axisHolder = 0;
                    MPPlayer* v7 = GetPlayer(allied_flag_status);
                    MPPlayer* v8 = GetPlayer(axisPlayer);
                    if (v7 != nullptr && v7->mClientIndex >= 0)
                    {
                        Entity* ent =
                            EntityManager::sInst->GetPlayer(
                                v7->mClientIndex);
                        if (ent != nullptr)
                            alliedHolder = ent->mHandle.mHandle.mVal;
                    }
                    if (v8 != nullptr && v8->mClientIndex >= 0)
                    {
                        Entity* ent =
                            EntityManager::sInst->GetPlayer(
                                v8->mClientIndex);
                        if (ent != nullptr)
                            axisHolder = ent->mHandle.mHandle.mVal;
                    }
                    gpBrocAPI->mBrocExports.mCallbackGameStateCTF(
                        Broc::vector(allied_flag_location[0],
                                     allied_flag_location[1],
                                     allied_flag_location[2]),
                        Broc::vector(allied_flag_angles[0],
                                     allied_flag_angles[1],
                                     allied_flag_angles[2]),
                        alliedHolder,
                        Broc::vector(axis_flag_location[0],
                                     axis_flag_location[1],
                                     axis_flag_location[2]),
                        Broc::vector(axis_flag_angles[0],
                                     axis_flag_angles[1],
                                     axis_flag_angles[2]),
                        axisHolder);
                }
            ...
            {
                *(unsigned char*)((char*)this + 0x5890) =
                    allied_flag_status;
                *(unsigned char*)((char*)this + 0x5891) = axisPlayer;
                float* allied_dst = (float*)((char*)this + 0x5894);
                allied_dst[0] = allied_flag_location[0];
                allied_dst[1] = allied_flag_location[1];
                allied_dst[2] = allied_flag_location[2];
                *(bool*)((char*)this + 0x5868) = true;
                *(int*)((char*)this + 0x586C) = 1;
                float* axis_dst = (float*)((char*)this + 0x58A0);
                axis_dst[0] = axis_flag_location[0];
                axis_dst[1] = axis_flag_location[1];
                axis_dst[2] = axis_flag_location[2];
            }
            if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
                delete buffer.m_ptr;
            if (msg.m_ptr != nullptr && msg.m_ptr->m_refCount-- == 1)
...
// ea: 0x00757BD0
void MPPlayerManager::HandlePlayerInfo(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    ...
    short TotalDeaths = 0;
    bdReference<bdBitBuffer> buffer = msg.m_ptr->getPayload();
    unsigned char playerId = 16;
    if (buffer.m_ptr != nullptr)
        ++buffer.m_ptr->m_refCount;
    bool v6 = MPUtility::ReadPlayerId(buffer, playerId)
              && buffer.m_ptr->readInt16(team)
              && buffer.m_ptr->readInt16(rank);
    if (v6 && buffer.m_ptr->readInt16(TotalScore)
        && buffer.m_ptr->readInt16(TotalKills)
        && buffer.m_ptr->readInt16(TotalDeaths))
    {
        MPPlayer* v7 = (MPPlayer*)((char*)this + 0x1010 + 0x310 * playerId);
        if (playerId >= 0x10u)
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile =
                "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
            AeAssert::gCurrentLine = 2732;
            AeAssert::gCurrentExpr = "player";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert(
                    "Getting a player info on a non-valid player."))
                __debugbreak();
            if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
                delete buffer.m_ptr;
            if (msg.m_ptr != nullptr && msg.m_ptr->m_refCount-- == 1)
        ...
        v7->mRank = rank;
        v7->mPlayerInfoSet = true;
        v7->mTotalScore = TotalScore;
        v7->mTotalKills = TotalKills;
        v7->mTotalDeaths = TotalDeaths;
        if (buffer.m_ptr->testBool())
        {
            buffer.m_ptr->readString(name);
            v7->SetName(name.getBuffer());
        }
        bool autoBalance = buffer.m_ptr->testBool();
        v7->UpdateInGamePlayerInfo(autoBalance, true);
    }
    if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
        delete buffer.m_ptr;
    if (msg.m_ptr != nullptr && msg.m_ptr->m_refCount-- == 1)
...
// ea: 0x007584F0
void MPPlayerManager::HandleVehicleDamage(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    if (Player != nullptr)
    {
            ...
            && *(bool*)((char*)this + 0x4112))
        {
            ...
            unsigned char attackerPlayerID = 16;
            if (buffer.m_ptr != nullptr)
                ++buffer.m_ptr->m_refCount;
            if (!MPUtility::ReadVehicleId(buffer, hitVehicleID))
                goto cleanup;
            ...
            if (!MPUtility::ReadNormal(buffer, normal))
                goto cleanup;
            if (!buffer.m_ptr->readInt16(damage))
                goto cleanup;
            if (!buffer.m_ptr->readUChar8(mod))
                goto cleanup;
            if (!buffer.m_ptr->readInt16(dflags))
                goto cleanup;
            if (!buffer.m_ptr->readBool(readPlayer))
                goto cleanup;
            if (readPlayer)
            {
                if (buffer.m_ptr != nullptr)
                    ++buffer.m_ptr->m_refCount;
                if (!MPUtility::ReadPlayerId(buffer, attackerPlayerID))
                    goto cleanup;
            }
            {
                Entity* mEntity =
                    (Entity*)((MPVehicle*)((char*)this + 0x4120
                                           + 0x210 * hitVehicleID))
                        ->mEntity;
                Entity* senderEnt = nullptr;
                if (readPlayer)
                {
                    MPPlayer* v9 = GetPlayer(attackerPlayerID);
                    if (v9 != nullptr && v9->mClientIndex >= 0)
                        senderEnt = EntityManager::sInst->GetPlayer(
                            v9->mClientIndex);
                }
                if (mEntity == nullptr)
                {
                    ...
                    AeAssert::gCurrentFile =
                        "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
                    AeAssert::gCurrentLine = 3561;
                    AeAssert::gCurrentExpr = "vehicleEnt";
                    if (!AeAssert::IsIgnored()
                        && AeAssert::Assert("old cod assert"))
                        __debugbreak();
                }
                if (mEntity->r.mOwner.mHandle.mVal != 0)
                {
                    const Entity* ownerEnt = *mEntity->r.mOwner;
                    MPPlayer* v11 = GetPlayer(ownerEnt);
                    if (v11 != nullptr)
                    {
                        Entity* v12 = v11->mClientIndex >= 0
                                          ? EntityManager::sInst->GetPlayer(
                                                v11->mClientIndex)
                                          : nullptr;
                        if (!IsLocalPlayer(v12))
                            goto cleanup;
                    }
                    else
                    {
                ...
                G_Damage(mEntity, senderEnt, senderEnt, normal, position,
                         damage, dflags | 0x80, mod, HITLOC_NONE, -1);
            }
        cleanup:
            if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
                delete buffer.m_ptr;
...
// ea: 0x0074B0E0
void MPPlayerManager::HandleVehicleFireMissile(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    ...
    unsigned char vehicleId = 0;
    char weapon = 0;
    float joltDir[3] = { 0.0f, 0.0f, 0.0f };
    float position[3] = { 0.0f, 0.0f, 0.0f };
    ...
    if (MPUtility::ReadVehicleId(buffer, vehicleId))
    {
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        if (MPUtility::ReadPosition(buffer, joltDir))
        {
            if (buffer.m_ptr != nullptr)
                ++buffer.m_ptr->m_refCount;
            if (MPUtility::ReadVector(buffer, position)
                && buffer.m_ptr->readChar8(weapon))
            {
                MPVehicle* v6 =
                    (MPVehicle*)((char*)this + 0x4120 + 0x210 * vehicleId);
                weaponFileInfo_t* InfoForWeapon =
                    BG_GetInfoForWeapon(weapon);
                Entity* mEntity = (Entity*)v6->mEntity;
                int v9 = InfoForWeapon->type - 1;
                if (v9 != 0)
                {
                    if (v9 == 1)
                    {
                        fire_rocket(mEntity, joltDir, position, 10.0f)
                            ->s.pos.trType = TR_GRAVITY;
                        CG_FireWeapon(mEntity, &mEntity->s, 186, 0);
                        float neg[3];
                        neg[0] = 0.0f - position[0];
                        neg[1] = 0.0f - position[1];
                        neg[2] = 0.0f - position[2];
                        math::Position3 dirVec = native_to_cdl_pos3(neg);
                        VEH_JoltBody(mEntity, dirVec, 1.0f, 0.0f, 0.0f);
                    }
                    else
                    {
                        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
                        ...
                        AeAssert::gCurrentExpr = nullptr;
                        if (!AeAssert::Error(
                                "Received a fire missile command for an invalid weapon type."))
                            goto cleanup;
                        __debugbreak();
                    }
                }
...
// ea: 0x0075F5B0
void MPPlayerManager::HandleProjectileExplosion(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    if (Player != nullptr)
    {
            ...
            && *(bool*)((char*)this + 0x4112))
        {
            ...
            MPEntityHandle handle;
            handle.mValue = 0;
            if (buffer.m_ptr->readRangedInt32(weapon, 0, 92))
            {
                if (buffer.m_ptr != nullptr)
                    ++buffer.m_ptr->m_refCount;
                if (MPUtility::ReadSnappedPosition(buffer, position))
                {
                    if (buffer.m_ptr != nullptr)
                        ++buffer.m_ptr->m_refCount;
                    if (MPUtility::ReadNormal(buffer, normal)
                        && buffer.m_ptr->readChar8((char&)surfaceType))
                    {
                        if (buffer.m_ptr != nullptr)
                            ++buffer.m_ptr->m_refCount;
                        if (MPUtility::ReadEntityHandle(buffer, handle))
                        {
                            unsigned int handleVal = handle.mValue;
                            weaponFileInfo_t* InfoForWeapon =
                                BG_GetInfoForWeapon(weapon);
                            Entity* v7 = G_TempEntity(normal, 210);
                            if (v7 != nullptr)
                            {
                                v7->s.eventParm = DirToByte(normal);
                                v7->s.weapon = (unsigned char)weapon;
                                float origin[3] = { normal[0], normal[1],
                                                    normal[2] };
                                v7->s.SetLerpOrigin(
                                    *(math::Position3*)origin);
                                v7->s.scale = InfoForWeapon->projExplosion;
                                v7->r.eventType &= 0xFFFFFFFE;
                                v7->think = THINK__G_FreeEntity;
                                v7->nextthink = level.time + 60000;
                                collision_context_t context(32);
                                math::Position3 cdl = native_to_cdl_pos3(
                                    normal);
                                if (SV_PointContents(cdl, context) != 0)
                                    v7->s.surfType = 20;
                                else
                                    v7->s.surfType = surfaceType;
                            }
                            if (handleVal != 0)
                            {
                                unsigned char playerId =
                                    (unsigned char)(((handleVal >> 11)
                                                     & 0x1F)
                                                    - 1);
                                MPPlayer* v12 = GetPlayer(playerId);
                                if (v12 != nullptr)
                                {
                                    v12->RemoveItem(
                                        (EDroppedItemTypes)kItemTypeMines,
                                        handle);
                                }
                            }
                        }
...
// ea: 0x0075FA90
void MPPlayerManager::HandleNextRound(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdMessage> msg = receivedMsg.getMessage();
    bdReference<bdBitBuffer> buffer = msg.m_ptr->getPayload();
    ...
    {
        MPPlayer* p = (MPPlayer*)((char*)this + 0x1010 + 0x310 * i);
        if (*(bool*)((char*)p + 0x44)  // mInVehicle
            && p->mId < 0x10u && p->mConnection.m_ptr != nullptr
            && p->mClientIndex >= 0
            && EntityManager::sInst->GetPlayer(p->mClientIndex) != nullptr)
        {
            Entity* Player =
                EntityManager::sInst->GetPlayer(p->mClientIndex);
            ...
            if (v6 < 0x540
                && mVal >> 12
                       == EntityHandleDb::sInst.mElements[v6].mKey)
                mObject = EntityHandleDb::sInst.mElements[v6].mObject;
            MPVehicle* Vehicle = GetVehicle(mObject);
            if (Vehicle == nullptr)
            {
                AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
                AeAssert::gCurrentFile =
                    "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
                AeAssert::gCurrentLine = 5368;
                AeAssert::gCurrentExpr = "mpVehicle";
                if (!AeAssert::IsIgnored()
                    && AeAssert::Assert("Could not find vehicle"))
                    __debugbreak();
            ...
            if (mEntity != nullptr)
            {
                Vehicle->GetOutOfVehicle(p, mEntity->health, true);
            }
            else
            {
                *(bool*)((char*)p + 0x44) = false;
                *(int*)((char*)p + 0x40) = -1;
                *(unsigned char*)((char*)p + 0x3C) = 0xFF;
            }
        }
    }
    if (buffer.m_ptr->readDataType(bdBitBuffer::BD_BB_BOOL_TYPE))
    {
        ...
        if (buffer.m_ptr->readBits(&allowChange, 1u) && allowChange)
        {
            if (buffer.m_ptr != nullptr)
                ++buffer.m_ptr->m_refCount;
            MPUIInterface::mServerParams.Deserialize(buffer);
    ...
    for (int i = 0; i < 10; ++i)
    {
        ...
        if (mVehicles->mId < 0x0Au)
        {
            mVehicles->Reset(true);
            mVehicles->RespawnVehicle();
        }
    }
    if (*(bool*)((char*)this + 0x4112))
    {
        if (gpBrocAPI->mBrocExports.mCallbackNextRound != nullptr)
            gpBrocAPI->mBrocExports.mCallbackNextRound();
    }
    if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
        delete buffer.m_ptr;
    if (msg.m_ptr != nullptr && msg.m_ptr->m_refCount-- == 1)
    ...
    const bdReceivedMessage& receivedMsg)
{
    ...
    if (buffer.m_ptr != nullptr)
        ++buffer.m_ptr->m_refCount;
    MPUtility::ReadPlayerData(buffer, data);
    unsigned char v6 =
        *(unsigned char*)((char*)this + 0x4111 + data.localIdx);
    MPPlayer* v7 = nullptr;
    if (v6 < 0x10u)
        v7 = (MPPlayer*)((char*)this + 0x1010 + 0x310 * v6);
    buffer.m_ptr->m_readPosition = 1;
    HandlePlayerSetup(receivedMsg);
    if (v7 != nullptr && !fromLoopback)
    {
        MPPlayer* Player = GetPlayer(data.id);
        if (Player != nullptr && Player->mId < 0x10u
            && Player->mConnection.m_ptr != nullptr && Player->mMasterClient
            && dword_F6A290[0] == 2)
        {
            MPPlayer* localPlayer = GetLocalPlayer(0);
            bdMessage* playerDetails = new bdMessage(0x21u, false);
            bdReference<bdMessage> pdRef;
            pdRef.m_ptr = playerDetails;
            if (playerDetails != nullptr)
                ++playerDetails->m_refCount;
            extern int g_NumBdMessages;
            ++g_NumBdMessages;
            bdReference<bdBitBuffer> b2 = playerDetails->getPayload();
            if (b2.m_ptr != nullptr)
                ++b2.m_ptr->m_refCount;
            WritePlayerData(b2, localPlayer);
            if (playerDetails != nullptr)
                ++playerDetails->m_refCount;
            connection.m_ptr->send(pdRef, true);
            if (b2.m_ptr != nullptr && b2.m_ptr->m_refCount-- == 1)
                delete b2.m_ptr;
            if (pdRef.m_ptr != nullptr && pdRef.m_ptr->m_refCount-- == 1)
                delete pdRef.m_ptr;
        }
    }
    if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
...
// ea: 0x0074A9C0
void MPPlayerManager::HandleMelee(const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    if (Player != nullptr)
    {
            ...
            && *(bool*)((char*)this + 0x4112))
        {
            ...
            if (!MPUtility::ReadPlayerId(buffer, attackingPlayerID))
                goto cleanup;
            MPPlayer* v6 = GetPlayer(attackingPlayerID);
            if (v6 == nullptr)
                goto cleanup;
            Entity* ent = v6->mClientIndex >= 0
                              ? EntityManager::sInst->GetPlayer(
                                    v6->mClientIndex)
                              : nullptr;
            if (ent == nullptr)
                ...
                if (MPUtility::ReadPlayerId(buffer, attackingPlayerID))
                {
                    ...
                    if (MPUtility::ReadPosition(buffer, position))
                    {
                        if (buffer.m_ptr != nullptr)
                            ++buffer.m_ptr->m_refCount;
                        if (MPUtility::ReadNormal(buffer, normal)
                            && buffer.m_ptr->readChar8(
                                (char&)surfaceType)
                            && buffer.m_ptr->readInt16(damage)
                            && buffer.m_ptr->readUChar8(mod))
                        {
                            if (IsLocalId(attackingPlayerID))
                            {
                                MPPlayer* v12 = GetPlayer(attackingPlayerID);
                                Entity* hitPlayerEntity =
                                    v12 != nullptr && v12->mClientIndex >= 0
                                        ? EntityManager::sInst->GetPlayer(
                                              v12->mClientIndex)
                                        : nullptr;
                                if (!IsLocalPlayer(hitPlayerEntity))
                                {
                                    AeAssert::gCurrentAuthor =
                                        (AeAssert::ECoderId)0;
                                    AeAssert::gCurrentFile =
                                        "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
                                    AeAssert::gCurrentLine = 3134;
                                    AeAssert::gCurrentExpr =
                                ...
                                if (v12 == nullptr)
                                {
                                    ...
                                    AeAssert::gCurrentFile =
                                        "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
                                    AeAssert::gCurrentLine = 3135;
                                    AeAssert::gCurrentExpr = "hitPlayer";
                                    if (!AeAssert::IsIgnored()
                                        && AeAssert::Assert(
                                            "Hit player has invalid player."))
                                        __debugbreak();
                                    goto cleanup;
                                }
                                G_Damage(hitPlayerEntity, ent, ent,
                                         normal, position, damage, 128, mod,
                                         (hitLocation_t)surfaceType, -1);
                            }
                            const char* weaponType =
                                BG_GetInfoForWeapon(0)->szInternalName;
                            PostEffectEventWeapon(
                                ent, weaponType,
                                (EAction)24);  // binary kActionWEAPON_NOTE_TRACK_SOUND_B
                        }
                    }
                }
            }
            else
            {
                ...
                PostEffectEventWeapon(
                    ent, weaponType,
                    (EAction)23);  // binary kActionWEAPON_NOTE_TRACK_SOUND_A
            }
        cleanup:
            if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
                delete buffer.m_ptr;
...
// ea: 0x0075A570
void MPPlayerManager::LocalPlayerEnterGame()
{
    if (MPUIInterface::mKicked && gpBrocAPI != nullptr
        && gpBrocAPI->mBrocExports.mCallbackLocalPlayerKicked != nullptr)
    {
        gpBrocAPI->mBrocExports.mCallbackLocalPlayerKicked();
        MPUIInterface::mKicked = false;
    }
    else if (MPUIInterface::mHostDisconnected && gpBrocAPI != nullptr
             && gpBrocAPI->mBrocExports.mCallbackHostDisconnected
                    != nullptr)
    {
        gpBrocAPI->mBrocExports.mCallbackHostDisconnected();
        MPUIInterface::mHostDisconnected = false;
    }
    else
    {
        if (MPUIInterface::mHostMigrated && gpBrocAPI != nullptr)
        {
            if (gpBrocAPI->mBrocExports.mCallbackHostMigrated != nullptr)
            {
                gpBrocAPI->mBrocExports.mCallbackHostMigrated();
                MPUIInterface::mHostMigrated = false;
            }
        }
        unsigned char v5 = *(unsigned char*)((char*)this + 0x4111);
        *(bool*)((char*)this + 0x4112) = true;
        if (v5 >= 0x10u || GetLocalPlayer(0) == nullptr)
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile =
                "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
            AeAssert::gCurrentLine = 7914;
            AeAssert::gCurrentExpr = "GetLocalPlayer()";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert(
                    "Invalid local player on LocalPlayerEnterGame."))
                __debugbreak();
        }
        if (dword_F6A290[0] == 2)
            GetLocalPlayer(0)->UpdateInGamePlayerInfo(false, true);
        for (int i = 0; i < 16; ++i)
        {
            MPPlayer* p = (MPPlayer*)((char*)this + 0x1010 + 0x310 * i);
            if (p->mId < 0x10u && p->mConnection.m_ptr != nullptr
            ...
            {
                float origin[3] = {
                    p->mInterpolatedPosition.v.m128_f32[0],
                    p->mInterpolatedPosition.v.m128_f32[1],
                    p->mInterpolatedPosition.v.m128_f32[2],
                };
                float angles[3] = { 0.0f, 0.0f, 0.0f };
                angles[1] = *(float*)((char*)p + 0x218);
                ClientConnect(p, origin, angles);
                if (p->mClientIndex < 0)
                {
                    AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
                    AeAssert::gCurrentFile =
                        "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
                    AeAssert::gCurrentLine = 7938;
                    AeAssert::gCurrentExpr =
                        "mPlayers[i].GetClientIndex() >= 0";
                    if (!AeAssert::IsIgnored()
                        && AeAssert::Assert(
                ...
                {
                    Entity* Player = p->mClientIndex >= 0
                                         ? EntityManager::sInst->GetPlayer(
                                               p->mClientIndex)
                                         : nullptr;
                    if (*(bool*)((char*)p + 0x45))  // mPlaying
                    {
                        gpBrocAPI->mBrocExports.mCallbackPlayerJoin(
                            Player->mHandle.mHandle.mVal, 3,
                            Player->client->pers.playerClass);
                        Player->client->ps.pm_type = 0;
                    }
                    else
                    {
                        gpBrocAPI->mBrocExports.mCallbackPlayerJoin(
                ...
                if (gpBrocAPI->mBrocExports.mCallbackPlayerEnter
                    != nullptr)
                {
                    Entity* v13 = p->mClientIndex >= 0
                                      ? EntityManager::sInst->GetPlayer(
                                            p->mClientIndex)
                                      : nullptr;
                    gpBrocAPI->mBrocExports.mCallbackPlayerEnter(
                        v13->mHandle.mHandle.mVal, 0);
                }
                p->UpdateInGamePlayerInfo(false, true);
            }
        }
        ...
        {
            MPPlayer* mPlayers =
                (MPPlayer*)((char*)this + 0x1010 + 0x310 * j);
            if (mPlayers->mId < 0x10u
                && mPlayers->mConnection.m_ptr != nullptr
                && IsLocalId(mPlayers->mId))
                SendPlayerEnter(mPlayers);
        }
        *(bool*)((char*)this + 0x5868) = false;  // mGameParamsSet
    }
}
...
// ea: 0x0074A6A0
void MPPlayerManager::HandlePainFlinch(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    if (Player != nullptr)
    {
            ...
            && *(bool*)((char*)this + 0x4112))
        {
            bdReference<bdMessage> msg = receivedMsg.getMessage();
            bdReference<bdBitBuffer> buffer = msg.m_ptr->getPayload();
            unsigned char hitPlayerID = 0;
            unsigned char hitLocation = 0;
            bool bLowBlow = false;
            float normal[3] = { 0.0f, 0.0f, 0.0f };
            if (buffer.m_ptr != nullptr)
                ++buffer.m_ptr->m_refCount;
            if (MPUtility::ReadPlayerId(buffer, hitPlayerID))
            {
                if (buffer.m_ptr != nullptr)
                    ++buffer.m_ptr->m_refCount;
                if (MPUtility::ReadNormal(buffer, normal)
                    && buffer.m_ptr->readUChar8(hitLocation)
                    && buffer.m_ptr->readBool(bLowBlow))
                {
                    MPPlayer* v7 = GetPlayer(hitPlayerID);
                    if (v7 == nullptr)
                    {
                        if (buffer.m_ptr != nullptr
                            && buffer.m_ptr->m_refCount-- == 1)
                    ...
                    Entity* ent = v7->mClientIndex >= 0
                                      ...
                                      : nullptr;
                    if (ent != nullptr
                        && gpBrocAPI->mBrocExports.mCallbackPainFlinch
                               != nullptr)
                    {
                        gpBrocAPI->mBrocExports.mCallbackPainFlinch(
                            ent->mHandle.mHandle.mVal, hitLocation);
                    }
                    Client* client = ent->client;
                    if (client != nullptr)
                    {
                        int v5 = 0;
                        while (client->AnimIKPainEvents[v5].time != 0)
                        {
                            ++v5;
                            if (v5 >= 15)
                                break;
                        ...
                        if (v5 < 15)
                        {
                            client->AnimIKPainEvents[v5].time = level.time;
                            float rndNormal[3] = { normal[0], normal[1],
                                                   normal[2] };
                            rndNormal[0] =
                                ((float)random() * 2.0f - 1.0f) * 0.4f
                                + rndNormal[0];
                            rndNormal[1] =
                                ((float)random() * 2.0f - 1.0f) * 0.4f
                                + rndNormal[1];
                            rndNormal[2] =
                                ((float)random() * 2.0f - 1.0f) * 0.4f
                                + rndNormal[2];
                            VectorNormalize(rndNormal);
                            ent->client->AnimIKPainEvents[v5].dir[0] =
                                rndNormal[0];
                            ent->client->AnimIKPainEvents[v5].dir[1] =
                                rndNormal[1];
                            ent->client->AnimIKPainEvents[v5].dir[2] =
                                rndNormal[2];
                            ent->client->AnimIKPainEvents[v5].bIsLowBlow =
                                bLowBlow;
...
// ea: 0x00737DA0
bool MPPlayerManager::acceptSession(const bdReceivedMessage& receivedMsg)
{
    ...
    if (gLogAllPktTypes)
    {
        ...
        ((bdSession*)((char*)this + 0x4114))->getPeerIndex(conn, peerID);
        if (payload.m_ptr != nullptr && payload.m_ptr->m_refCount-- == 1)
            delete payload.m_ptr;
        if (conn.m_ptr != nullptr && conn.m_ptr->m_refCount-- == 1)
            delete conn.m_ptr;
    }
    if (index < 0 || index >= 0x54)
    {
        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
        AeAssert::gCurrentLine = 1238;
        AeAssert::gCurrentExpr =
            "index >= 0 && index < (MESSAGE_TYPE_NUM - ((28u)+1))";
        if (!AeAssert::IsIgnored()
            && AeAssert::Assert("BOLLOCKS"))
    ...
    void** callbacks = (void**)((char*)this + 0x55C8);
    if (callbacks[2 * index] != nullptr)
    {
        bool dispatch = true;
        if (type == 89 || type == 91 || type == 30 || type == 31)
        {
            dispatch = false;
        }
        else if (type == 32)
        {
            bdReference<bdConnection> conn = receivedMsg.getConnection();
            bdReference<bdCommonAddr> addr = conn.m_ptr->getAddress();
            bool isLoopback =
                addr.m_ptr != nullptr && addr.m_ptr->isLoopback();
            if (addr.m_ptr != nullptr && addr.m_ptr->m_refCount-- == 1)
                delete addr.m_ptr;
        ...
        if (type == 38 || type == 111 || type == 112)
            dispatch = false;
        if (*(bool*)((char*)this + 0x4112) || !dispatch)
        {
            void* fn = callbacks[2 * index];
            int thisDelta = (int)callbacks[2 * index + 1];
            typedef void (__thiscall* CbFn)(void*, const bdReceivedMessage*);
            ((CbFn)fn)((char*)this + thisDelta, &receivedMsg);
        }
        else if (type != 35 && type != 36 && type != 76 && type != 100
                 && *(int*)((char*)this + 0x0C) < 512)
        {
            int num = *(int*)((char*)this + 0x0C);
            bdReference<bdMessage> storedMsg = receivedMsg.getMessage();
            bdReference<bdConnection> storedConn =
                receivedMsg.getConnection();
            *(bdMessage**)((char*)this + 0x10 + 4 * num) = storedMsg.m_ptr;
            if (storedMsg.m_ptr != nullptr)
                ++storedMsg.m_ptr->m_refCount;
            *(bdConnection**)((char*)this + 0x810 + 4 * num) =
                storedConn.m_ptr;
            if (storedConn.m_ptr != nullptr)
                ++storedConn.m_ptr->m_refCount;
            ++*(int*)((char*)this + 0x0C);
        }
        if (message.m_ptr != nullptr && message.m_ptr->m_refCount-- == 1)
            delete message.m_ptr;
...
// ea: 0x0074CDA0
void MPPlayerManager::HandlePlayerReviveRequest(
    const bdReceivedMessage& receivedMsg)
{
    tlPrintf("Host: Received revive request from player\n");
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    if (Player != nullptr)
    {
            ...
            && m_ptr->getStatus() == bdConnection::BD_CONNECTED)
        {
            ...
            if (buffer.m_ptr != nullptr)
                ++buffer.m_ptr->m_refCount;
            if (!MPUtility::ReadPlayerId(buffer, medicPlayerID))
                goto cleanup;
            {
                MPPlayer* v6 = GetPlayer(respawningPlayerID);
                if (v6 == nullptr)
                {
                    AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
                    AeAssert::gCurrentFile =
                        "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
                    AeAssert::gCurrentLine = 5032;
                    AeAssert::gCurrentExpr = "respawningPlayer";
                    if (!AeAssert::IsIgnored()
                        && AeAssert::Assert(
                            "Client: Reviving player id does not match any player in session!\n"))
                        __debugbreak();
                    goto cleanup;
                }
                MPPlayer* medicPlayer = GetPlayer(medicPlayerID);
                if (medicPlayer == nullptr)
                {
                    AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
                    AeAssert::gCurrentFile =
                        "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
                    AeAssert::gCurrentLine = 5039;
                    AeAssert::gCurrentExpr = "medicPlayer";
                    if (!AeAssert::IsIgnored()
                        && AeAssert::Assert(
                            "Client: Medic player id does not match any player in session!\n"))
                        __debugbreak();
                    goto cleanup;
                }
                Entity* ent = v6->mClientIndex >= 0
                                  ? EntityManager::sInst->GetPlayer(
                                        v6->mClientIndex)
                                  : nullptr;
                if (IsLocalPlayer(ent) && IsACorpse(ent)
                    && ent->client->pers.playerState == 4)
                {
                    bdMessage* out = new bdMessage(0x5Cu, false);
                    bdReference<bdMessage> message;
                    message.m_ptr = out;
                    if (out != nullptr)
                        ++out->m_refCount;
                    extern int g_NumBdMessages;
                    ++g_NumBdMessages;
                    bdReference<bdBitBuffer> newBuffer = out->getPayload();
                    unsigned char id = v6->mId;
                    if (newBuffer.m_ptr != nullptr)
                        ++newBuffer.m_ptr->m_refCount;
                    MPUtility::WritePlayerId(newBuffer, id);
                    unsigned char medicId = medicPlayer->mId;
                    if (newBuffer.m_ptr != nullptr)
                        ++newBuffer.m_ptr->m_refCount;
                    MPUtility::WritePlayerId(newBuffer, medicId);
                    if (newBuffer.m_ptr != nullptr)
                        ++newBuffer.m_ptr->m_refCount;
                    MPUtility::WritePosition(
                        newBuffer, ent->r.currentOrigin.v.m128_f32);
                    float yaw = ent->r.currentAngles.v.m128_f32[1];
                    if (newBuffer.m_ptr != nullptr)
                        ++newBuffer.m_ptr->m_refCount;
                    MPUtility::WriteAngle(newBuffer, yaw);
                    newBuffer.m_ptr->writeBool(
                        ent->sentient->eTeam == TEAM_AXIS);
                    SendAll(message, true, false);
                    if (newBuffer.m_ptr != nullptr
                        && newBuffer.m_ptr->m_refCount-- == 1)
                        delete newBuffer.m_ptr;
                    if (message.m_ptr != nullptr
                        && message.m_ptr->m_refCount-- == 1)
                        delete message.m_ptr;
                }
        ...
        tlPrintf("Host: Reviving player not connected!\n");
    }
    else
    {
        tlPrintf("Host: Reviving player does not exist!\n");
    }
}

// ea: 0x0074C840
void MPPlayerManager::HandlePlayerDamage(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* player = GetPlayer(conn);
    ...
    if (buffer.m_ptr != nullptr)
        ++buffer.m_ptr->m_refCount;
    bool ok = MPUtility::ReadPlayerId(buffer, hitPlayerID);
    if (buffer.m_ptr->testBool())
    {
        if (!ok)
            goto cleanup;
    ...
    if (!MPUtility::ReadNormal(buffer, normal)
        || !buffer.m_ptr->readInt16(damage)
        || !buffer.m_ptr->readUChar8(mod)
        || !buffer.m_ptr->readInt16(dflags)
        || !buffer.m_ptr->readUChar8(hitLocation)
        || !buffer.m_ptr->readChar8(weapon))
        goto cleanup;
    {
        MPPlayer* hitPlayer = GetPlayer(hitPlayerID);
        if (hitPlayer == nullptr)
            goto cleanup;
        Entity* hitEntity = hitPlayer->mClientIndex >= 0
                                ? EntityManager::sInst->GetPlayer(
                                      hitPlayer->mClientIndex)
                                : nullptr;
        if (!IsLocalPlayer(hitEntity))
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile =
                "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
            AeAssert::gCurrentLine = 4648;
            AeAssert::gCurrentExpr = "IsLocalPlayer(hitPlayer->GetEntity())";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert(
                    "Hit player message came to the wrong player."))
                __debugbreak();
        ...
        if (player->mClientIndex >= 0)
            senderEnt =
                EntityManager::sInst->GetPlayer(player->mClientIndex);
        if (senderEnt == nullptr)
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile =
                "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
            AeAssert::gCurrentLine = 4651;
            AeAssert::gCurrentExpr = "sendingPlayer";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert("old cod assert"))
                __debugbreak();
        }
        MPPlayer* v10 = GetPlayer(attackerID);
        if (v10 != nullptr && v10->mClientIndex >= 0)
            senderEnt =
                EntityManager::sInst->GetPlayer(v10->mClientIndex);
        if (hitPlayer->mClientIndex >= 0)
        {
            Entity* hitEnt =
                EntityManager::sInst->GetPlayer(hitPlayer->mClientIndex);
            if (hitEnt != nullptr)
            {
                G_Damage(hitEnt, senderEnt, senderEnt, normal, position,
                         damage, dflags | 0x80, mod,
                         (hitLocation_t)hitLocation, weapon);
            }
        }
    }
cleanup:
...
// ea: 0x0075F1F0
void MPPlayerManager::HandlePlayerRevive(
    const bdReceivedMessage& receivedMsg)
{
    tlPrintf("Client: Received revive info from host\n");
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    MPPlayer* Player = GetPlayer(conn);
    if (Player != nullptr)
    {
            ...
            && m_ptr->getStatus() == bdConnection::BD_CONNECTED)
        {
            bdReference<bdMessage> msg = receivedMsg.getMessage();
            bdReference<bdBitBuffer> buffer = msg.m_ptr->getPayload();
            unsigned char respawningPlayerID = 16;
            unsigned char medicPlayerID = 16;
            float position[3] = { 0.0f, 0.0f, 0.0f };
            float angle = 0.0f;
            bool axisTeam = false;
            if (buffer.m_ptr != nullptr)
                ...
                if (MPUtility::ReadPlayerId(buffer, medicPlayerID))
                {
                    ...
                    if (MPUtility::ReadPosition(buffer, position))
                    {
                        ...
                        if (MPUtility::ReadAngle(buffer, angle)
                            && buffer.m_ptr->readBool(axisTeam))
                        {
                            MPPlayer* v6 = GetPlayer(respawningPlayerID);
                            MPPlayer* medicPlayer = GetPlayer(medicPlayerID);
                            if (v6 == nullptr)
                            ...
                            Entity* v7 = v6->mClientIndex >= 0
                                             ? EntityManager::sInst
                                                   ->GetPlayer(
                                                       v6->mClientIndex)
                                             : nullptr;
                            if (axisTeam
                                != (v7->sentient->eTeam == TEAM_AXIS))
                            {
                                if (buffer.m_ptr != nullptr
                                    && buffer.m_ptr->m_refCount-- == 1)
                                    delete buffer.m_ptr;
                            ...
                            v6->PlayerRevive(position, angles);
                            if (*(bool*)((char*)this + 0x4112))
                            {
                                tlPrintf("Client: Reviving client!\n");
                                Entity* ent = v6->mClientIndex >= 0
                                                  ? EntityManager::sInst
                                                        ->GetPlayer(
                                                            v6->mClientIndex)
                                                  : nullptr;
                                ClientSpawn(ent, position, angles, true,
                                            true);
                                Entity* v9 = v6->mClientIndex >= 0
                                                 ? EntityManager::sInst
                                                       ->GetPlayer(
                                                           v6->mClientIndex)
                                                 : nullptr;
                                if (IsLocalPlayer(v9))
                                {
                                    Entity* v10 = v6->mClientIndex >= 0
                                                      ? EntityManager::sInst
                                                            ->GetPlayer(
                                                                v6->mClientIndex)
                                                      : nullptr;
                                    cl_stance_ss[v10->GetPlayerIndex()] = 0;
                                    Entity* v11 = v6->mClientIndex >= 0
                                                      ? EntityManager::sInst
                                                            ->GetPlayer(
                                                                v6->mClientIndex)
                                                      : nullptr;
                                    v11->client->ps.pm_time = 1800;
                                    v11->client->ps.pm_flags &=
                                        0xBFFFDCFF;
                                    v11->client->ps.pm_flags |=
                                        0x40000000u;
                                }
                                if (gpBrocAPI->mBrocExports
                                        .mCallbackPlayerRevive != nullptr)
                                {
                                    tlPrintf(
                                        "Client: Dispatched revive info to script!\n");
                                    unsigned int medicVal = 0;
                                    if (medicPlayer != nullptr
                                        && medicPlayer->mClientIndex >= 0)
                                    {
                                        Entity* medEnt =
                                            EntityManager::sInst->GetPlayer(
                                                medicPlayer->mClientIndex);
                                        if (medEnt != nullptr)
                                            medicVal =
                                                medEnt->mHandle.mHandle.mVal;
                                    }
                                    unsigned int respawnVal = 0;
                                    if (v6->mClientIndex >= 0)
                                    {
                                        Entity* rEnt =
                                            EntityManager::sInst->GetPlayer(
                                                v6->mClientIndex);
                                        if (rEnt != nullptr)
                                            respawnVal =
                                                rEnt->mHandle.mHandle.mVal;
                                    }
                                    gpBrocAPI->mBrocExports
                                        .mCallbackPlayerRevive(respawnVal,
                                                               medicVal);
                                }
                                else
                                {
                                    tlPrintf(
                                        "Client: No revive callback registered!\n");
                                }
                            }
                            else
                            {
                                tlPrintf(
                                    "Client: Cannot act on revive info as local player not in game!\n");
                            }
                        }
                    }
                }
        ...
        {
            tlPrintf(
                "Client: Received revive info for non-connected player!\n");
        }
    }
    else
    {
        tlPrintf(
            "Client: Received revive info for player that does not exist!\n");
    }
}

// ea: 0x00739510
bool MPPlayerManager::VehicleEventProcessEvent(MPVehicleEvent* event)
{
    unsigned char v3 = *(unsigned char*)((char*)event + 9);  // playerId
    if (v3 >= 0x10u)
    {
        event->bUsed = false;
        event->eventType = VEHICLE_EVENT_ENTER;
        event->vehicleId = 0;
        event->sequence = 0;
        event->health = 0;
        event->seatIdx = 0;
    ...
    MPVehicle* v6 = (MPVehicle*)((char*)this + 0x4120
                                 + 0x210 * *(unsigned char*)((char*)event
                                                             + 8));
    MPPlayer* v7 = (MPPlayer*)((char*)this + 0x1010 + 0x310 * v3);
    if (v7->mId >= 0x10u || v7->mConnection.m_ptr == nullptr)
    {
        event->bUsed = false;
        event->eventType = VEHICLE_EVENT_ENTER;
        event->vehicleId = 0;
    ...
    unsigned int v8 = event->sequence;
    unsigned int playerSeq = v7->mVehicleEventSequence;
    if (playerSeq > v8)
    {
        if (v8 != 0
            || event->expireTime
    ...
    if (eventType == VEHICLE_EVENT_ENTER)
    {
        if (!v7->IsValid())
            goto done;
        if (v6->IsFullyOccupied())
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile =
                "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
            AeAssert::gCurrentLine = 6273;
            AeAssert::gCurrentExpr = "!vehicle.IsFullyOccupied()";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert("Vehicle already occupied"))
                __debugbreak();
        }
        if (playerSeq >= event->sequence)
            goto done;
        v6->GetInVehicle(v7, event->health, event->seatIdx, event->entryIdx);
        v7->mVehicleEventSequence = event->sequence;
        goto done;
    }
    int v10 = eventType - 1;
    if (v10 == 0)
    {
        if (!v7->IsInVehicle(v6->mId))
        {
            event->bUsed = false;
            event->eventType = VEHICLE_EVENT_ENTER;
            event->vehicleId = 0;
        ...
        v6->GetOutOfVehicle(v7, event->health, true);
        v7->mVehicleEventSequence = event->sequence;
        goto done;
    }
    if (v10 == 1)
    {
        int seatIdx = event->seatIdx;
        if (*(unsigned char*)((char*)v6 + 0x08 + seatIdx) != 16
            || (seatIdx == 1
                && *(unsigned char*)((char*)v6 + 0x0E) != 16))
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile =
                "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
            AeAssert::gCurrentLine = 6284;
            AeAssert::gCurrentExpr =
                "!vehicle.IsSeatOccupied( event->seatIdx )";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert("Cannot move to occupied seat"))
                __debugbreak();
        }
        if (playerSeq >= event->sequence)
            goto done;
        if (!v7->IsInVehicle(v6->mId))
        {
            v6->GetInVehicle(v7, event->health, event->seatIdx, 0);
            v7->mVehicleEventSequence = event->sequence;
            goto done;
        }
        v6->SeatChange(v7, event->seatIdx);
        v7->mVehicleEventSequence = event->sequence;
        goto done;
    }
    AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
    AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPPlayerMgr.cpp";
    AeAssert::gCurrentLine = 6323;
    AeAssert::gCurrentExpr = "0";
    if (!AeAssert::IsIgnored()
        && AeAssert::Assert("Invalid vehicle event sequence type"))
        __debugbreak();
    return false;
done:
    event->bUsed = false;
...
// ea: 0x0074D710
void MPPlayerManager::HandleVehicleRequestOwnership(
    const bdReceivedMessage& receivedMsg)
{
    bdReference<bdConnection> conn = receivedMsg.getConnection();
    GetPlayer(conn);
               ...
               == bdSession::BD_SESSION_HOST)
    {
        ...
        if (MPUtility::ReadVehicleId(buffer, vehicleId))
        {
            if (buffer.m_ptr != nullptr)
                ++buffer.m_ptr->m_refCount;
            okRead = MPUtility::ReadPlayerId(buffer, playerId);
        }
                   ...
                   == bdConnection::BD_CONNECTED)
        {
            if (okRead)
            {
                MPVehicle* v10 = (MPVehicle*)((char*)this + 0x4120
                                              + 0x210 * vehicleId);
                Entity* mEntity = (Entity*)v10->mEntity;
                if (mEntity->scr_vehicle->seats[0].occupant.mHandle.mVal
                    == 0)
                {
                    Entity* ent = Player->mClientIndex >= 0
                                      ? EntityManager::sInst->GetPlayer(
                                      ...
                                      : nullptr;
                    mEntity->scr_vehicle->AssignPhysics(ent);
                    bdMessage* out = new bdMessage(0x63u, false);
                    bdReference<bdMessage> message;
                    message.m_ptr = out;
                    if (out != nullptr)
                        ++out->m_refCount;
...
// ============================================================================
// mp.o batch 19 - utilities / serialization / UI (mp.o misc)
// ============================================================================
// ea: 0x0073C540
bool MPUtility::ReadPlayerData(bdReference<bdBitBuffer> buffer,
                               PlayerData& playerData)
{
    bool ok = false;
    if (buffer.m_ptr != nullptr)
        ++buffer.m_ptr->m_refCount;
    if (MPUtility::ReadPlayerId(buffer, playerData.id))
    {
        if (buffer.m_ptr != nullptr)
            ++buffer.m_ptr->m_refCount;
        if (MPUtility::ReadPosition(buffer, playerData.origin))
        {
            ok = true;
            if (buffer.m_ptr != nullptr)
                ++buffer.m_ptr->m_refCount;
            if (!MPUtility::ReadAnglesYawPitch(buffer, playerData.angles))
            {
                ok = false;
            }
            else
            {
                ...
                else
                {
                    ...
                    if (!buffer.m_ptr->readBits(&playing, 1u))
                    {
                        ok = false;
                    }
                    else
                    {
                        playerData.playing = playing != 0;
                        ok = true;
                        if (buffer.m_ptr != nullptr)
                            ++buffer.m_ptr->m_refCount;
                        if (!MPUtility::ReadVehicleId(buffer,
                                                      playerData.vehicleId))
                        {
                            ok = false;
                        }
                        else
                        {
                            ok = true;
                            if (!buffer.m_ptr->readRangedInt32(
                                    playerData.vehicleHealth, 0, 2000))
                            {
                                ok = false;
                            }
                            else
                            {
                                ...
                                if (buffer.m_ptr != nullptr)
                                    ++buffer.m_ptr->m_refCount;
                                if (MPUtility::ReadPlayerClass(
                                        buffer, playerData.playerClass)
                                    && (ok = true,
                                        buffer.m_ptr->readString(
                                            playerData.name, 0x20u))
                                    && (ok = true,
                                        buffer.m_ptr->readDataType(
                                            bdBitBuffer::
                                                BD_BB_SIGNED_INTEGER32_TYPE))
                                    && buffer.m_ptr->readBits(
                                        &playerData.vehicleEventSequence,
                                        0x20u)
                                    && (ok = true,
                                        buffer.m_ptr->readDataType(
                                            bdBitBuffer::
                                    ...
                                    && buffer.m_ptr->readBits(
                                        &playerData.localIdx, 8u)
                                    && (ok = true,
                                        buffer.m_ptr->readDataType(
                                            bdBitBuffer::BD_BB_BOOL_TYPE))
                                    && (playing = 0,
                                        buffer.m_ptr->readBits(&playing,
                                                               1u)))
                                {
                                    playerData.teamAxis = playing != 0;
                                    ok = true;
                                }
                                else
                                {
    ...
    if (LiveWrapper::theWrapper->sessionState == kInSession)
    {
        ok = ok && buffer.m_ptr->readBits(&playerData.livePlayer, 0x200u)
             && (ok = true, buffer.m_ptr->readBool(playerData.wasInvited));
    }
    bool v4 = ok;
    if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
        delete buffer.m_ptr;
...
// ea: 0x0073CD60
void sServerCreateParams::Deserialize(bdReference<bdBitBuffer> buffer)
{
    int v3 = 0;
    bool v4 = true;
    do
        v4 = v4
             && buffer.m_ptr->readDataType(
                 bdBitBuffer::BD_BB_SIGNED_CHAR8_TYPE)
             && buffer.m_ptr->readBits(&this->mName[v3++], 8u);
    while (v3 < 24);
    if (v4
        && buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
        && buffer.m_ptr->readBits(&this->mMapID, 8u)
        && buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
        && buffer.m_ptr->readBits(&this->mGameType, 8u)
        && buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
        && buffer.m_ptr->readBits(&this->mGameSubType, 8u)
        && buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
        && buffer.m_ptr->readBits(&this->mMaxPlayers, 8u)
        && buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
        && buffer.m_ptr->readBits(&this->mTeamBalancing, 8u)
        && buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
        && buffer.m_ptr->readBits(&this->mFriendlyFire, 8u)
        && buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
        && buffer.m_ptr->readBits(&this->mMapRotation, 8u)
        && buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
        && buffer.m_ptr->readBits(&this->mTimeLimit, 8u)
        && buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
        && buffer.m_ptr->readBits(&this->mScoreLimit, 8u)
        && buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
        && buffer.m_ptr->readBits(&this->mRoundLimit, 8u)
        && buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
        && buffer.m_ptr->readBits(&this->mSwapEnds, 8u)
        && buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
        && buffer.m_ptr->readBits(&this->mRespawnTime, 8u)
        && buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
        && buffer.m_ptr->readBits(&this->mEnableAARVote, 8u)
        && buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE)
        && buffer.m_ptr->readBits(&this->mEnablePenaltyVote, 8u)
        && buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_UNSIGNED_CHAR8_TYPE))
    {
        buffer.m_ptr->readBits(&this->mPrivateSlots, 8u);
    }
    if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
        delete buffer.m_ptr;
}
...
// ea: 0x00751720
void MPPlayer::serializeVehicle(bdReference<bdBitBuffer> buffer)
{
    int mClientIndex = this->mClientIndex;
    int v3 = 0;
    if (mClientIndex == -1)
    ...
    Entity* v5 = mClientIndex >= 0
                     ? EntityManager::sInst->GetPlayer(mClientIndex)
                     : nullptr;
    if (v5->client->mVehicleAnimMoving)
    {
        if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
            delete buffer.m_ptr;
        return;
    ...
    bdBitBuffer* m_ptr = buffer.m_ptr;
    if (!IsPlayerFullySeatedInVehicle(v5) || v5->IsCameraTweening())
    {
        buffer.m_ptr->writeDataType(bdBitBuffer::BD_BB_BOOL_TYPE);
        unsigned char b = 0;
        buffer.m_ptr->writeBits(&b, 1u);
    ...
    unsigned int mVal = v5->r.mOwner.mHandle.mVal;
    unsigned int v12 = mVal & 0xFFF;
    if (v12 < 0x540
        && mVal >> 12 == EntityHandleDb::sInst.mElements[v12].mKey
        && EntityHandleDb::sInst.mElements[v12].mObject != nullptr
        && this->mVehSeatIdx == 1 && this->IsLocalPlayer())
    {
        scr_vehicle_t* scr = ((Entity*)EntityHandleDb::sInst.mElements[v12]
                                  .mObject)
                                 ->scr_vehicle;
        if (scr->seats[1].overheating)
            v3 = 2;
        else if (scr->seats[1].firing)
            v3 = 1;
    }
    buffer.m_ptr->writeRangedInt32(v3, 0, 3);
    unsigned char weapon = (unsigned char)v5->client->ps.weapon;
    buffer.m_ptr->writeDataType(bdBitBuffer::BD_BB_SIGNED_CHAR8_TYPE);
    buffer.m_ptr->writeBits(&weapon, 8u);
    if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
        delete buffer.m_ptr;
...
// ea: 0x0075C960
struct MPPlayerYawEntry {
    float mLegsYawAngle;
    float mLastLegsYawAngle;
    int   mLegsYawing;
    int   mLastLegsDirection;
    kuju::knet::sTime mLastLegsDirectionChange;
    float mLift;
};
...
void MPPlayer::PlayerSpawn(float* position, float* angles)
{
    ...
    this->mNetPosition.v.m128_f32[1] = position[1];
    this->mNetPosition.v.m128_f32[2] = position[2];
    ...
    float v6 = angles[1];
    this->mNetHeading = v6;
    if (v6 < -180.0f || v6 > 180.0f)
        v6 = fmod(v6 + 180.0f, 360.0f) - 180.0f;
    *(float*)((char*)this + 0x218) = v6;  // mInterpolatedHeading
    *(float*)((char*)this + 0x210) = 0.0f;  // mInterpolatedPitch
    *(float*)((char*)this + 0x214) = 0.0f;  // mInterpolatedLean
    memset(&this->mInterpolatedSpeed, 0, sizeof(math::Dir3));
    memset(&this->mNetSpeed, 0, sizeof(math::Dir3));
    this->mPlaying = true;
    int mTime = MultiplayerMgr::sInst->getLocalTime().mTime;
    this->mLastReceivedTime.mTime = mTime;
    this->mLastInterpolatedTime.mTime = mTime;
    this->mLastLegsYawTime.mTime = mTime;
    this->mStarvationTime.mTime = mTime;
    ...
    *(float*)((char*)this + 0x260) =
        *(float*)((char*)this + 0x218);  // mLastHeadingAngle
    yaw[0].mLastLegsDirectionChange.mTime =
        MultiplayerMgr::sInst->getLocalTime().mTime;
    memcpy(&yaw[1], &yaw[0], sizeof(MPPlayerYawEntry));
    memcpy(&yaw[2], &yaw[0], sizeof(MPPlayerYawEntry));
    OnModified();
    float t = this->mLastInterpolatedTime.mTime * 0.001f;
    *(float*)((char*)this + 0x1D4) = t;  // mInitialDate
    *(float*)((char*)this + 0x1D8) = 0.0f;  // mTimeInterval
    memcpy((char*)this + 0x180, &this->mInterpolatedPosition,
           sizeof(math::Position3));
    *(float*)((char*)this + 0x190) = t;  // mInitialTime
    memcpy((char*)this + 0x1A0, &this->mNetPosition,
           sizeof(math::Position3));
    *(float*)((char*)this + 0x1B0) = t;  // mFinalTime
    float* speed = (float*)((char*)this + 0x1C0);  // mLinearSpeed
    float inv = *(float*)((char*)this + 0x1D8) != 0.0f
                    ? 1.0f / *(float*)((char*)this + 0x1D8)
                    : 0.0f;
    for (int k = 0; k < 4; ++k)
        speed[k] = (this->mNetPosition.v.m128_f32[k]
                    - this->mInterpolatedPosition.v.m128_f32[k])
                   * inv;
    *(int*)((char*)this + 0x1D0) =
        kuju::cBezierTrajectoryInterpolator::kInterpolationLinear;
    if (this->mClientIndex >= 0)
    {
        Entity* Player = EntityManager::sInst->GetPlayer(this->mClientIndex);
        this->mNetWeapon = Player->client->ps.weapon;
        Player->s.apos.trBase[0] = angles[0];
        Player->s.apos.trBase[1] = angles[1];
        Player->s.apos.trBase[2] = angles[2];
        Player->s.apos.trDelta[0] = angles[0];
        Player->s.apos.trDelta[1] = angles[1];
        Player->s.apos.trDelta[2] = angles[2];
        Player->r.currentAngles.v.m128_f32[0] = angles[0];
        Player->r.currentAngles.v.m128_f32[1] = angles[1];
        Player->r.currentAngles.v.m128_f32[2] = angles[2];
...
void MPPlayer::PlayerRevive(float* position, float* angles)
{
    ...
    *(int*)((char*)this + 0x1D0) =
    ...
    if (this->mClientIndex >= 0)
    {
        ...
        {
            AnimationPlayer* v19 =
                (AnimationPlayer*)ent->mDObj->animPlayers[0];
            if (v19 != nullptr)
            {
                if (*(int*)((char*)this + 0x27C) == 0)  // mLastAnimEvent
                {
                    if (*(bool*)((char*)this + 0x60)
                        && !v19->IsPartialIdle(true))
                        return;
                    MP_ANIM_INDEX* v21 = nullptr;
                    int animIndex = base_anim_indices[1].anims[0].animIndex;
                    if (base_anim_indices[1].anims[0].animIndex != 0)
                    {
                        v21 = &base_anim_names[animIndex];
                        if (v21->anim == nullptr
                            && base_anim_indices[1].anims[0].animIndex != 0)
                            v21 = &base_anim_names[animIndex];
                    }
                    nalGeneric::nalGenericAnim* anim =
                        v21 != nullptr ? v21->anim : nullptr;
                    v19->StopAnims();
                    if (anim != nullptr)
                        v19->Play(anim, true, 0.0f, nullptr, 0.0f, nullptr,
                                  1.0f, 1.0f);
                    animRate = 2.0f;
                }
                *(int*)((char*)this + 0x27C) = 0;
                ent->client->ps.eFlags |= 0x80000000;
                MP_ANIM_INDEX* v23 =
                    MPPlayer::getAnimIndex(10, 0, 0, false);
                if (v23 != nullptr)
                {
                    nalGeneric::nalGenericAnim* anim = v23->anim;
                    if (anim != nullptr)
                    {
                        v19->StopModifiers(0xFE00000A);
                        if ((v23->flags & 1) != 0)
                            v19->PlayModifier(
                                anim, AnimationPlayer::nalAdditiveModifier,
                                1.0f, 0xFE00000A);
                        else
                            v19->PlayModifier(anim, 1.0f, 0xFE00000A);
                        v19->SetModifierSpeed(0xFE00000A, 1.0f, anim,
                                              animRate);
                    }
                    *(bool*)((char*)this + 0x60) = true;
                }
                                          ...
                                          eGameType gameType)
{
    ...
    case SETTING_TIME_LIMIT:
        ...
        case GAME_TYPE_DM:
        case GAME_TYPE_DOM:
            result = GetOptionFromValue(setting, 20, gameType);
            break;
        case GAME_TYPE_SND:
            result = GetOptionFromValue(setting, 4, gameType);
            break;
        default:
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile =
            ...
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert("old cod assert"))
                __debugbreak();
            result = GetOptionFromValue(setting, 0, gameType);
            break;
        }
        break;
    case SETTING_SCORE_LIMIT:
        ...
        case GAME_TYPE_TDM:
        case GAME_TYPE_DOM:
            result = GetOptionFromValue(setting, 100, gameType);
            break;
        case GAME_TYPE_CTF:
        case GAME_TYPE_SCF:
            result = GetOptionFromValue(setting, 5, gameType);
            break;
        case GAME_TYPE_HQ:
            result = GetOptionFromValue(setting, 300, gameType);
            break;
        case GAME_TYPE_DM:
            result = GetOptionFromValue(setting, 20, gameType);
            break;
        ...
        break;
    case SETTING_ROUND_LIMIT:
        result = GetOptionFromValue(setting, 1, gameType);
        break;
    case SETTING_RESPAWN_TIME:
        switch (gameType)
        {
        ...
        case GAME_TYPE_DOM:
        case GAME_TYPE_SND:
            result = GetOptionFromValue(setting, 10, gameType);
            break;
        case GAME_TYPE_TDM:
            result = GetOptionFromValue(setting, 5, gameType);
            break;
    ...
    case SETTING_LAST_MAN_STANDING:
        ...
        break;
    case SETTING_MAX_PLAYERS:
        result = GetOptionFromValue(setting, 16, gameType);
        break;
    default:
        AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
        AeAssert::gCurrentFile = "c:\\cod\\code\\game\\mp/MPUIInterface.cpp";
        AeAssert::gCurrentLine = 1543;
        AeAssert::gCurrentExpr = "0";
        if (!AeAssert::IsIgnored()
            && AeAssert::Assert("old cod assert"))
            __debugbreak();
...
// ea: 0x0073D6C0 (SAXXZ: void return per binary mangling)
void MPUIInterface::ResolveVote()
{
    MPPeer* mPeer = MultiplayerMgr::sInst->mPeer;
    bool endRound = false;
    MPPlayerManager* p_mPlayerManager =
    ...
    bdMessage* msg = new bdMessage(0x54u, false);
    bdReference<bdMessage> message;
    message.m_ptr = msg;
    if (msg != nullptr)
        ++msg->m_refCount;
    extern int g_NumBdMessages;
    ++g_NumBdMessages;
    int CurrentPlayerCount = p_mPlayerManager->GetCurrentPlayerCount();
    unsigned char mYesVotes = currVote->mYesVotes;
    unsigned char mNoVotes = currVote->mNoVotes;
    if (CurrentPlayerCount / 3 > mNoVotes + mYesVotes || mYesVotes <= 1u
        || mYesVotes <= mNoVotes)
    {
        bdReference<bdBitBuffer> buffer = msg->getPayload();
        buffer.m_ptr->writeDataType(bdBitBuffer::BD_BB_BOOL_TYPE);
    ...
    else
    {
        ...
        if (buffer.m_ptr != nullptr && buffer.m_ptr->m_refCount-- == 1)
            delete buffer.m_ptr;
        switch (currVote->mVoteType)
        {
        case kKickPlayer:
        {
            MPPlayerSet kickedPlayers(currVote->voteSubject);
            if (kickedPlayers.mBitPlayers != 0)
            {
                bdMessage* kick = new bdMessage(0x50u, false);
                bdReference<bdMessage> kickRef;
                kickRef.m_ptr = kick;
                if (kick != nullptr)
                    ++kick->m_refCount;
                extern int g_NumBdMessages;
                ++g_NumBdMessages;
                MPPlayerSet set(kickedPlayers);
                p_mPlayerManager->Send(kickRef, set, true);
                if (kickRef.m_ptr != nullptr
                    && kickRef.m_ptr->m_refCount-- == 1)
                    delete kickRef.m_ptr;
            }
            break;
        }
        case kMapVote:
            MPUIInterface::mNextServerParams.mMapID =
                currVote->voteSubject;
            endRound = true;
            break;
        case kGameModeVote:
            MPUIInterface::mNextServerParams.mGameType =
            ...
            MPUIInterface::mNextServerParams.mRespawnTime =
                (unsigned char)MPUIInterface::GetDefaultOption(
                    SETTING_RESPAWN_TIME,
                    (eGameType)MPUIInterface::mNextServerParams.mGameType);
            MPUIInterface::mNextServerParams.mScoreLimit =
                (unsigned char)MPUIInterface::GetDefaultOption(
                    SETTING_SCORE_LIMIT,
                    (eGameType)MPUIInterface::mNextServerParams.mGameType);
            MPUIInterface::mNextServerParams.mTimeLimit =
                (unsigned char)MPUIInterface::GetDefaultOption(
                    SETTING_TIME_LIMIT,
                    (eGameType)MPUIInterface::mNextServerParams.mGameType);
            endRound = true;
    ...
    if (endRound)
        gpBrocAPI->mBrocExports.mCallbackHostOptionsChanged(1);
    if (message.m_ptr != nullptr && message.m_ptr->m_refCount-- == 1)
        delete message.m_ptr;
}

// ea: 0x00761EA0
bool MPPlayerItems::DeserializeDropWeapon(bdReference<bdBitBuffer> buffer)
{
    char weapon = 0;
    int netIndex = 0;
    float position[3] = { 0.0f, 0.0f, 0.0f };
    ...
    if (!buffer.m_ptr->readDataType(bdBitBuffer::BD_BB_SIGNED_CHAR8_TYPE)
        ...
        || !buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER32_TYPE)
        || !buffer.m_ptr->readBits(&netIndex, 0x20u))
        goto cleanup;
    if (buffer.m_ptr != nullptr)
        ++buffer.m_ptr->m_refCount;
    if (!MPUtility::ReadPosition(buffer, position))
    ...
    if (buffer.m_ptr != nullptr)
        ++buffer.m_ptr->m_refCount;
    if (MPUtility::ReadVector(buffer, velocity)
        && buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER16_TYPE)
        && buffer.m_ptr->readBits(&clipCount, 0x10u)
        && buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_SIGNED_INTEGER16_TYPE)
        && buffer.m_ptr->readBits(&ammoCount, 0x10u))
    {
        ok = true;
        if (netIndex == 0)
        {
            ...
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert("Invalid handle"))
                __debugbreak();
        }
        int peerId = netIndex >> 11;
        if ((peerId - 1) >= 0x10)
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile =
            ...
            AeAssert::gCurrentExpr =
                "handle.GetPeerId() >= 0 && handle.GetPeerId() < 16";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert("Invalid peer id"))
                __debugbreak();
        }
        MPPlayer* Player =
            ((MPPlayerManager*)((char*)MultiplayerMgr::sInst->mPeer
                                + 0x74E0))
            ->GetPlayer((int)((peerId & 0x1F) - 1));
        Entity* owner = nullptr;
        if (Player != nullptr && Player->mClientIndex >= 0)
            owner = EntityManager::sInst->GetPlayer(Player->mClientIndex);
        if (weapon == 0)
        {
            AeAssert::gCurrentAuthor = (AeAssert::ECoderId)0;
            AeAssert::gCurrentFile =
            ...
            AeAssert::gCurrentExpr = "weapon != 0";
            if (!AeAssert::IsIgnored()
                && AeAssert::Assert(
                    "Recieved weapon none in weapon drop message"))
                __debugbreak();
        }
        Entity* v8 = LaunchItem(CurPakId(), &bg_itemlist[weapon], position,
                                angles, velocity, owner);
        if (v8 != nullptr && MultiplayerMgr::sInst->mPeer != nullptr)
            ((MPPlayerManager*)((char*)MultiplayerMgr::sInst->mPeer
                                + 0x74E0))
                ->RegisterDroppedItem(
                    (EDroppedItemTypes)kItemTypeWeapons, v8, owner,
...
// ea: 0x007621A0
bool MPPlayerItems::DeserializeFireMissile(bdReference<bdBitBuffer> buffer,
                                           bool do_effects)
{
    float position[3] = { 0.0f, 0.0f, 0.0f };
    float dir[3] = { 0.0f, 0.0f, 0.0f };
    ...
    if (buffer.m_ptr != nullptr)
        ++buffer.m_ptr->m_refCount;
    if (!MPUtility::ReadVector(buffer, dir)
        || !buffer.m_ptr->readDataType(
            bdBitBuffer::BD_BB_SIGNED_CHAR8_TYPE)
        || !buffer.m_ptr->readBits(&weapon, 8u))
        goto cleanup;
                    ...
                    if (MPUtility::ReadEntityHandle(buffer, handle))
                    {
                        ...
                        Entity* v8 = Player->mClientIndex >= 0
                                         ? EntityManager::sInst->GetPlayer(
                                               Player->mClientIndex)
                                         : nullptr;
                        if (InfoForWeapon->type == WEAPTYPE_GRENADE)
                        {
                            float vTossVel[3];
                            vTossVel[0] =
                                (float)InfoForWeapon->iProjectileSpeed
                                * dir[0];
                            vTossVel[1] =
                                (float)InfoForWeapon->iProjectileSpeed
                                * dir[1];
                            vTossVel[2] =
                                (float)InfoForWeapon->iProjectileSpeed
                                    * dir[2]
                                + (float)InfoForWeapon->iProjectileSpeedUp;
                            Entity* v11 =
                                fire_grenade(v8, position, vTossVel, weapon,
                                             InfoForWeapon->iFuseTime);
                            VectorNormalize(vTossVel);
                            float v12 =
                                (v8->client->ps.velocity.v.m128_f32[2]
                                     * dir[2]
                                 + v8->client->ps.velocity.v.m128_f32[1]
                                       * dir[1])
                                + v8->client->ps.velocity.v.m128_f32[0]
                                      * dir[0];
                            v11->s.pos.trDelta[0] += v12 * dir[0];
                            v11->s.pos.trDelta[1] += v12 * dir[1];
                            v11->s.pos.trDelta[2] += v12 * dir[2];
                        }
                        else
                        {
                            if (InfoForWeapon->type != WEAPTYPE_PROJECTILE)
                            {
                                if (InfoForWeapon->type == 10 /* WEAPTYPE_NUM */)
                                {
                                    Entity* v9 =
                                        fire_mine(v8, position, dir,
                                                  weapon);
                                    if (MultiplayerMgr::sInst->mPeer
                                        != nullptr)
                                        ((MPPlayerManager*)((char*)
                                                                MultiplayerMgr::
                                                                    sInst
                                                                        ->mPeer
                                                            + 0x74E0))
                                ...
                                if (do_effects)
                                    CG_FireWeapon(v8, &v8->s, 186, 0);
                                if (buffer.m_ptr != nullptr
                                    && buffer.m_ptr->m_refCount-- == 1)
                                    delete buffer.m_ptr;
                                return ok;
                            }
                            fire_rocket(v8, position, dir, 10.0f);
                        }
                        if (do_effects)
                            CG_FireWeapon(v8, &v8->s, 186, 0);
                    }
...
// ea: 0x00733EB0
void MPProfileMainMenu::OnSelectionChange()
{
    int menuClass = this->mMenuStatus[0];
    for (int v3 = 0; v3 < 6; ++v3)
    {
        if (this->mMenuStatus[v3] == 0)
        {
            this->mMenuStatus[v3] = 2;
            const char* STBString =
                STBManager::sInst->GetSTBString("FEMENU_PROFILE_EMPTY");
            this->entries[v3]->SetText(STBString);
        }
    }
    int highlighted = this->highlighted;
    int v7 = this->mMenuStatus[highlighted];
    if (v7 == 2)
    {
        this->mMenuStatus[highlighted] = 0;
        const char* v10 =
            STBManager::sInst->GetSTBString("FEMENU_PROFILE_CREATE");
        this->entries[this->highlighted]->SetText(v10);
    }
    FEText* TextPointer =
        this->mPanel->GetTextPointer("text_current_selected");
    {
        Broc::string v37 = this->entries[this->highlighted]->GetText();
        const char* v14 =
            v37.mBlock != nullptr ? v37.GetBuff() : defaultFileName;
        const char* v16 = STBManager::sInst->GetSTBString(v14);
        TextPointer->SetText(v16);
    }
    if (menuClass == 1)
    {
        {
            Broc::string v37b = this->entries[this->highlighted]->GetText();
        }
        SaveGameData* v34 = this->mSaveSlots[this->highlighted];
        ProfileManager::Me()->SetProfile(v34);
        FEText* v19 =
            this->mPanel->GetTextPointer("text_option_set_01");
        v19->SetShown(true);
        const char* v35;
        int stickConfig =
            gSaveGameData[controller::inst()->locked_port]
        ...
        {
        case 0:
            v35 = "FEMENU_COP_STICK_DEFAULT";
            break;
        case 1:
            v35 = "FEMENU_COP_STICK_LEGACY";
            break;
        case 2:
            v35 = "FEMENU_COP_STICK_SOUTHPAW";
            break;
        case 3:
            v35 = "FEMENU_COP_STICK_LEGACYSOUTHPAW";
            break;
        default:
            v35 = nullptr;
            break;
        ...
        if (v35 != nullptr)
            v19->SetText(STBManager::sInst->GetSTBString(v35));
        FEText* v22 = this->mPanel->GetTextPointer("text_option_set_02");
        int buttonConfig =
            gSaveGameData[controller::inst()->locked_port]
                .mStubData.mControllerButtonConfiguration;
        v22->SetText(STBManager::sInst->GetSTBString(
            MPOptionsControlsMenu::kButtonLayoutStrings[buttonConfig]));
        v22->SetShown(true);
        FEText* v26 = this->mPanel->GetTextPointer("text_option_01");
        v26->SetShown(true);
        FEText* v27 = this->mPanel->GetTextPointer("text_option_02");
        v27->SetShown(true);
        {
            Broc::string v36 = this->entries[this->highlighted]->GetText();
            if (v36.mBlock != nullptr)
                this->mSelectedProfile = v36.GetBuff();
            else
                this->mSelectedProfile = defaultFileName;
        }
    }
    else
    {
        this->mPanel->GetTextPointer("text_option_01")->SetShown(false);
